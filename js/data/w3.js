...
(function (root) {
  ...
  const LS = {
    zero10: 1000 / 1.05 ** 10, zeroG: 5000 / 1.04 ** 8, zeroSemi: 1000 / 1.03 ** 10,             // L1
    b7: [FIN.pvAnnuity(70, 0.06, 10), 1000 / 1.06 ** 10],                                        // L2: 7% coupon, 10 years, 6% yield
    tbl7: [0.06, 0.07, 0.08].map((y) => FIN.bondPrice(1000, 0.07, y, 10)),
    b2G: FIN.bondPrice(1000, 0.06, 0.075, 8),
    s9: [FIN.pvAnnuity(45, 0.04, 14), 1000 / 1.04 ** 14],                                        // L3: 9% semi-annual, 7 years, 8%
    s3G: FIN.bondPrice(1000, 0.07, 0.08, 6, 2),
    p5: r2(FIN.bondPrice(1000, 0.08, 0.09, 5)), p7: r2(FIN.bondPrice(1000, 0.07, 0.08, 6, 2)),   // L4 prices
    later: FIN.bondPrice(1000, 0.06, 0.05, 8), laterG: FIN.bondPrice(1000, 0.08, 0.10, 15, 2),  // L5
    var1: FIN.ddmMulti(1, [0.15, 0.15], 0.04, 0.12), varG: FIN.ddmMulti(0.5, [0.2, 0.2, 0.2], 0.05, 0.11), // L8
    pn: FIN.tvm.solveFV(4, 0.10, -20, 1.5),
  };
  LS.y5 = FIN.bondYieldPeriodic(LS.p5, 1000, 80, 5);
  LS.y7 = FIN.bondYieldPeriodic(LS.p7, 1000, 35, 12);            // per half-year
  ...
  const PACK = {
    ...
    nodes: [
      { id: 'w3-L1', kind: 'lesson', name: 'What a bond is', lesson: 'w3-L1' },
      { id: 'w3-L2', kind: 'lesson', name: 'Pricing a coupon bond', lesson: 'w3-L2' },
      ...
      { id: 'w3-L3', kind: 'lesson', name: 'Semi-annual coupons', lesson: 'w3-L3' },
      { id: 'w3-L4', kind: 'lesson', name: 'Yield to maturity', lesson: 'w3-L4' },
      { id: 'w3-L5', kind: 'lesson', name: 'Interest-rate risk', lesson: 'w3-L5' },
      ...
      { id: 'w3-L8', kind: 'lesson', name: 'Variable growth: the 4-step method', lesson: 'w3-L8' },
    ...
    lessons: {
      'w3-L1': {
        ...
        goal: R`Name the parts of a bond, and price the simplest bond: a zero-coupon bond.`,
        ...
        cards: [
          ...
          { kind: 'example', title: 'Worked example', q: R`A zero-coupon bond pays $1,000 in 10 years. The yield is 5% p.a. What is it worth today?`,
            ...
            ti: [TI.solver({ N: 10, I: 5, Pmt: 0, FV: 1000, PpY: 1, CpY: 1 }, 'PV', { note: R`No coupons, so \(Pmt = 0\). The minus sign means it is the price you pay.` })] },
          { kind: 'check', ref: 'w3-q07' },
          { kind: 'guided', title: 'Your turn', q: R`A zero-coupon bond pays $5,000 in 8 years. The yield is 4% p.a. What is the bond worth today?`,
            parts: [
              { ask: R`What goes in \(N\)?`, answer: 8, unit: '', dp: 0, hint: 'The number of years to maturity.', why: R`\(N = 8\).` },
              { ask: R`What goes in \(Pmt\)?`, choices: ['0', '40', '5000'], answer: 0, hint: 'How big is each coupon?', why: R`\(Pmt = 0\): a zero-coupon bond pays no coupons.` },
              { ask: R`What goes in \(FV\)?`, answer: 5000, unit: '', dp: 0, hint: 'The amount paid at maturity.', why: R`\(FV = 5000\).` },
              { ask: R`Solve \(PV\). What is the bond worth?`, answer: LS.zeroG, unit: '$', dp: 2, hint: 'Ignore the minus sign.', why: R`\(\frac{5{,}000}{1.04^{8}} = ${L.money(LS.zeroG)}\).` },
            ],
            answer: R`The bond is worth \(${L.money(LS.zeroG)}\).`,
          ...
          { kind: 'recap', title: 'Remember', points: [
            R`A bond pays coupons (fixed interest) plus the face value at maturity. It is a liability for the issuer and an asset for the holder.`,
            R`Coupon rate \(= \frac{\text{annual coupon}}{\text{face value}}\).`,
            R`Price \(=\) PV of the cash flows, discounted at the **yield**.`,
            R`Zero-coupon bond: \(P = \frac{FV}{(1+i)^{n}}\). It always sells below face value.`,
            R`Exam trap: discounting at the coupon rate instead of the yield.`,
          ], formula: 'zero-bond' },
        ],
      },

      'w3-L2': {
        title: 'Pricing a coupon bond',
        goal: R`Price a bond that pays coupons, and tell whether it trades at a premium, at par or at a discount.`,
        topics: ['bondprice'],
        cards: [
          ...
          { kind: 'example', title: 'Worked example', q: R`A $1,000 bond has a 7% coupon rate, paid annually, and 10 years to maturity. The yield is 6% p.a. What is its price?`,
            steps: [
              R`The coupon: \(C = 7\% \times 1{,}000 = \$70\). Also \(i = 0.06\) and \(n = 10\).`,
              R`The coupons: \(\frac{70}{0.06}\left(1 - \frac{1}{1.06^{10}}\right) = ${L.money(LS.b7[0])}\).`,
              R`The face value: \(\frac{1{,}000}{1.06^{10}} = ${L.money(LS.b7[1])}\).`,
              R`The price: \(${L.num(LS.b7[0])} + ${L.num(LS.b7[1])} = ${L.money(LS.b7[0] + LS.b7[1])}\).`,
            ],
            answer: R`The bond is worth \(${L.money(LS.b7[0] + LS.b7[1])}\).`,
            ti: [TI.solver({ N: 10, I: 6, Pmt: 70, FV: 1000, PpY: 1, CpY: 1 }, 'PV', { note: R`The coupon goes in \(Pmt\) and the face value in \(FV\): both are money you receive. The PV is negative: it is the price you pay.` })] },
          { kind: 'check', gen: 'w3-g-annual' },
          ...
          { kind: 'guided', title: 'Your turn', q: R`A $1,000 bond pays a 6% coupon once a year and has 8 years to maturity. The yield is 7.5% p.a. What is the bond worth?`,
            parts: [
              { ask: 'What is the coupon each year?', answer: 60, unit: '$', dp: 2, hint: R`\(6\% \times 1{,}000\)`, why: R`\(C = 0.06 \times 1{,}000 = \$60\).` },
              { ask: 'Before you calculate: how will it trade?', choices: ['At a discount', 'At par', 'At a premium'], answer: 0, hint: 'Compare the 6% coupon rate with the 7.5% yield.', why: 'The coupon rate is below the yield, so the price will be below $1,000.' },
              { ask: R`What goes in \(N\)?`, answer: 8, unit: '', dp: 0, hint: 'The number of coupons left.', why: R`\(N = 8\).` },
              { ask: R`Solve \(PV\). What is the bond worth?`, answer: LS.b2G, unit: '$', dp: 2, hint: R`\(I(\%) = 7.5\), \(Pmt = 60\), \(FV = 1000\). Ignore the minus sign.`, why: R`\(P = ${L.money(LS.b2G)}\): below $1,000, as predicted.` },
            ],
            answer: R`The bond is worth \(${L.money(LS.b2G)}\): a discount bond.`,
            ti: [TI.solver({ N: 8, I: 7.5, Pmt: 60, FV: 1000, PpY: 1, CpY: 1 }, 'PV', { note: 'The minus sign means it is the price you pay.' })] },
          ...
          { kind: 'recap', title: 'Remember', points: [
            ...
            R`TI-Nspire: \(N\) = coupons left, \(I(\%)\) = yield, \(Pmt\) = coupon, \(FV\) = face value. Solve \(PV\).`,
            R`Coupon rate \(>\) yield: **premium**. Equal: **par**. Coupon rate \(<\) yield: **discount**.`,
            R`Exam trap: leaving out the face value, or discounting at the coupon rate.`,
          ], formula: 'bond-price' },
        ],
      },
      ...
      'w3-L3': {
        ...
        goal: R`Price a bond that pays coupons twice a year, and turn its yield into an effective annual yield.`,
        topics: ['semi'],
        cards: [
          ...
          { kind: 'example', title: 'Worked example', q: R`A $1,000 bond has a 9% coupon rate, paid semi-annually, and 7 years to maturity. The yield is 8% p.a. What is its price?`,
            steps: [
              R`Per half-year: \(C = \$45\), \(i = 4\%\) and \(n = 14\).`,
              R`The coupons: \(\frac{45}{0.04}\left(1 - \frac{1}{1.04^{14}}\right) = ${L.money(LS.s9[0])}\).`,
              R`The face value: \(\frac{1{,}000}{1.04^{14}} = ${L.money(LS.s9[1])}\).`,
              R`The price: \(${L.num(LS.s9[0])} + ${L.num(LS.s9[1])} = ${L.money(LS.s9[0] + LS.s9[1])}\). A premium, because the 9% coupon rate is above the 8% yield.`,
            ],
            answer: R`The bond is worth \(${L.money(LS.s9[0] + LS.s9[1])}\).`,
            ti: [TI.solver({ N: 14, I: 8, Pmt: 45, FV: 1000, PpY: 2, CpY: 2 }, 'PV', { note: R`\(N = 14\) half-years, \(Pmt = 45\), \(I(\%) = 8\) (the yearly yield) and \(PpY = CpY = 2\).` })] },
          { kind: 'ti', title: 'Let the TI halve the yield',
            body: R`On the TI-Nspire, keep \(I(\%)\) as the **yearly** yield and set \(PpY = CpY = 2\). The solver then works per half-year for you.\n\nYou can also halve it yourself: \(I(\%) = 4\) with \(PpY = CpY = 1\). Both give the same price.`,
          ...
          { kind: 'guided', title: 'Your turn', q: R`A $1,000 bond pays a 7% coupon semi-annually and has 6 years to maturity. The yield is 8% p.a. What is its price?`,
            parts: [
              { ask: 'What is the coupon every half-year?', answer: 35, unit: '$', dp: 2, hint: R`\(\frac{7\% \times 1{,}000}{2}\)`, why: R`\(\frac{70}{2} = \$35\).` },
              { ask: R`What goes in \(N\)?`, answer: 12, unit: '', dp: 0, hint: 'Count half-years.', why: R`\(N = 6 \times 2 = 12\).` },
              { ask: R`What goes in \(PpY\) and \(CpY\)?`, choices: ['1', '2', '12'], answer: 1, hint: 'How many coupons are paid each year?', why: R`Two coupons a year: \(PpY = CpY = 2\).` },
              { ask: R`Solve \(PV\). What is the price?`, answer: LS.s3G, unit: '$', dp: 2, hint: R`\(I(\%) = 8\), \(Pmt = 35\), \(FV = 1000\). Ignore the minus sign.`, why: R`\(P = ${L.money(LS.s3G)}\).` },
            ],
            answer: R`The bond is worth \(${L.money(LS.s3G)}\).`,
            ti: [TI.solver({ N: 12, I: 8, Pmt: 35, FV: 1000, PpY: 2, CpY: 2 }, 'PV', { note: 'The minus sign means it is the price you pay.' })] },
          { kind: 'recap', title: 'Remember', points: [
            R`Semi-annual: halve the coupon, halve the yield, double the periods.`,
            R`TI-Nspire: \(N = 2 \times\) years, \(Pmt\) = the half-year coupon, \(I(\%)\) = the yearly yield, \(PpY = CpY = 2\).`,
            R`\(EAY = \left(1 + \frac{y}{2}\right)^{2} - 1\), or \(\text{eff}(y, 2)\) on the TI.`,
            R`Exam trap: halving a yield that is already per half-year. Compound it instead: \((1 + i)^{2} - 1\).`,
          ], formula: 'eay' },
        ],
      },
      ...
      'w3-L4': {
        ...
        goal: R`Find a bond’s yield from its price, turn it into an effective annual yield, and work out a realised yield.`,
        topics: ['yield'],
        cards: [
          ...
          { kind: 'example', title: 'Worked example', q: R`A $1,000 bond pays an 8% coupon once a year and has 5 years to maturity. It costs ${T.money(LS.p5)}. What is its YTM?`,
            ...
            answer: R`The YTM is \(${L.pct(LS.y5, 2)}\).`,
            ti: [TI.solver({ N: 5, PV: -LS.p5, Pmt: 80, FV: 1000, PpY: 1, CpY: 1 }, 'I', { note: R`You pay the price, so \(PV\) is negative. The coupons and the face value come to you, so they are positive.` })] },
          { kind: 'ti', title: 'The same thing in one line',
            body: R`Instead of the Finance Solver, you can type one \(\text{tvmI}\) line: \(\text{tvmI}(N, PV, Pmt, FV, PpY, CpY)\).`,
          ...
          { kind: 'example', title: 'Worked example: a semi-annual YTM', q: R`A $1,000 bond has a 7% coupon, paid semi-annually, and 6 years to maturity. Its price is ${T.money(LS.p7)}. What is its effective annual YTM?`,
            steps: [
              ...
              R`Solving gives a nominal yield of \(${L.pct(2 * LS.y7, 2)}\) p.a.: \(${L.pct(LS.y7, 2)}\) per half-year.`,
              R`\[EAY = (1 + ${L.dec(LS.y7, 4)})^{2} - 1 = ${L.pct((1 + LS.y7) ** 2 - 1, 2)}\]`,
            ],
            answer: R`The effective annual YTM is \(${L.pct((1 + LS.y7) ** 2 - 1, 2)}\).`,
          ...
          { kind: 'guided', title: 'Your turn: a realised yield', q: R`Three years ago you paid $1,000 for a bond with an 8% coupon, paid semi-annually. Today, just after a coupon, you sell it for $1,040. What was your realised yield, as an effective annual rate?`,
            parts: [
              ...
              { ask: R`What goes in \(FV\)?`, choices: ['1000', '1040', '0'], answer: 1, hint: 'You sold before maturity.', why: 'The sale price, $1,040, takes the place of the face value.' },
              { ask: R`Solve \(I(\%)\) with \(PV = -1000\), \(Pmt = 40\) and \(PpY = CpY = 2\). What is the nominal yield?`, answer: P(2 * LS.yR), unit: '%', dp: 2, hint: 'Solve I(%) in the Finance Solver.', why: R`\(I(\%) = ${L.num(P(2 * LS.yR), 4)}\).` },
              { ask: R`Now type \(\text{eff}(\text{ans}, 2)\). What is the realised yield?`, answer: P((1 + LS.yR) ** 2 - 1), unit: '%', dp: 2, hint: 'Two half-years in a year.', why: R`\(${L.pct((1 + LS.yR) ** 2 - 1, 2)}\) a year.` },
            ],
            answer: R`Your realised yield was \(${L.pct((1 + LS.yR) ** 2 - 1, 2)}\) a year.`,
            ti: [TI.solver({ N: 6, PV: -1000, Pmt: 40, FV: 1040, PpY: 2, CpY: 2 }, 'I', { note: 'The nominal yearly yield.' }), TI.line('eff(ans,2)', { note: 'The realised yield, as an effective annual rate.' })] },
          { kind: 'check', ref: 'w3-q24' },
          { kind: 'recap', title: 'Remember', points: [
            R`YTM: the rate that makes the PV of the coupons and face value equal the price.`,
            R`TI-Nspire: \(PV = -\)price, \(Pmt\) = coupon, \(FV\) = face value. Solve \(I(\%)\).`,
            R`Semi-annual: \(PpY = CpY = 2\) gives the nominal yield; \(\text{eff}(\text{ans}, 2)\) gives the EAY.`,
            R`Realised yield: the sale price goes in \(FV\), and \(N\) counts the periods you held the bond.`,
            R`Exam trap: giving the nominal yield when the question asks for the effective annual yield.`,
          ], formula: 'eay' },
        ],
      },
      ...
      'w3-L5': {
        ...
        goal: R`Explain why bond prices move when interest rates move, and which bonds move the most.`,
        topics: ['raterisk'],
        cards: [
          ...
          { kind: 'example', title: 'Worked example: after rates change', q: R`A 10-year, $1,000 bond pays a 6% coupon once a year. It was issued at par. Two years later, market yields have fallen to 5%. What is the bond worth now?`,
            ...
            ti: [TI.solver({ N: 8, I: 5, Pmt: 60, FV: 1000, PpY: 1, CpY: 1 }, 'PV', { note: 'Use the years left and the new yield. The minus sign means it is the price you pay.' })] },
          { kind: 'guided', title: 'Your turn', q: R`A 20-year, $1,000 bond pays an 8% coupon semi-annually. It was issued at par. Five years later, market yields have risen to 10% p.a. What is it worth then?`,
            parts: [
              { ask: 'How many half-years are left?', choices: ['30', '40', '15'], answer: 0, hint: '15 years are left.', why: R`\(15 \times 2 = 30\) half-years.` },
              { ask: 'Before you calculate: will it trade at a premium or a discount?', choices: ['A discount', 'Par', 'A premium'], answer: 0, hint: 'Rates rose above the 8% coupon rate.', why: 'The yield (10%) is now above the coupon rate (8%), so the price is below $1,000.' },
              { ask: R`Solve \(PV\). What is the bond worth?`, answer: LS.laterG, unit: '$', dp: 2, hint: R`\(N = 30\), \(I(\%) = 10\), \(Pmt = 40\), \(FV = 1000\), \(PpY = CpY = 2\).`, why: R`\(P = ${L.money(LS.laterG)}\).` },
            ],
            answer: R`The bond is worth \(${L.money(LS.laterG)}\): a discount, because rates rose.`,
            ti: [TI.solver({ N: 30, I: 10, Pmt: 40, FV: 1000, PpY: 2, CpY: 2 }, 'PV', { note: 'The minus sign means it is the price you pay.' })] },
          { kind: 'check', gen: 'w3-g-later' },
          { kind: 'check', gen: 'w3-g-sens' },
          { kind: 'recap', title: 'Remember', points: [
            R`Bond prices and interest rates move in **opposite** directions.`,
            R`**Longer maturity** \(\Rightarrow\) more interest-rate risk.`,
            R`**Lower coupon** \(\Rightarrow\) more interest-rate risk.`,
            R`A future price uses the years left **then** and the yield **then**.`,
            R`Exam trap: rates fall and you say prices fall. Rates down means prices up.`,
          ] },
        ],
      },

      'w3-L6': {
        title: 'Shares with steady dividends',
        goal: R`Value a share whose dividend stays the same, or grows at a constant rate, forever.`,
        topics: ['pref', 'ddm'],
        cards: [
          ...
          { kind: 'example', title: 'Worked example', q: R`A preference share pays a fixed dividend of $2.50 a year, forever. The next dividend is in one year. The required return is 10%. What is the share worth?`,
            steps: [
              R`A fixed dividend forever is a perpetuity.`,
              R`\[P_0 = \frac{2.50}{0.10} = \$25.00\]`,
              R`In 5 years it still pays $2.50 forever, so it will still be worth $25.00.`,
            ],
            answer: R`The share is worth \(\$25.00\).`,
            ti: [TI.line('2.50/0.10')] },
          ...
          { kind: 'guided', title: 'Your turn', q: R`A company has **just paid** a dividend of $1.50. Dividends grow at 4% a year, forever. The required return is 11%. What is a share worth?`,
            parts: [
              { ask: 'Which dividend goes on top of the formula?', choices: [R`\(D_1 = 1.50 \times 1.04\)`, R`\(D_0 = 1.50\)`, R`\(1.50 \times 0.04\)`], answer: 0, hint: 'The formula needs the next dividend.', why: R`\(D_0\) is already paid. Use \(D_1 = D_0(1+g)\).` },
              { ask: R`What is \(D_1\)?`, answer: 1.5 * 1.04, unit: '$', dp: 2, hint: R`\(1.50 \times 1.04\)`, why: R`\(D_1 = ${L.money(1.5 * 1.04)}\).` },
              { ask: R`What is \(r_E - g\), as a decimal?`, answer: 0.07, unit: '', dp: 3, hint: R`\(0.11 - 0.04\)`, why: R`\(0.11 - 0.04 = 0.07\).` },
              { ask: 'What is a share worth?', answer: 1.5 * 1.04 / 0.07, unit: '$', dp: 2, hint: R`\(\frac{1.56}{0.07}\)`, why: R`\(\frac{1.56}{0.07} = ${L.money(1.5 * 1.04 / 0.07)}\).` },
            ],
            answer: R`A share is worth \(${L.money(1.5 * 1.04 / 0.07)}\).`,
            ti: [TI.line('1.50*1.04/(0.11-0.04)')] },
          { kind: 'check', gen: 'w3-g-ddm' },
          { kind: 'check', ref: 'w3-q46' },
          { kind: 'recap', title: 'Remember', points: [
            R`Share price \(=\) PV of all future dividends, at the required return \(r_E\).`,
            R`Zero growth (preference shares): \(P_0 = \frac{D}{r_E}\). The price stays the same.`,
            R`Constant growth: \(P_0 = \frac{D_1}{r_E - g}\), with \(r_E > g\).`,
            R`A higher required return (more risk) means a lower price.`,
            R`Exam trap: putting \(D_0\) on top. Use \(D_1 = D_0(1+g)\).`,
          ], formula: 'share-ddm' },
        ],
      },

      'w3-L7': {
        title: 'Where a share’s return comes from',
        goal: R`Split a share’s return into a dividend yield and a capital gains yield.`,
        topics: ['returns'],
        cards: [
          ...
          { kind: 'example', title: 'Worked example', q: R`You buy a share for $40. You receive a $2 dividend in one year, and then sell the share for $42. What are the three yields?`,
            steps: [
              R`Dividend yield: \(\frac{2}{40} = 5\%\).`,
              R`Capital gains yield: \(\frac{42 - 40}{40} = 5\%\).`,
              R`Total return: \(5\% + 5\% = 10\%\). Check: \(\frac{2 + 42 - 40}{40} = 10\%\).`,
            ],
            answer: R`A total return of 10%: half from the dividend, half from the price rise.`,
            ti: [TI.line('(2+42-40)/40', { pct: true, note: 'The total return as a decimal. Multiply by 100 for %.' })] },
          ...
          { kind: 'example', title: 'Worked example: the required return', q: R`A share trades at $25. It will pay a $1.00 dividend next year, and dividends grow at 6% a year forever. What return do investors require?`,
            steps: [
              R`Dividend yield: \(\frac{1.00}{25} = 4\%\).`,
              R`Capital gains yield: \(g = 6\%\).`,
              R`\(r_E = 4\% + 6\% = 10\%\).`,
            ],
            answer: R`Investors require 10%.`,
            ti: [TI.line('1.00/25+0.06', { pct: true, note: 'Multiply by 100 for %.' })] },
          ...
          { kind: 'guided', title: 'Your turn', q: R`You would pay $50 for a share today. You expect to sell it for $53 in one year. You require a 10% return. What dividend must you expect at the end of the year?`,
            parts: [
              { ask: R`What must the share give you in total at year 1 (dividend plus sale price)?`, answer: 55, unit: '$', dp: 2, hint: R`A 10% return on $50: \(50 \times 1.10\).`, why: R`\(50 \times 1.10 = \$55\).` },
              { ask: 'What is the capital gain in dollars?', answer: 3, unit: '$', dp: 2, hint: '$53 − $50', why: R`\(53 - 50 = \$3\).` },
              { ask: 'What dividend must you expect?', answer: 2, unit: '$', dp: 2, hint: 'The total you need, minus the sale price.', why: R`\(55 - 53 = \$2\).` },
            ],
            answer: R`You need a $2.00 dividend: a 4% dividend yield plus a 6% capital gains yield make 10%.`,
            ti: [TI.line('50*1.10-53')] },
          { kind: 'recap', title: 'Remember', points: [
            R`Dividend yield \(= \frac{D_1}{P_0}\). Capital gains yield \(= \frac{P_1 - P_0}{P_0}\).`,
            R`Total return \(=\) dividend yield \(+\) capital gains yield. It can be negative.`,
            R`Constant growth: the price grows at \(g\), so \(r_E = \frac{D_1}{P_0} + g\) and \(P_n = P_0(1+g)^{n}\).`,
            R`Exam trap: dividing by the sale price. Always divide by the price you pay, \(P_0\).`,
          ], formula: 'total-return' },
        ],
      },

      'w3-L8': {
        title: 'Variable growth: the 4-step method',
        goal: R`Value a share whose dividends grow fast at first, then settle down to a constant rate.`,
        topics: ['vargrowth'],
        cards: [
          ...
          { kind: 'learn', title: 'The 4-step method',
            ...
            points: [
              R`**Step 1:** work out each dividend in the fast-growth years: \(D_1, D_2, \ldots, D_n\).`,
              R`**Step 2:** discount each one to today.`,
          ...
          { kind: 'example', title: 'Worked example', q: R`A company has **just paid** a dividend of $1.00. Dividends will grow at 15% a year for 2 years, then at 4% a year forever. The required return is 12%. What is a share worth?`,
            steps: [
              R`Step 1: \(D_1 = 1.00 \times 1.15 = ${n4(LS.var1.divs[0])}\) and \(D_2 = 1.15 \times 1.15 = ${n4(LS.var1.divs[1])}\).`,
              R`Step 2: \(\frac{${n4(LS.var1.divs[0])}}{1.12} = ${n4(LS.var1.pvDivs[0])}\) and \(\frac{${n4(LS.var1.divs[1])}}{1.12^{2}} = ${n4(LS.var1.pvDivs[1])}\).`,
              R`Step 3: \(D_3 = ${n4(LS.var1.divs[1])} \times 1.04 = ${n4(LS.var1.dNext)}\), so \(P_2 = \frac{${n4(LS.var1.dNext)}}{0.12 - 0.04} = ${n4(LS.var1.PT)}\).`,
              R`Step 4: \(\frac{${n4(LS.var1.PT)}}{1.12^{2}} = ${n4(LS.var1.pvPT)}\). Add: \(${n4(LS.var1.pvDivs[0])} + ${n4(LS.var1.pvDivs[1])} + ${n4(LS.var1.pvPT)} = ${L.money(LS.var1.price)}\).`,
            ],
            answer: R`A share is worth \(${L.money(LS.var1.price)}\).`,
          ...
          { kind: 'guided', title: 'Your turn', q: R`A company has **just paid** a dividend of $0.50. Dividends will grow at 20% a year for 3 years, then at 5% a year forever. The required return is 11%. What is a share worth?`,
            parts: [
              { ask: R`Step 1: what is \(D_1\)?`, answer: LS.varG.divs[0], unit: '$', dp: 2, hint: R`\(0.50 \times 1.20\)`, why: R`\(D_1 = ${L.money(LS.varG.divs[0])}\).` },
              { ask: R`What is \(D_3\)? (3 decimal places)`, answer: LS.varG.divs[2], unit: '$', dp: 3, hint: R`\(0.50 \times 1.20^{3}\)`, why: R`\(D_2 = ${n4(LS.varG.divs[1])}\) and \(D_3 = ${n4(LS.varG.divs[2])}\).` },
              { ask: R`Step 3: what is \(P_3 = \frac{D_4}{r_E - g}\)?`, answer: LS.varG.PT, unit: '$', dp: 2, hint: R`\(D_4 = D_3 \times 1.05\). Then divide by \(0.11 - 0.05\).`, why: R`\(P_3 = \frac{${n4(LS.varG.dNext)}}{0.06} = ${L.money(LS.varG.PT)}\).` },
              { ask: 'Steps 2 and 4: what is a share worth today?', answer: LS.varG.price, unit: '$', dp: 2, hint: R`\(\text{npv}(11, 0, \{D_1, D_2, D_3 + P_3\})\)`, why: R`\(P_0 = ${L.money(LS.varG.price)}\).` },
            ],
            answer: R`A share is worth \(${L.money(LS.varG.price)}\).`,
            ti: [TI.line(`${tn(LS.varG.divs[2])}*1.05/(0.11-0.05)`, { note: R`\(P_3\).` }), TI.line(`npv(11,0,{${LS.varG.divs.map((d, k) => tn(d) + (k === 2 ? '+ans' : '')).join(',')}})`, { note: R`Year 3 holds \(D_3 + P_3\).` })] },
          ...
          { kind: 'recap', title: 'Remember', points: [
            ...
            R`\(P_n\) sits at \(t = n\): discount it \(n\) years, not \(n + 1\).`,
            R`TI-Nspire: \(\text{npv}(r_E, 0, \{D_1, \ldots, D_n + P_n\})\).`,
            R`A dividend due tomorrow: add it. Just paid: leave it out.`,
            R`Exam trap: using \(D_n\) instead of \(D_{n+1}\) in \(P_n\).`,
          ], formula: 'share-general' },
        ],
      },
    },
    ...
    questions: [
      ...
      { id: 'w3-q12', topic: 'bondprice', kind: 'num', level: 1, section: 'B', src: 'Lecture W3 Examples 2–4', formula: 'bond-price',
        ...
        steps: [
          ...
          R`\[P = \frac{50}{0.06}\left(1 - \frac{1}{1.06^{10}}\right) + \frac{1{,}000}{1.06^{10}} = ${L.num(EX3.c, dpSum([EX3.c, EX3.f]))} + ${L.num(EX3.f, dpSum([EX3.c, EX3.f]))} = ${L.money(EX3.p)}\]`,
      ...
      { id: 'w3-q13', topic: 'bondprice', kind: 'num', level: 1, section: 'B', src: 'Tutorial W3 Q1', formula: 'bond-price',
        ...
        steps: [
          ...
          R`\[P = \frac{80}{0.07}\left(1 - \frac{1}{1.07^{12}}\right) + \frac{1{,}000}{1.07^{12}} = ${[FIN.pvAnnuity(80, 0.07, 12), 1000 / 1.07 ** 12].map((x, k, a) => L.num(x, dpSum(a))).join(' + ')} = ${L.money(FIN.bondPrice(1000, 0.08, 0.07, 12))}\]`,
        ],
      ...
      { id: 'w3-q62', topic: 'vargrowth', kind: 'num', level: 2, section: 'B', src: 'Lecture W3 Example 8', formula: 'share-general',
        ...
        steps: [
          ...
          R`Step 2, their PVs: \[\frac{0.18}{1.10} + \frac{0.216}{1.10^{2}} + \frac{0.2592}{1.10^{3}} = ${EX8.pvDivs.map((x) => L.num(x, dpSum(EX8.pvDivs, 4))).join(' + ')} = ${n4(EX8.sum)}\]`,
        ...
        make(rng) {
          ...
          return {
            ...
            steps: [
              ...
              bondFormula,
              R`\[P = ${bondTex(cp, y, n, face)} = ${L.num(pvC, dpSum([pvC, pvF]))} + ${L.num(pvF, dpSum([pvC, pvF]))} = ${L.money(price)}\]`,
        ...
        make(rng) {
          ...
          return {
            ...
            steps: [
              ...
              R`\[P = \frac{1}{(${L.onePlus(i)})^{${s}}} \times \frac{${ml(cp)}}{${L.dec(i)}}\left(1 - \frac{1}{(${L.onePlus(i)})^{${n - s - 1}}}\right) + \frac{${ml(cfs[n])}}{(${L.onePlus(i)})^{${n}}} = ${L.num(partC, dpSum([partC, partF]))} + ${L.num(partF, dpSum([partC, partF]))} = ${L.money(price)}\]`,
            ],
        ...
        make(rng) {
          ...
          return {
            ...
            steps: [
              ...
              R`\[P = ${bondTex(cp, i, n, face)} = ${L.num(pvC, dpSum([pvC, pvF]))} + ${L.num(pvF, dpSum([pvC, pvF]))} = ${L.money(price)}\]`,
            ],
        ...
        make(rng) {
          ...
          return {
            ...
            steps: [
              ...
              R`\[\text{PV} = ${res.divs.map((x, t) => R`\frac{${n4(x)}}{(${L.onePlus(r)})^{${t + 1}}}`).join(' + ')} + \frac{${n4(res.PT)}}{(${L.onePlus(r)})^{${T0}}} = ${L.num(sumPV, dpSum([sumPV, res.pvPT], 4))} + ${L.num(res.pvPT, dpSum([sumPV, res.pvPT], 4))} = ${n4(res.price)}\]`,
