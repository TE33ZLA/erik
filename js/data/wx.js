...
(function (root) {
  ...
  // L4 worked example: market values and costs; its balance sheet (book) values
  const WX = { E: 600, P: 100, D: 300, re: 0.12, rp: 0.08, rd: 0.06, tc: 0.3, book: { E: 250, P: 100, D: 320 } };
  const share = (x, tot) => R`\(${pc(x / tot, 0)}\)`;
  // L5 worked example: equity $600m at 14%, debt $400m at 6% (no taxes)
  const MX5 = { E: 600, D: 400, rE: 0.14, rD: 0.06 };
  MX5.rU = FIN.rUnlevered(MX5.rE, MX5.rD, MX5.E, MX5.D);
  // L6 small example: $100 of profit before interest and tax, $20 of interest, 30% tax
  const SM = { ebit: 100, int: 20, tc: 0.3 };
  SM.taxU = SM.ebit * SM.tc; SM.taxL = (SM.ebit - SM.int) * SM.tc; SM.eqU = SM.ebit - SM.taxU; SM.eqL = SM.ebit - SM.int - SM.taxL;
  const m$ = (x) => R`\(\$${nt(x, 2)}\)`; // LaTeX dollars for pie legends
  // L7: r_U = 10%, r_D = 6%, T_c = 30% (the worked examples), for D/E from 0 to 1.5
  const TW = { rU: 0.10, rD: 0.06, tc: 0.3 };
  const deSteps = Array.from({ length: 16 }, (_, k) => k / 10);
  const reLine = (tc) => deSteps.map((x) => [x, P(FIN.rELevTax(TW.rU, TW.rD, x, 1, tc))]);
  const waccLine = (tc) => deSteps.map((x) => [x, P(FIN.wacc({ E: 1, D: x, re: FIN.rELevTax(TW.rU, TW.rD, x, 1, tc), rd: TW.rD, tc }))]);
  const flat = (y) => [[0, P(y)], [1.5, P(y)]];
  const mmChart = (title, tc, cap, key) => ({ type: 'lines', title, cap, key, keys: [], x: { label: 'Debt-to-equity ratio D/E', min: 0, max: 1.5 }, y: { label: 'Cost of capital', fmt: '%', min: 4, max: 17 },
    series: [{ name: 'r_E', c: 1, pts: reLine(tc) }, { name: 'WACC', c: 3, pts: waccLine(tc) }, { name: 'r_D', c: 2, pts: flat(TW.rD) }] });
      ...
      'wx-L1': {
        ...
        cards: [
          { kind: 'learn', title: 'A preview floor',
            ...
            viz: { type: 'flow', cap: R`Built only from the formula sheet. If the lectures say something different, follow your lecturer.`,
              steps: [{ icon: '📄', t: 'Formula sheet', s: 'the only source' }, { icon: '⛰️', t: 'This floor', s: 'a preview built from it', c: 3 }, { icon: '🎓', t: 'The lectures', s: 'check this floor against them' }] } },
          { kind: 'learn', title: 'Where a firm’s money comes from',
            ...
            viz: { type: 'pie', center: 'Capital', cap: R`The firm’s **capital** comes from three kinds of investors: lenders, preference shareholders and owners.`,
              parts: [{ label: '**Debt**: lenders, paid first', v: WX.D, c: SRC_C.D, show: '' }, { label: '**Preference shares**: a fixed dividend', v: WX.P, c: SRC_C.P, show: '' }, { label: '**Equity**: the owners, paid last', v: WX.E, c: SRC_C.E, show: '' }] } },
          { kind: 'learn', title: 'The cost of capital',
            ...
            viz: { type: 'flow', cap: R`What investors require is what the money **costs** the firm.`,
              steps: [{ icon: '👥', t: 'Investors', s: 'could earn it elsewhere' }, { icon: '🏢', t: 'The firm', s: 'must offer at least that', c: 2 }, { icon: '🏷️', t: 'Cost of capital', s: 'the required return', c: 3 }] } },
          { kind: 'learn', title: 'Riskier claims cost more',
            ...
            viz: { type: 'bars', key: true, values: false, cap: R`Paid later means more risk, so investors require more. Debt is the cheapest; equity costs the most.`,
              bars: [{ label: 'Debt\npaid 1st', v: P(WX.rd), c: SRC_C.D }, { label: 'Preference\npaid 2nd', v: P(WX.rp), c: SRC_C.P }, { label: 'Equity\npaid last', v: P(WX.re), c: SRC_C.E }] } },
          ...
          { kind: 'learn', title: 'The plan for this floor',
            ...
            viz: { type: 'flow', cap: 'Three steps: each cost, then the blend, then the big question about debt.',
              steps: [{ icon: '🔍', t: R`\(r_E\), \(r_p\), \(r_d\)`, s: 'the cost of each source' }, { icon: '🥣', t: 'WACC', s: 'one blended rate', c: 3 }, { icon: '❓', t: 'Debt or equity?', s: 'does the mix matter?', c: 2 }] } },
      ...
      'wx-L2': {
        ...
        cards: [
          { kind: 'learn', title: 'Two ways to estimate it',
            ...
            viz: { type: 'compare', key: true, vsText: 'or', cap: 'Two ways to estimate the same thing: the return shareholders require.',
              items: [{ icon: '📈', title: 'CAPM (Week 9)', big: R`\(r_E = r_f + \beta_E(E[R_M] - r_f)\)`, points: ['Uses beta: the systematic risk shareholders bear'], c: 1 },
                { icon: '🌱', title: 'Dividend growth (Week 3)', big: R`\(r_E = \frac{D_1}{P_0} + g\)`, points: ['Uses the next dividend, the share price and the growth rate'], c: 2 }] } },
          { kind: 'learn', title: 'Method 1: CAPM',
            ...
            formula: 'capm',
            viz: { type: 'split', fmt: '%', total: R`\(r_E = ${pc(FIN.capm(0.04, 1.2, 0.10), 1)}\) when \(\beta_E = 1.2\), \(r_f = 4\%\) and \(E[R_M] = 10\%\)`,
              cap: R`The blue part pays for waiting. The orange part pays for the systematic risk: \(1.2 \times 6\%\).`,
              parts: [{ label: R`Risk-free rate \(r_f\)`, v: 4, c: 1 }, { label: R`Reward for risk: \(\beta_E(E[R_M] - r_f)\)`, v: P(1.2 * (0.10 - 0.04)), c: 2 }] } },
          ...
          { kind: 'learn', title: 'Method 2: the dividend growth model',
            ...
            viz: { type: 'split', fmt: '%', total: R`\(r_E = ${pc((1.5 * 1.04) / 26 + 0.04, 0)}\) for a \(\$26\) share with \(D_1 = \$1.56\) and \(g = 4\%\)`,
              cap: R`The cost of equity is the **dividend yield** plus the **growth rate**: \(\frac{1.56}{26} = 6\%\), plus \(4\%\).`,
              parts: [{ label: R`Dividend yield \(\frac{D_1}{P_0}\)`, v: P((1.5 * 1.04) / 26), c: 4 }, { label: R`Growth rate \(g\)`, v: 4, c: 2 }] } },
      ...
      'wx-L3': {
        ...
        cards: [
          { kind: 'learn', title: 'The cost of debt is the yield',
            ...
            viz: { type: 'compare', cap: R`This bond pays a \(6\%\) coupon but trades at \(\$960\). Its cost is the YTM, not the coupon rate.`,
              items: [{ icon: '🎟️', title: 'Coupon rate', big: R`\(6\%\)`, points: ['Fixed when the bond was first sold'], mark: 'bad', markText: 'Not the cost', c: 'grey' },
                { icon: '📊', title: 'Yield to maturity', big: R`\(${pc(YTM_EX)}\)`, points: ['What lenders require today, at today’s price'], mark: 'good', markText: R`The cost of debt, \(r_d\)`, c: SRC_C.D }] } },
          ...
          { kind: 'learn', title: 'Interest saves tax',
            ...
            viz: { type: 'split', key: true, fmt: '%', total: R`Interest at \(r_d = 7\%\)`, cap: R`With a \(30\%\) tax rate, the tax saving covers \(2.1\%\). The firm really pays \(4.9\%\).`,
              parts: [{ label: R`After-tax cost \(r_d(1 - T_c)\): what the firm really pays`, v: P(0.07 * (1 - 0.3)), c: 2 }, { label: R`Tax saved: \(7\% \times 0.30\)`, v: P(0.07 * 0.3), c: 'good' }] } },
          ...
          { kind: 'learn', title: 'The cost of preference shares',
            ...
            formula: 'cost-pref',
            viz: { type: 'flow', op: true, cap: R`A perpetuity of \(\$4.20\) a year. Divide by today’s **market price** (\(\$56\)), not the par value (\(\$60\)).`,
              steps: [{ t: R`\(\$4.20\)`, s: R`dividend each year (\(7\%\) of par)`, c: 4 }, { t: R`\(\$56\)`, s: 'market price today', c: 1 }, { t: R`\(${pc(FIN.costPref(4.2, 56), 1)}\)`, s: R`the cost, \(R_p\)`, c: 3 }], links: [R`\(\div\)`, R`\(=\)`] } },
      ...
      'wx-L4': {
        ...
        cards: [
          { kind: 'learn', title: 'One rate for the whole firm',
            ...
            formula: 'wacc',
            viz: { type: 'anatomy', key: true, cap: 'Each cost times its weight, then add. Only the cost of debt is cut for tax.',
              tex: R`\begin{aligned} r_{WACC} &= \colA{r_e}\colE{\frac{E}{V}} + \colD{r_p}\colE{\frac{P}{V}} \\ &\quad + \colB{r_d(1 - T_c)}\colE{\frac{D}{V}} \end{aligned}`,
              parts: [{ sym: 'r_e', say: 'the cost of equity', c: 'A' }, { sym: 'r_p', say: 'the cost of preference shares', c: 'D' }, { sym: R`r_d(1 - T_c)`, say: 'the cost of debt, after tax', c: 'B' },
                { sym: R`\frac{E}{V}, \frac{P}{V}, \frac{D}{V}`, say: R`the weights: each source’s share of the total value \(V\)`, c: 'E' }] } },
          { kind: 'learn', title: 'Use market values',
            ...
            viz: { type: 'pie', cap: 'Same firm, very different weights. Use the market values: what investors would pay today.',
              pies: [['Book values: do not use', WX.book], ['Market values: use these', WX]].map(([title, v]) => {
                const tot = v.E + v.P + v.D;
                return { title, center: `${T.moneyT(tot)}m`, parts: [{ label: 'Equity', v: v.E, c: SRC_C.E, show: share(v.E, tot) }, { label: 'Preference', v: v.P, c: SRC_C.P, show: share(v.P, tot) }, { label: 'Debt', v: v.D, c: SRC_C.D, show: share(v.D, tot) }] };
              }) } },
          ...
          { kind: 'learn', title: 'When to use the WACC',
            ...
            viz: { type: 'lines', keys: [], cap: R`Above average risk, the WACC is too low. The red project beats the WACC but not the rate it needs.`,
              x: { label: 'Project risk', ticks: [] }, y: { label: 'Required return', ticks: [], min: 0, max: 20 },
      ...
      'wx-L5': {
        ...
        cards: [
          { kind: 'learn', title: 'Capital structure',
            ...
            viz: { type: 'pie', arrow: '?', cap: 'The same firm with two different mixes. Is the levered firm worth more, less or the same?',
              pies: [{ title: 'Unlevered: no debt', parts: [{ label: 'Equity', v: 100, c: SRC_C.E, show: R`\(100\%\)` }] },
                { title: 'Levered: some debt', parts: [{ label: 'Equity', v: MX5.E, c: SRC_C.E, show: share(MX5.E, MX5.E + MX5.D) }, { label: 'Debt', v: MX5.D, c: SRC_C.D, show: share(MX5.D, MX5.E + MX5.D) }] }] } },
          { kind: 'learn', title: 'A perfect market',
            ...
            viz: { type: 'cards', cap: R`MM’s **perfect market** has these three features. Real markets have taxes: the next lessons add them back.`,
              items: [{ icon: '🚫', t: 'No taxes', s: 'The government takes no share of the profit', c: 2 }, { icon: '🆓', t: 'No costs', s: 'Trading and issuing shares or bonds is free', c: 3 }, { icon: '🔒', t: 'Same cash flows', s: 'Financing does not change what the assets earn', c: 1 }] } },
          { kind: 'learn', title: 'Proposition I: the pie stays the same size',
            ...
            formula: 'mm-nt-value',
            viz: { type: 'pie', key: true, arrow: '=', cap: 'Same assets, same cash flows: slicing the pie differently does not change its size.',
              pies: [{ title: R`Unlevered: \(U\)`, center: `${T.moneyT(MX5.E + MX5.D)}m`, parts: [{ label: 'Equity', v: MX5.E + MX5.D, c: SRC_C.E, show: m$(MX5.E + MX5.D) + 'm' }] },
                { title: R`Levered: \(E + D\)`, center: `${T.moneyT(MX5.E + MX5.D)}m`, parts: [{ label: 'Equity', v: MX5.E, c: SRC_C.E, show: m$(MX5.E) + 'm' }, { label: 'Debt', v: MX5.D, c: SRC_C.D, show: m$(MX5.D) + 'm' }] }] } },
          ...
          { kind: 'learn', title: 'The unlevered cost of capital',
            ...
            formula: 'mm-nt-ru',
            viz: { type: 'split', fmt: '%', dp: 1, total: R`\(r_U = ${pc(MX5.rU, 1)}\): equity \(\$600\text{m}\) at \(14\%\), debt \(\$400\text{m}\) at \(6\%\)`,
              cap: 'The return on the assets is shared: each cost times its share of the firm.',
              parts: [{ label: R`Owners: \(0.6 \times 14\%\)`, v: P(0.6 * MX5.rE), c: SRC_C.E }, { label: R`Lenders: \(0.4 \times 6\%\)`, v: P(0.4 * MX5.rD), c: SRC_C.D }] } },
          ...
          { kind: 'learn', title: 'Proposition II: equity gets riskier',
            ...
            viz: { type: 'lines', w: 360, keys: [], cap: R`As debt rises, \(r_E\) climbs, but the WACC stays at \(r_U = ${pc(MX5.rU, 1)}\). Dot: the example firm, \(r_E = ${pc(FIN.rELevNoTax(MX5.rU, MX5.rD, MX5.D, MX5.E), 0)}\).`,
              x: { label: 'Debt-to-equity ratio D/E', min: 0, max: 1.5 }, y: { label: 'Cost of capital', fmt: '%', min: 0, max: 20 },
              series: [{ name: 'r_E', c: 1, pts: deSteps.map((x) => [x, P(FIN.rELevNoTax(MX5.rU, MX5.rD, x, 1))]) }, { name: 'WACC', c: 3, pts: flat(MX5.rU) }, { name: 'r_D', c: 2, pts: flat(MX5.rD) }],
              marks: [{ x: MX5.D / MX5.E, y: P(FIN.rELevNoTax(MX5.rU, MX5.rD, MX5.D, MX5.E)), c: 1 }] } },
      ...
      'wx-L6': {
        ...
        cards: [
          { kind: 'learn', title: 'Taxes change the story',
            ...
            viz: { type: 'seesaw', down: 'right', cap: 'Interest is tax deductible: more debt means less tax, so more cash is left for investors.',
              left: { icon: '🏦', label: 'Debt' }, right: { icon: '🏛️', label: 'Tax paid' } } },
          { kind: 'learn', title: 'A small example',
            ...
            viz: { type: 'pie', cap: R`The same \(\$100\). With debt, the tax slice shrinks from \(\$30\) to \(\$24\), so investors get \(\$6\) more.`,
              pies: [{ title: 'Firm U: no debt', center: T.moneyT(SM.ebit), parts: [{ label: 'Shareholders', v: SM.eqU, c: SRC_C.E, show: m$(SM.eqU) }, { label: 'Tax', v: SM.taxU, c: 'bad', show: m$(SM.taxU) }] },
                { title: 'Firm L: some debt', center: T.moneyT(SM.ebit), parts: [{ label: 'Shareholders', v: SM.eqL, c: SRC_C.E, show: m$(SM.eqL) }, { label: 'Lenders', v: SM.int, c: SRC_C.D, show: m$(SM.int) }, { label: 'Tax', v: SM.taxL, c: 'bad', show: m$(SM.taxL) }] }] } },
          { kind: 'learn', title: 'The interest tax shield',
            ...
            formula: 'its',
            viz: { type: 'flow', op: true, cap: R`Firm L’s \(\$20\) of interest saves \(\$6\) of tax every year: its interest tax shield.`,
              steps: [{ t: m$(SM.int), s: 'interest', c: SRC_C.D }, { t: R`\(${pc(SM.tc, 0)}\)`, s: R`tax rate \(T_c\)`, c: 'grey' }, { t: m$(FIN.interestTaxShield(SM.int, SM.tc)), s: 'tax saved', c: 'good' }], links: [R`\(\times\)`, R`\(=\)`] } },
          ...
          { kind: 'learn', title: 'MM with taxes: debt adds value',
            ...
            formula: 'mm-t-value',
            viz: { type: 'bars', key: true, cap: R`Borrowing \(\$80\text{m}\) forever adds only the shield, \(0.30 \times \$80\text{m} = \$24\text{m}\), not the whole loan.`,
              bars: [{ label: 'No debt', note: `${T.moneyT(200)}m`, parts: [{ v: 200, c: 1 }] }, { label: R`$80m of debt`, note: `${T.moneyT(200 + FIN.pvTaxShieldPerm(80, 0.3))}m`, parts: [{ v: 200, c: 1 }, { v: FIN.pvTaxShieldPerm(80, 0.3), c: 'good' }] }],
              keys: [{ c: 1, label: R`\(V_U\): the firm with no debt` }, { c: 'good', label: R`\(T_c D\): the value of the tax shield` }] } },
      ...
      'wx-L7': {
        ...
        cards: [
          { kind: 'learn', title: 'Proposition II with taxes',
            ...
            formula: 'mm-t-re',
            viz: { type: 'lines', w: 360, keys: [], cap: R`At \(D/E = 0.5\): \(r_E = ${pc(FIN.rELevTax(TW.rU, TW.rD, 0.5, 1, TW.tc), 1)}\) with taxes, \(${pc(FIN.rELevNoTax(TW.rU, TW.rD, 0.5, 1), 0)}\) without. The \((1 - T_c)\) flattens the line.`,
              title: R`Cost of equity \(r_E\) when \(r_U = 10\%\), \(r_D = 6\%\), \(T_c = 30\%\)`,
              x: { label: 'Debt-to-equity ratio D/E', min: 0, max: 1.5 }, y: { label: 'Cost of equity', fmt: '%', min: 8, max: 17 },
              series: [{ name: 'No taxes', c: 'grey', dash: true, pts: reLine(0) }, { name: 'With taxes', c: 1, pts: reLine(TW.tc) }],
              marks: [{ x: 0.5, y: P(FIN.rELevNoTax(TW.rU, TW.rD, 0.5, 1)), label: tp(FIN.rELevNoTax(TW.rU, TW.rD, 0.5, 1), 1), c: 'grey', pos: 'left' },
                { x: 0.5, y: P(FIN.rELevTax(TW.rU, TW.rD, 0.5, 1, TW.tc)), label: tp(FIN.rELevTax(TW.rU, TW.rD, 0.5, 1, TW.tc), 1), c: 1, pos: 'right' }] } },
          ...
          { kind: 'learn', title: 'The WACC with taxes',
            ...
            formula: 'mm-t-wacc',
            viz: { type: 'lines', w: 360, keys: [], cap: R`With taxes, the WACC **falls** as debt rises: \(${pc(FIN.wacc({ E: 400, D: 200, re: 0.114, rd: 0.06, tc: 0.3 }), 0)}\) at \(D/E = 0.5\), below \(r_U = 10\%\).`,
              title: R`WACC when \(r_U = 10\%\), \(r_D = 6\%\), \(T_c = 30\%\)`,
              x: { label: 'Debt-to-equity ratio D/E', min: 0, max: 1.5 }, y: { label: 'WACC', fmt: '%', min: 7, max: 11 },
              series: [{ name: 'No taxes', c: 'grey', dash: true, pts: waccLine(0) }, { name: 'With taxes', c: 3, pts: waccLine(TW.tc) }],
              marks: [{ x: 0.5, y: P(FIN.wacc({ E: 400, D: 200, re: 0.114, rd: 0.06, tc: 0.3 })), label: tp(FIN.wacc({ E: 400, D: 200, re: 0.114, rd: 0.06, tc: 0.3 }), 1), c: 3, pos: 'below' }] } },
          ...
          { kind: 'learn', title: 'No taxes or taxes: a summary',
            ...
            viz: [mmChart('No taxes', 0, R`No taxes: \(r_E\) rises, the WACC stays at \(r_U\), and firm value stays the same.`, true),
