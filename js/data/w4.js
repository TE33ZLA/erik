...
(function (root) {
  ...
  /* ---------- lesson pictures (viz): numbers come from the same inputs as the card text ---------- */
  const IRR_L = FIN.irr(EX_L);
  const PV_IN_L = sum(EX_L.slice(1).map((c, i) => disc(c, 0.1, i + 1))); // PV of L's inflows at 10%
  const NO_IRR = [1000, -3000, 2500];
  /** NPV profile points [rate in %, NPV] for rates 0..max% */
  const profile = (cfs, max, step = 1) => Array.from({ length: Math.round(max / step) + 1 }, (_, k) => [k * step, FIN.npv((k * step) / 100, cfs)]);
  const runTotal = (cfs, t) => sum(cfs.slice(0, t + 1));
  /** "60 ÷ 1.1^{2}": one cash flow discounted back to today (chart label) */
  const discLabel = (c, t) => `${c} ÷ 1.1${t > 1 ? `^{${t}}` : ''}`;
  ...
  root.registerPack({
    ...
    briefing: [
      ...
      { h: 'Net present value (NPV)', points: [
        ...
        R`\(NPV = PV(\text{benefits}) - PV(\text{costs})\). It measures the **gain in shareholder wealth**.`,
      ...
      { h: 'NPV profiles and the crossover rate', points: [
        ...
        R`The y-intercept is the NPV at 0%: the **sum of the cash flows**. The x-intercept is the **IRR**.`,
        R`Later (rising) cash flows give a **steeper** profile.`,
        R`The **crossover rate** is the rate where the two NPVs are equal. It is the **IRR of the incremental cash flows** \((A - B)\).`,
      ...
      'w4-L1': {
        ...
        cards: [
          ...
          { kind: 'learn', title: 'Why not just add it up?',
            ...
            viz: { type: 'bars', fmt: '$', key: true, line: { v: -CAFE[0], label: 'Cost' },
              bars: [{ label: 'Added up', v: sum(CAFE.slice(1)), c: 'grey' }, { label: 'Value today', v: FIN.pvAnnuity(4000, 0.1, 3), c: 3, note: T.money(FIN.pvAnnuity(4000, 0.1, 3)) }],
              cap: R`Just added up, the cash in beats the \(\$10{,}000\) cost. Valued **today** at 10%, it falls short.` },
          ...
          { kind: 'learn', title: 'The discount rate k',
            ...
            viz: [
              { type: 'numline', min: 0, max: 20, fmt: '%', ticks: [0, 5, 10, 15, 20],
                zones: [{ from: 0, to: 10, c: 'bad', label: 'Invest elsewhere' }, { from: 10, to: 20, c: 'good', label: 'Earns enough' }],
                marks: [{ v: 10, label: 'k = 10%', c: 1 }],
                cap: R`With \(k = 10\%\), a project must earn at least 10%. Below that, investors do better elsewhere.` },
              { type: 'seesaw', down: 'right', left: { icon: '⚠️', label: 'k' }, right: { icon: '💵', label: 'Value today' },
                cap: R`A higher \(k\) makes later cash worth **less** today.` },
            ],
          ...
          { kind: 'learn', title: 'The five steps',
            ...
            viz: { type: 'flow', steps: [
              { icon: '🔮', t: 'Estimate', s: 'the cash flows' },
              { icon: '🎲', t: 'Assess', s: 'their risk' },
              { icon: '🎯', t: 'Choose', s: R`the rate \(k\)` },
              { icon: '🧮', t: 'Find', s: 'the NPV or IRR' },
              { icon: '✅', t: 'Decide', s: 'accept if it adds value', c: 'good' }],
              cap: 'Always in this order: you need the cash flows and their risk before you can pick the rate.' },
          ...
          { kind: 'learn', title: 'Independent or mutually exclusive?',
            ...
            viz: { type: 'compare', items: [
              { icon: '🤝', title: 'Independent', big: 'A and B', points: ['A coffee machine **and** a website', 'Judge each one alone'], c: 1 },
              { icon: '⚔️', title: 'Mutually exclusive', big: 'A or B', points: ['Room for **one** machine only', 'Pick the better one'], c: 2 }],
              cap: 'Independent projects can all go ahead. Mutually exclusive ones compete for the same spot.' } },
          ...
          { kind: 'learn', title: 'Count the sign changes',
            ...
            viz: [
              { type: 'bars', title: 'Conventional: 1 change', sign: true, values: false, bars: [-1, 1, 1, 1, 1].map((v, t) => ({ label: 'Yr ' + t, v })),
                alt: 'Signs in years 0 to 4: minus, plus, plus, plus, plus. One sign change.', cap: 'Red, then green: the colour flips **once**.' },
              { type: 'bars', title: 'Non-conventional: 2 changes', sign: true, values: false, bars: [-1, 1, 1, 1, -1].map((v, t) => ({ label: 'Yr ' + t, v })),
                alt: 'Signs in years 0 to 4: minus, plus, plus, plus, minus. Two sign changes.', cap: 'A clean-up cost at the end flips it back: **two** flips.' },
            ],
      ...
      'w4-L2': {
        ...
        cards: [
          { kind: 'learn', title: 'How long until you are paid back?',
            ...
            viz: { type: 'bars', fmt: '$', sign: true, h: 230, line: { v: 100 },
              bars: [1, 2, 3, 4].map((t) => ({ label: 'Yr ' + t, v: Math.min(100, 30 * t) })),
              cap: R`The dashed line is the \(\$100\) you lent. The total reaches it early in year 4: the **payback period**.` } },
          { kind: 'learn', title: 'Keep a running total',
            ...
            viz: { type: 'bars', fmt: '$', sign: true, key: true,
              bars: EX_L.map((_, t) => ({ label: t ? 'Yr ' + t : 'Today', v: runTotal(EX_L, t) })),
              cap: 'Red bars: money still to recover. The total crosses zero, so the cost is paid back, **during year 3**.' },
          ...
          { kind: 'learn', title: 'Count part of the last year', formula: 'payback',
            ...
            viz: { type: 'split', fmt: '$', total: R`Year 3 brings \(${M(EX_L[3])}\)`,
              parts: [{ label: 'Still to recover', v: -runTotal(EX_L, 2), c: 2 }, { label: 'The rest of year 3', v: EX_L[3] + runTotal(EX_L, 2), c: 'grey' }],
              cap: R`Only \(${M(-runTotal(EX_L, 2))}\) of the \(${M(EX_L[3])}\) is needed: \(\frac{${-runTotal(EX_L, 2)}}{${EX_L[3]}} = ${L.numT(-runTotal(EX_L, 2) / EX_L[3], 4)}\) of the year.` },
          ...
          { kind: 'learn', title: 'Equal cash flows: a shortcut',
            ...
            viz: { type: 'flow', op: true, steps: [
              { t: R`\(\$15{,}000\)`, s: 'the cost' },
              { t: R`\(\$7{,}000\)`, s: 'cash each year' },
              { t: R`\(${L.num(15000 / 7000)}\) years`, s: 'the payback', c: 3 }],
              links: [R`\(\div\)`, R`\(=\)`],
              cap: R`Two years bring \(\$14{,}000\). A small part of year 3 covers the last \(\$1{,}000\).` },
          ...
          { kind: 'learn', title: 'What payback ignores',
            ...
            viz: { type: 'tl', cfs: EX_L.concat(['+$100m']), unit: 'Year', dim: [4],
              spans: [{ from: 0, to: 3, label: 'Counted, not discounted', c: 1 }, { from: 3, to: 4, label: 'Ignored', c: 'bad', row: 1 }],
              cap: R`Payback stops at \(${L.numT(FIN.payback(EX_L), 4)}\) years. It never sees the \(\$100\) million in year 4.` },
      ...
      'w4-L3': {
        ...
        cards: [
          { kind: 'learn', title: 'Value today minus cost today',
            ...
            viz: { type: 'balance', tilt: 'left',
              left: { icon: '💰', label: 'PV of cash in', sub: T.money(PV_IN_L), c: 'good' },
              right: { icon: '🏗️', label: 'Cost today', sub: T.money(-EX_L[0], 0), c: 'bad' },
              note: R`\(NPV = ${M(PV_IN_L)} - ${M(-EX_L[0])} = ${M(FIN.npv(0.1, EX_L))}\)`,
              cap: R`Project L at 10%: the cash in outweighs the cost, so it **adds** wealth.` } },
          { kind: 'learn', title: 'The formula', formula: 'npv',
            ...
            viz: { type: 'waterfall', fmt: '$', dp: 2, key: true,
              steps: [1, 2, 3].map((t) => ({ label: discLabel(EX_L[t], t), v: disc(EX_L[t], 0.1, t) }))
                .concat([{ label: 'Cost', v: EX_L[0], note: T.money(EX_L[0], 0) }, { label: 'NPV', total: true, c: 3 }]),
              cap: 'Project L at 10%: green bars are cash flows moved back to today. The cost comes off last.' } },
          { kind: 'learn', title: 'The NPV rule',
            body: R`The NPV gives a clear rule:`,
            viz: { type: 'numline', min: -40, max: 40, fmt: '$', ticks: [0],
              zones: [{ from: -40, to: 0, c: 'bad', label: 'Reject: NPV < 0' }, { from: 0, to: 40, c: 'good', label: 'Accept: NPV > 0' }],
              marks: [{ v: 0, label: 'NPV = 0: earns exactly k', c: 'grey' }],
              alt: 'A number line of NPV. Below zero: reject. Above zero: accept. At zero the project earns exactly k.',
              cap: 'Right of zero the project adds wealth. Left of zero it destroys wealth.' },
          ...
          { kind: 'learn', title: 'Repeated cash flows: add a count list',
            ...
            viz: { type: 'tl', n: 5, unit: 'Year', at: Object.fromEntries(NPV_CNT.map((c, t) => [t, T.money(c / 1000, 0) + 'k'])),
              spans: [{ from: 1, to: 5, label: 'The same $140,000, 5 times', c: 1 }],
              cap: R`One value in the list, \(\{140000\}\), and one count, \(\{5\}\), cover all five years.` },
          ...
          { kind: 'learn', title: 'Cash flows that last forever', formula: 'pv-grow-perp',
            ...
            viz: { type: 'balance', tilt: 'left',
              left: { icon: '♾️', label: 'Cash forever', sub: `${T.money(36000, 0)} ÷ 0.12 = ${T.money(36000 / 0.12, 0)}`, c: 'good' },
              right: { icon: '🏪', label: 'Cost today', sub: T.money(250000, 0), c: 'bad' },
              note: R`\(NPV = ${M(36000 / 0.12)} - \$250{,}000 = ${M(36000 / 0.12 - 250000)}\)`,
              cap: R`The business in the calculator step below: \(\$36{,}000\) a year forever is worth more than its cost.` },
      ...
      'w4-L4': {
        ...
        cards: [
          { kind: 'learn', title: 'The project’s own interest rate',
            ...
            viz: { type: 'balance', tilt: 'level',
              left: { icon: '💵', label: 'Value today of $110', sub: `110 ÷ 1.10 = ${T.money(110 / 1.1, 0)}`, c: 'good' },
              right: { icon: '🏗️', label: 'Cost today', sub: '$100', c: 'bad' },
              note: R`At 10%: \(NPV = ${M(110 / 1.1)} - \$100 = \$0\)`,
              cap: R`At 10% the two sides balance, so \(NPV = 0\). That makes 10% the **IRR**.` } },
          { kind: 'learn', title: 'The IRR rule', formula: 'irr',
            ...
            viz: { type: 'numline', min: 0, max: 30, fmt: '%',
              zones: [{ from: 0, to: 10, c: 'bad', label: 'Reject: IRR < k' }, { from: 10, to: 30, c: 'good', label: 'Accept: IRR > k' }],
              marks: [{ v: 10, label: 'k = 10%', c: 1 }, { v: P(IRR_L), label: 'IRR = ' + pctLabel(IRR_L), c: 2 }],
              cap: R`Project L’s IRR is above \(k = 10\%\). It earns more than the cost of capital: accept.` } },
          { kind: 'learn', title: 'Watch the NPV fall to zero',
            ...
            viz: { type: 'lines', key: true, w: 340, h: 240, x: { label: 'Discount rate', fmt: '%', min: 0, max: 25 }, y: { label: 'NPV of Project L', fmt: '$' },
              series: [{ c: 1, pts: profile(EX_L, 25) }],
              marks: [{ x: P(IRR_L), y: 0, c: 2 }], vlines: [{ x: P(IRR_L), label: 'IRR = ' + pctLabel(IRR_L) }],
              cap: 'The NPV falls as the rate rises. The rate where it crosses **zero** is the IRR.' },
          ...
          { kind: 'learn', title: 'Why NPV is the safer rule',
            ...
            viz: { type: 'compare', items: [
              { icon: '💵', title: 'NPV', big: 'Dollars', points: [R`Assumes reinvesting at \(k\)`, 'Measures the wealth added'], c: 1, mark: 'good', markText: 'Follow it if they disagree' },
              { icon: '📊', title: 'IRR', big: 'A percentage', points: ['Assumes reinvesting at the IRR', 'Less realistic'], c: 2, mark: 'bad', markText: 'Can mislead' }],
              cap: 'For one independent project the two rules agree. When they **disagree**, trust the dollars.' },
      ...
      'w4-L5': {
        ...
        cards: [
          ...
          { kind: 'learn', title: 'Two points you can find fast',
            ...
            viz: { type: 'lines', w: 340, h: 240, x: { label: 'Discount rate', fmt: '%', min: 0, max: 25 }, y: { label: 'NPV of Project L', fmt: '$', max: 65 },
              series: [{ c: 1, dash: true, pts: profile(EX_L, 25) }],
              marks: [{ x: 0, y: sum(EX_L), label: 'y-intercept: ' + T.money(sum(EX_L), 0), c: 3, pos: 'right' }, { x: P(IRR_L), y: 0, c: 2 }],
              vlines: [{ x: P(IRR_L), label: 'x-intercept: IRR ' + pctLabel(IRR_L) }],
              cap: 'Find the two dots first. Then join them with a smooth curve.' },
          ...
          { kind: 'learn', title: 'The crossover rate', formula: 'crossover',
            ...
            viz: { type: 'lines', key: true, w: 360, h: 240, x: { label: 'Discount rate', fmt: '%', min: 0, max: 25 }, y: { label: 'NPV', fmt: '$' },
              series: [{ name: 'L', c: 1, pts: profile(EX_L, 25) }, { name: 'S', c: 2, pts: profile(EX_S, 25) }],
              marks: [{ x: P(CROSS_LS), y: FIN.npv(CROSS_LS, EX_L), label: 'Crossover', c: 3, pos: 'left' }],
              keys: [{ c: 1, label: 'Project L (late cash)' }, { c: 2, label: 'Project S (early cash)' }],
              cap: 'Left of the dot, L has the higher NPV. Right of it, S does.' },
          ...
          { kind: 'learn', title: 'When NPV and IRR disagree',
            ...
            viz: { type: 'numline', min: 0, max: 25, fmt: '%', ticks: [0, 5, 10, 15, 20, 25],
              zones: [{ from: 0, to: P(CROSS_LS), c: 'bad', label: 'Rules clash' }, { from: P(CROSS_LS), to: 25, c: 'good', label: 'Rules agree' }],
              marks: [{ v: P(CROSS_LS), label: 'Crossover ' + pctLabel(CROSS_LS), c: 3 }],
              cap: R`With \(k\) below the crossover, NPV picks L but IRR picks S. **Follow NPV**.` },
      ...
      'w4-L6': {
        ...
        cards: [
          { kind: 'learn', title: 'Four traps',
            ...
            viz: { type: 'cards', key: true, items: [
              { icon: '📏', t: 'Scale and timing', s: 'A small or fast project can win on IRR but lose on NPV' },
              { icon: '🔀', t: 'Several IRRs', s: 'The signs change more than once' },
              { icon: '🏦', t: 'Borrowing', s: 'Cash comes in first and goes out later' },
              { icon: '🚫', t: 'No IRR', s: 'The NPV never reaches zero' }],
              cap: 'In all four traps, the **NPV** still gives the right answer.' },
          ...
          { kind: 'learn', title: 'Trap 1: scale and timing',
            ...
            viz: { type: 'bars', fmt: '$', title: 'Dollars you make',
              bars: [{ label: '50% on $10', v: 0.5 * 10, c: 'grey' }, { label: '20% on $1,000', v: 0.2 * 1000, c: 'good' }],
              cap: R`The higher percentage makes only \(\$5\). Choose the project that adds **more wealth**.` },
          ...
          { kind: 'learn', title: 'Trap 4: no IRR at all',
            ...
            viz: { type: 'lines', w: 340, h: 240, x: { label: 'Discount rate', fmt: '%', min: 0, max: 100 }, y: { label: 'NPV', fmt: '$', zero: true },
              series: [{ c: 1, pts: profile(NO_IRR, 100, 2) }],
              marks: [{ x: 10, y: FIN.npv(0.1, NO_IRR), label: 'At 10%: ' + T.money(FIN.npv(0.1, NO_IRR)), c: 'good', pos: 'right' }],
              cap: 'The curve never touches zero, so there is **no IRR**. The NPV still says accept.' },
      ...
      'w4-L7': {
        ...
        cards: [
          { kind: 'learn', title: 'Not enough money for every good project',
            ...
            viz: { type: 'compare', items: [
              { icon: '📋', title: 'Soft rationing', points: ['Managers set the budget', 'They can relax it at any time'], c: 1 },
              { icon: '🔒', title: 'Hard rationing', points: ['The firm cannot raise more money', 'Some positive-NPV projects miss out'], c: 2 }],
              cap: 'A soft limit can bend. A hard limit cannot, so some good projects are passed up.' } },
          ...
          { kind: 'learn', title: 'Value per dollar: the PI', formula: 'pi',
            ...
            viz: { type: 'anatomy', tex: R`\colA{PI} = \frac{\colB{NPV}}{\colC{\text{Initial investment}}}`,
              parts: [{ sym: 'PI', say: R`profitability index: the NPV you get for each \(\$1\) you invest` }, { sym: 'NPV', say: 'the wealth the project adds' }, { sym: R`\text{Initial investment}`, say: 'the money paid out today' }],
              cap: R`A **ratio**: NPV per dollar. It is above 0 exactly when the NPV is above 0.` },
          ...
          { kind: 'learn', title: 'Rank by PI, fund from the top',
            ...
            viz: [
              { type: 'split', fmt: '$', total: R`The \(\$300{,}000\) budget`,
                parts: [RB, RC].map((p, i) => ({ label: `${p.nm} (rank ${RANK.indexOf(p.nm) + 1})`, v: p.out, c: i ? 3 : 1 })),
                cap: 'The two best PIs fill the budget exactly. A and D miss out.' },
              { type: 'bars', fmt: '$', key: true, title: 'NPV created',
                bars: [{ label: 'B + C', v: RB.npv + RC.npv, c: 'good', note: T.money(RB.npv + RC.npv) }, { label: 'A alone', v: RA.npv, c: 'grey', note: T.money(RA.npv) }],
                cap: 'Same budget, but ranking by PI creates more NPV.' },
            ],
          ...
          { kind: 'learn', title: 'Weaknesses of the PI',
            ...
            viz: { type: 'compare', items: [
              { icon: '🐭', title: 'Small project', big: R`\(PI = 0.2\)`, points: [R`Costs \(\$1{,}000\)`, R`NPV \(${M(0.2 * 1000)}\)`], c: 1 },
              { icon: '🐘', title: 'Big project', big: R`\(PI = 0.2\)`, points: [R`Costs \(\$1{,}000{,}000\)`, R`NPV \(${M(0.2 * 1e6)}\)`], c: 2 }],
              cap: 'Same PI, but the big project adds 1,000 times more wealth. The PI **hides size**.' },
      ...
      'w4-L8': {
        ...
        cards: [
          { kind: 'learn', title: 'A 2-year machine against a 4-year machine',
            ...
            viz: { type: 'split', scale: 'abs', rows: [['Short', SHORT, NPV_SH], ['Long', LONG, NPV_LG]].map(([nm, cfs, npv]) => ({
              label: `${nm}: NPV ${T.money(npv)}`, parts: [{ label: 'Years the machine works', v: cfs.length - 1, c: 1, show: `${cfs.length - 1} years` }] })),
              cap: 'Long’s NPV is bigger, but it buys twice as many years of work. That is not a fair test.' },
          ...
          { kind: 'learn', title: 'Method 1: the replacement chain',
            ...
            viz: { type: 'tl', cfs: [SHORT[0], SHORT[1], SHORT[2] + SHORT[0], SHORT[1], SHORT[2]], unit: 'Year', hi: [2],
              spans: [{ from: 0, to: 2, label: 'First Short', c: 1 }, { from: 2, to: 4, label: 'Second Short', c: 2 }],
              cap: 'Two Short machines, one after the other, cover the same 4 years as one Long machine.' } },
          ...
          { kind: 'learn', title: 'Method 2: the equivalent annual annuity', formula: 'eav',
            ...
            viz: { type: 'tl', cfs: [NPV_SH, EAA_SH, EAA_SH], unit: 'Year', hi: [0],
              moves: [{ from: 1, to: 0, label: '÷ 1.1', c: 2 }, { from: 2, to: 0, label: '÷ 1.1^{2}', c: 2 }],
              cap: R`Moved back to today, \(${M(EAA_SH)}\) a year for 2 years is worth Short’s NPV, \(${M(NPV_SH)}\). That is its EAA.` } },
          ...
          { kind: 'learn', title: 'Costs only: the equivalent annual cost',
            ...
            viz: [
              { type: 'bars', fmt: '$', key: true, title: 'NPV of all the costs', hi: [1],
                bars: [{ label: 'P', v: NPV_P, note: T.money(NPV_P) }, { label: 'Q', v: NPV_Q, note: T.money(NPV_Q) }],
                cap: 'Q looks cheaper in total, but it lasts 3 years, not 6.' },
              { type: 'bars', fmt: '$', title: 'Cost per year (EAC)', hi: [0],
                bars: [{ label: 'P', v: EAC_P, note: T.money(EAC_P) }, { label: 'Q', v: EAC_Q, note: T.money(EAC_Q) }],
                cap: 'Per year, P is cheaper. Choose P.' },
            ],
          ...
          { kind: 'learn', title: 'When to retire an old machine',
            ...
            viz: { type: 'bars', fmt: '$', c: 3, hi: [1], title: 'Value today of each choice',
              bars: [{ label: 'Retire now', v: 10000, note: T.money(10000, 0) }, { label: 'In 1 year', v: RET1, note: T.money(RET1) }, { label: 'In 2 years', v: RET2, note: T.money(RET2) }],
              cap: 'The middle bar is the tallest: keep the machine 1 more year, then retire it.' },
    ...
    questions: [
      ...
      { id: 'w4-q13', topic: 'payback', kind: 'num', level: 2, section: 'B', src: 'Mock MST Q06', formula: 'payback',
        ...
        mistakes: [
          ...
          { v: 1450000 / (sum(BARCODE.slice(1)) / 4), why: R`The shortcut \(\text{cost} \div \text{average cash flow}\) only works when the cash flows are equal.` },
        ],
        steps: [
      ...
      { id: 'w4-q15', topic: 'npv', kind: 'mcq', level: 1, section: 'A', src: 'Lecture W4',
        ...
        choices: [R`\(NPV = PV(\text{benefits}) - PV(\text{costs})\), the net gain in shareholder wealth`, 'NPV equals the project’s accounting profit', 'NPV shows how quickly the cost is recovered', 'NPV is the rate of return the project earns'], answer: 0,
      ...
      { id: 'w4-q18', topic: 'npv', kind: 'num', level: 1, section: 'B', src: 'Lecture W4 Example 1', formula: 'npv',
        ...
        mistakes: [
          ...
          { v: FIN.npv(0.1, EX_L) / 1.1, why: R`That discounts every cash flow one year too many. The $100 is paid today, at \(t = 0\).` },
        ],
      ...
      { id: 'w4-q20', topic: 'npv', kind: 'num', level: 2, section: 'B', src: 'Tutorial W4 Q2', formula: 'pv-perp',
        ...
        mistakes: [
          ...
          { v: -190000 + 30000 / 0.15 / 1.15, why: R`The perpetuity formula already gives the value at \(t = 0\), one period before the first cash flow. Do not discount again.` },
      ...
      { id: 'w4-q24', topic: 'npv', kind: 'num', level: 3, section: 'B', src: 'Tutorial W4 Q7', formula: 'pv-grow-perp', boss: true,
        ...
        mistakes: [
          ...
          { v: 11 - 8 + 5 / 0.5, why: R`The lost TV income is paid at \(t = 1\), so discount it.` },
          { v: 11 - 8 / 1.1 + 5 / (0.1 - 0.4), why: R`The royalties shrink, so \(g = -40\%\) and \(r - g = 0.10 + 0.40 = 0.50\).` },
          { v: 11 - 8 / 1.1 + 5 / 0.1, why: R`The royalties fall by 40% a year. Use a growing perpetuity with \(g = -40\%\).` },
        ],
        steps: [
      ...
      { id: 'w4-q31', topic: 'irr', kind: 'mcq', level: 2, section: 'A', src: 'Tutorial W4 concept check 3',
        ...
        choices: [R`Because the IRR is the rate where \(NPV = 0\), the IRR rule always identifies the correct decision`, 'The IRR rule says to reject any project whose IRR is below the cost of capital', 'For a standalone (independent) project, the NPV and IRR rules usually agree', 'Some projects have more than one IRR'], answer: 0,
      ...
      { id: 'w4-q37', topic: 'profile', kind: 'mcq', level: 1, section: 'A', src: 'Tutorial W4 Q6',
        ...
        choices: [R`At the IRR, where \(NPV = 0\)`, 'At the cost of capital', 'At the crossover rate', 'At a 0% discount rate'], answer: 0,
      ...
      { id: 'w4-q38', topic: 'profile', kind: 'mcq', level: 2, section: 'A', src: 'Mock MST Q07',
        ...
        chart: { type: 'npv', projects: [{ name: 'Curve 1', cfs: MOCK7_D }, { name: 'Curve 2', cfs: MOCK7_R }], rMax: 0.25, showIRR: true },
      ...
      { id: 'w4-q45', topic: 'profile', kind: 'num', level: 3, section: 'B', src: 'Tutorial W4 Q6', formula: 'crossover', boss: true,
        ...
        steps: [
          R`Incremental cash flows, Project 1 minus Project 2: ${TM_D.map((d) => R`\(${L.moneyT(d)}\)`).join(', ')}.`,
      ...
      { id: 'w4-q55', topic: 'pi', kind: 'mcq', level: 1, section: 'A', formula: 'pi',
        ...
        why: R`PI is the value created per dollar of resource consumed. Some textbooks use \(\frac{PV}{\text{cost}}\) with a cut-off of 1. This unit uses \(\frac{NPV}{\text{investment}}\) with a cut-off of 0.` },
      ...
      { id: 'w4-q58', topic: 'pi', kind: 'num', level: 2, section: 'B', src: 'Lecture W4 Example 2', formula: 'pi',
        ...
        mistakes: [
          { v: 1 + HOBART.pi, why: R`That is \(\frac{PV \text{ of inflows}}{\text{cost}}\), the textbook version. BFC2140 uses \(\frac{NPV}{\text{investment}}\).` },
      ...
      { id: 'w4-q60', topic: 'pi', kind: 'mcq', level: 2, section: 'B', src: 'Tutorial W4 Q4', formula: 'pi',
        ...
        why: R`Ranking by PI picks D (0.739) and C (0.546). Together they use the $35m and create \(${L.moneyT(T4[2].npv + T4[3].npv, 2)}\text{m}\) of NPV.` },
      ...
      { id: 'w4-q65', topic: 'lives', kind: 'num', level: 3, section: 'B', src: 'Lecture W4 (replacement chain)', formula: 'npv', boss: true,
        ...
        mistakes: [
          ...
          { v: NPV_Y_CLEAN + NPV_Y_CLEAN / Math.pow(1.1, 6), why: R`The second cycle’s NPV is valued at \(t = 5\) (when it starts), not \(t = 6\).` },
        ],
      ...
      { id: 'w4-q67', topic: 'lives', kind: 'num', level: 2, section: 'B', src: 'Tutorial W4 Q5', formula: 'eav',
        ...
        mistakes: [
          ...
          { v: NPV_BS * 0.1, why: R`\(NPV \times k\) is a perpetuity payment. This project lasts only 2 years.` },
    ...
    generators: [
          ...
          for (let tries = 0; tries < 60; tries++) {
            ...
            return {
              ...
              mistakes: clean([
                ...
                { v: cost / (sum(cfs.slice(1)) / n), why: R`The shortcut \(\text{cost} \div \text{average cash flow}\) only works when every cash flow is equal.` },
              ], pb, 'yrs', 2),
              steps: [
      ...
      { id: 'w4-g-payback-decide', topic: 'payback', level: 2, section: 'B', formula: 'payback', src: 'Tutorial W4 Q1',
        make(rng) {
          for (let tries = 0; tries < 80; tries++) {
            ...
            return {
              kind: 'mcq',
              q: R`Your boss tells {NAME} to accept only projects that pay back within **${cut} years**. The projects are independent. Which should be accepted?`,
      ...
      { id: 'w4-g-perp', topic: 'npv', level: 2, section: 'B', formula: 'pv-grow-perp', src: 'Tutorial W4 Q2',
        make(rng) {
          ...
          if (g === 0) {
            mistakes.push({ v: c1 / r / (1 + r) - cost, why: R`\(\frac{C}{r}\) already values the perpetuity at \(t = 0\), one period before the first cash flow. Do not discount again.` });
            ...
            mistakes.push({ v: (c1 * (1 + g)) / (r - g) - cost, why: R`The cash flow given is already next year’s, \(C_1\). Do not grow it again.` });
          }
          return {
        ...
        make(rng) {
          ...
          return {
            ...
            mistakes: [
              ...
              { v: adv - lost + roy / (r + d), why: R`The lost income is paid at \(t = 1\), so discount it.` },
      ...
      { id: 'w4-g-irr', topic: 'irr', level: 2, section: 'B', formula: 'irr', src: 'Lecture W4 Example 1',
        make(rng) {
          for (let tries = 0; tries < 60; tries++) {
            ...
            if (Number.isFinite(shifted)) mistakes.push({ v: P(shifted), why: R`That discounts each inflow one year too many. The first inflow is at \(t = 1\).` });
            const ms = clean(mistakes, P(irr), '%', 2);
            if (ms.length < 3) continue;
            const co = rng.company();
            return {
              q: R`${co} is looking at the project below. What is its **IRR**? The cost of capital is ${T.pctT(k)}.`,
              givens: [['k', L.pctT(k)]],
              table: cfTable([cfs], ['Cash flow']),
      ...
      { id: 'w4-g-irr-level', topic: 'irr', level: 1, section: 'B', formula: 'irr',
        make(rng) {
          for (let tries = 0; tries < 60; tries++) {
            ...
            const mistakes = [
              { v: P(c / cost), why: R`\(\frac{C}{\text{cost}}\) is the return on a perpetuity. These payments stop after ${n} years.` },
        ...
        make(rng) {
          for (let tries = 0; tries < 200; tries++) {
            ...
            return {
              ...
              table: cfTable([pr.A, pr.B], [`Project ${a}`, `Project ${b}`]),
              chart: { type: 'npv', projects: [{ name: `Project ${a}`, cfs: pr.A }, { name: `Project ${b}`, cfs: pr.B }], rMax, showIRR: true, mark: { r: k, label: `k = ${T.pctT(k)}` } },
        ...
        make(rng) {
          for (let tries = 0; tries < 40; tries++) {
            ...
            return {
              ...
              answer: pi, unit: '', dp: 3,
              mistakes: clean([
                { v: pv / out, why: R`That is \(\frac{PV}{\text{cost}}\), the textbook version. BFC2140 uses \(\frac{NPV}{\text{initial investment}}\).` },
          ...
          for (let tries = 0; tries < 40; tries++) {
            ...
            return {
              ...
              mistakes: [
                ...
                { v: npv * r, why: R`\(NPV \times k\) is a perpetuity payment. This project lasts only ${n} years.` },
