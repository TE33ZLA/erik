...
(function (root) {
  ...
  // Tutorial Q3 joint path probabilities, as the conditional-probability lesson pictures use them
  const JOINT = { hh: 0.60, hl: 0.15, ll: 0.20, lh: +(1 - 0.60 - 0.15 - 0.20).toFixed(4) };
  const UNTER = (() => {
      ...
      'w7-L1': {
        ...
        cards: [
          { kind: 'learn', title: 'An NPV is built on guesses',
            ...
            viz: { type: 'tl', n: 3, unit: 'Year', at: { 0: 'Cost', 1: 'Guess', 2: 'Guess', 3: 'Guess' }, hi: [0],
              moves: [{ from: 1, to: 0, c: 2 }, { from: 2, to: 0, c: 2 }, { from: 3, to: 0, c: 2, label: 'discount to today' }],
              spans: [{ from: 1, to: 3, label: 'forecasts: best guesses', c: 2 }],
              cap: R`Only today’s cost is known. Every later cash flow in the NPV is a **forecast**.` } },
          { kind: 'learn', title: 'What could turn out different?',
            ...
            viz: { type: 'bars', fmt: '$', bars: [{ label: 'Forecast', v: 200 * 5, c: 1 }, { label: 'Might happen', v: 120 * 4.5, c: 'bad' }],
              cap: R`Two inputs slip, and a day’s sales fall from \(\$1{,}000\) (\(200 \times \$5\)) to \(\$540\) (\(120 \times \$4.50\)).` },
          ...
          { kind: 'learn', title: 'Four tools to test a forecast',
            ...
            viz: { type: 'cards', key: true, c: 1, items: [
              { icon: '⚖️', t: 'Break-even', s: 'How far can an input fall before NPV hits 0?' },
              { icon: '🎚️', t: 'Sensitivity', s: 'Turn one dial' },
              { icon: '🎬', t: 'Scenario', s: 'Turn several dials together' },
              { icon: '🌳', t: 'Decision tree', s: 'Map choices and chances' }],
              cap: R`Four tools, one question: **what if** our numbers are wrong?` },
          ...
          { kind: 'learn', title: 'Garbage in, garbage out',
            ...
            viz: { type: 'flow', steps: [
              { icon: '🗑️', t: 'Bad inputs', s: 'unreliable forecasts', c: 'bad' },
              { icon: '⚙️', t: 'Any tool', s: 'even all four', c: 'grey' },
              { icon: '🗑️', t: 'Bad answers', s: 'unreliable results', c: 'bad' }],
              links: ['go in', 'come out'],
              cap: R`No tool can clean up its inputs. Unreliable forecasts in means unreliable answers out.` } },
          ...
          { kind: 'learn', title: 'What stays out',
            ...
            viz: { type: 'compare', vs: false, items: [
              { icon: '🧾', title: 'Sunk cost', big: 'In the past', mark: 'bad', markText: 'Leave it out', c: 'grey' },
              { icon: '🏦', title: 'Interest', big: 'In the rate', mark: 'bad', markText: 'Leave it out', c: 'grey' }],
              cap: R`Both are real costs, but neither belongs in the project’s free cash flows.` } },
      ...
      'w7-L2': {
        ...
        cards: [
          { kind: 'learn', title: 'Breaking even',
            ...
            // an idea picture: no numbers in the card, so no numbers on the axes (revenue 2 a cup, costs 40 + 1 a cup)
            viz: { type: 'lines', key: true, x: { label: 'Cups sold', min: 0, max: 80, ticks: [] }, y: { label: 'Dollars', hide: true, min: 0 },
              series: [{ name: 'Revenue', c: 1, pts: [[0, 0], [80, 160]] }, { name: 'Total cost', c: 2, pts: [[0, 40], [80, 120]] }],
              shade: [{ x1: 0, x2: 40, c: 'bad', label: 'Loss' }, { x1: 40, x2: 80, c: 'good', label: 'Profit' }],
              marks: [{ x: 40, y: 80, label: 'Break-even', c: 3, pos: 'right' }],
              cap: R`Left of the dot the costs are higher: a loss. Right of it the sales win: a profit.` } },
          { kind: 'learn', title: 'Margin per unit',
            ...
            viz: { type: 'split', fmt: '$', total: R`Price of one unit: \(\$80\)`, parts: [{ label: 'Cost per unit', v: 60, c: 2 }, { label: 'Margin per unit', v: 80 - 60, c: 3 }],
              cap: R`Out of each \(\$80\) sale, \(\$60\) pays for making it. The aqua \(\$20\) is the margin.` } },
          { kind: 'learn', title: 'The accounting (EBIT) break-even',
            ...
            viz: { type: 'balance', tilt: 'level', left: { icon: '📦', label: 'All the margins', sub: 'units × margin', c: 3 }, right: { icon: '🏢', label: 'Costs to cover', sub: 'SG&A + Dep', c: 2 },
              note: R`Level: \(EBIT = 0\)`,
              cap: R`At the EBIT break-even, the margins from all the units exactly cover SG&A and depreciation.` },
          ...
          { kind: 'learn', title: 'The NPV break-even',
            ...
            viz: { type: 'compare', items: [
              { icon: '📒', title: 'EBIT break-even', big: R`\(EBIT = 0\)`, points: ['Covers the costs on paper', 'Ignores the cost of capital'], mark: 'bad', markText: 'Can still lose value', c: 'grey' },
              { icon: '💰', title: 'NPV break-even', big: R`\(NPV = 0\)`, points: ['Uses cash flows', 'Charges the cost of capital'], mark: 'good', markText: 'The real test', c: 1 }],
              cap: R`Only sales above the **NPV** break-even create value for the owners.` } },
        ...
        cards: [
          { kind: 'learn', title: 'Turn one dial',
            ...
            viz: { type: 'cards', key: true, items: [
              { icon: '🎚️', t: 'Unit sales', s: 'Turned', c: 2 },
              { icon: '🔒', t: 'Price', s: 'Stays at base', c: 'grey' },
              { icon: '🔒', t: 'Cost per unit', s: 'Stays at base', c: 'grey' },
              { icon: '🔒', t: 'Discount rate', s: 'Stays at base', c: 'grey' }],
              cap: R`One dial turns (orange). The other three stay locked at base. Then watch the NPV.` } },
          { kind: 'learn', title: 'The lecture project',
            ...
            viz: { type: 'anatomy', tex: R`NPV = \frac{(\colA{P} - \colB{v}) \times \colC{Q}}{\colD{r}} - \colE{I}`,
              parts: [{ sym: 'P', say: 'price per unit: $80' }, { sym: 'v', say: 'cost per unit: $60' }, { sym: 'Q', say: 'units sold a year: 6,000' }, { sym: 'r', say: 'cost of capital: 10%' }, { sym: 'I', say: 'initial cost: $500,000' }],
              cap: R`Each coloured letter is one dial. A sensitivity test turns one of them at a time.` },
          ...
          { kind: 'learn', title: 'Why the NPV moves more',
            ...
            viz: { type: 'split', scale: 'abs', rows: [6000, 5500].map((q) => ({
              label: q === 6000 ? R`Base: inflows worth \(${mL((20 * q) / 0.1 / 1e6, 1)}\)` : R`5,500 units: inflows worth \(${mL((20 * q) / 0.1 / 1e6, 1)}\)`,
              parts: [{ label: 'Initial cost (fixed)', v: 0.5, c: 2, show: '$0.5m' }, { label: 'NPV', v: npvPerp(q, 80, 60, 0.1, 5e5) / 1e6, c: 3, show: mS(npvPerp(q, 80, 60, 0.1, 5e5)) }] })),
              cap: R`The cost stays at \(\$0.5\text{m}\), so the whole \(\$0.1\text{m}\) fall comes out of the NPV.` },
          ...
          { kind: 'learn', title: 'Which input matters most?',
            ...
            viz: { type: 'bars', hi: [1], bars: [
              ['Sales', npvPerp(6500, 80, 60, 0.1, 5e5) - npvPerp(5500, 80, 60, 0.1, 5e5)],
              ['Price', npvPerp(6000, 85, 60, 0.1, 5e5) - npvPerp(6000, 75, 60, 0.1, 5e5)],
              ['Cost', npvPerp(6000, 80, 58, 0.1, 5e5) - npvPerp(6000, 80, 62, 0.1, 5e5)],
              ['Rate', npvPerp(6000, 80, 60, 0.08, 5e5) - npvPerp(6000, 80, 60, 0.12, 5e5)]].map(([label, v]) => ({ label, v, c: 2, note: mS(v) })),
              cap: R`Each bar is one input’s NPV swing. The price has the tallest bar, so it matters most.` },
        ...
        cards: [
          { kind: 'learn', title: 'Bad news travels together',
            ...
            viz: { type: 'cards', items: [
              { icon: '📉', t: 'Unit sales', s: 'Fall', c: 'bad' },
              { icon: '🏷️', t: 'Price', s: 'Cut', c: 'bad' },
              { icon: '🏦', t: 'Discount rate', s: 'Rises: borrowing is dearer', c: 'bad' }],
              cap: R`One story, a recession, turns several dials at once. That whole story is one **scenario**.` } },
          ...
          { kind: 'learn', title: 'Scenario or sensitivity?',
            ...
            viz: { type: 'compare', key: true, items: [
              { icon: '🎚️', title: 'Sensitivity', big: 'One input', points: ['The rest stay at base', 'Which input matters most?'], c: 1 },
              { icon: '🎬', title: 'Scenario', big: 'Several inputs', points: ['They move together, as one story', 'How bad or how good could it get?'], c: 2 }],
              cap: R`Count the inputs that move. One: sensitivity. Several at once: scenario.` },
          ...
          { kind: 'learn', title: 'Reading the results',
            ...
            viz: { type: 'bars', fmt: '$', sign: true, line: { v: 0, label: 'NPV = 0' },
              bars: [['Worst', npvPerp(5500, 75, 62, 0.12, 5e5)], ['Base', npvPerp(6000, 80, 60, 0.10, 5e5)], ['Best', npvPerp(6500, 85, 58, 0.08, 5e5)]].map(([label, v]) => ({ label, v })),
              cap: R`All three bars stay above zero, but look how far apart they are.` },
      ...
      'w7-L5': {
        ...
        cards: [
          { kind: 'learn', title: 'A coin-toss bet',
            ...
            viz: { type: 'bars', fmt: '$', bars: [{ label: 'Heads\n(50%)', v: 10, c: 1 }, { label: 'Tails\n(50%)', v: 2, c: 1 }], line: { v: 0.5 * 10 + 0.5 * 2, label: 'Average: $6' },
              cap: R`No single toss pays \(\$6\). It is the average win over many tosses: the **expected value**.` } },
          { kind: 'learn', title: 'Probabilities',
            ...
            viz: [
              { type: 'numline', min: 0, max: 1, ticks: [0, 0.5, 1], marks: [{ v: 0, label: 'Never', c: 'grey' }, { v: 0.8, label: '80% = 0.8', c: 1 }, { v: 1, label: 'Always', c: 'grey' }],
                cap: R`Every probability sits between 0 (never) and 1 (always).` },
              { type: 'grid100', parts: [{ n: 80, c: 1, label: 'High demand: 0.8' }, { n: 20, c: 2, label: 'Low demand: 0.2' }],
                cap: R`The outcomes fill all 100 squares: \(0.8 + 0.2 = 1\).` }],
          ...
          { kind: 'learn', title: 'The expected value formula',
            ...
            viz: { type: 'flow', op: true, key: true, steps: [
              { t: R`\(0.5 \times \$10\)`, s: R`heads: \(\$5\)`, c: 1 },
              { t: R`\(0.5 \times \$2\)`, s: R`tails: \(\$1\)`, c: 1 },
              { t: R`\(\$6\)`, s: 'the expected value', c: 3 }],
              links: [R`\(+\)`, R`\(=\)`],
              cap: R`The coin toss again: each outcome times its probability, then add. That is all \(\Sigma\) means.` },
            formula: 'expected' },
          ...
          { kind: 'learn', title: 'Uncertain cash flows in an NPV',
            ...
            viz: { type: 'tl', n: 3, unit: 'Year', at: { 0: 'Cost', 1: 'E[CF]', 2: 'E[CF]', 3: 'E[CF]' },
              moves: [{ from: 1, to: 0, c: 2 }, { from: 2, to: 0, c: 2 }, { from: 3, to: 0, c: 2, label: 'discount as usual' }],
              cap: R`Each year gets its **expected** cash flow, \(E[CF]\). Then the NPV discounts it back to today.` } },
      ...
      'w7-L6': {
        ...
        cards: [
          { kind: 'learn', title: 'Choices and chances, step by step',
            ...
            viz: { type: 'flow', steps: [
              { icon: '🟦', t: 'You decide', s: 'today', c: 1 },
              { icon: '🟠', t: 'Nature', s: 'high or low?', c: 2 },
              { icon: '🟦', t: 'Decide again', s: 'later', c: 1 },
              { icon: '💵', t: 'Cash flow', s: 'the outcome', c: 3 }],
              cap: R`A tree draws these steps in order: today on the left, the future on the right.` } },
          ...
          { kind: 'learn', title: 'Valuing the nodes',
            viz: { type: 'compare', key: true, items: [
              { icon: '🟠', title: 'Chance node', big: 'Expected value', points: ['Weight each branch by its probability', 'Add them up'], c: 2 },
              { icon: '🟦', title: 'Decision node', big: 'Best branch', points: ['You choose', 'Keep the highest NPV'], c: 1 }],
              cap: R`Then discount each node’s value back to today. Never average at a decision node.` },
          ...
          { kind: 'learn', title: 'Solve from right to left',
            ...
            viz: { type: 'tl', n: 3, unit: 't', labels: { 0: 'Today', 3: 'End' }, at: { 0: 'NPV', 3: 'Outcomes' }, hi: [0, 3],
              moves: [{ from: 3, to: 2, label: '1st', c: 2 }, { from: 2, to: 1, label: '2nd', c: 2 }, { from: 1, to: 0, label: '3rd', c: 2 }],
              cap: R`Start with the outcomes at the right-hand end. Fold the tree back, one node at a time, to today.` },
      ...
      'w7-L7': {
        ...
        cards: [
          ...
          { kind: 'learn', title: 'All the paths add up to 1',
            ...
            viz: { type: 'grid100', parts: [
              { n: Math.round((JOINT.hh + JOINT.hl + JOINT.ll) * 100), c: 'grey', label: 'Given: HH + HL + LL = 95' },
              { n: Math.round(JOINT.lh * 100), c: 3, label: 'LH: the 5 that are left' }],
              cap: R`The three given paths fill 95 squares. The 5 aqua squares left belong to the missing path, LH.` },
          ...
          { kind: 'learn', title: 'The chance of year 1',
            ...
            viz: { type: 'split', rows: [
              { label: 'The four paths', parts: [['HH', JOINT.hh, 1], ['HL', JOINT.hl, 1], ['LH', JOINT.lh, 2], ['LL', JOINT.ll, 2]].map(([label, v, c]) => ({ label, v, c, show: v.toFixed(2) })) },
              { label: 'Year 1 alone', parts: [{ label: 'High year 1', v: JOINT.hh + JOINT.hl, c: 1, show: (JOINT.hh + JOINT.hl).toFixed(2) }, { label: 'Low year 1', v: JOINT.lh + JOINT.ll, c: 2, show: (JOINT.lh + JOINT.ll).toFixed(2) }] }],
              cap: R`The blue paths start high: \(0.60 + 0.15 = 0.75\). The orange ones start low: \(0.25\).` } },
          ...
          { kind: 'learn', title: 'Conditional probability',
            ...
            viz: { type: 'flow', op: true, key: true, steps: [
              { t: R`\(${(JOINT.hh + JOINT.hl).toFixed(2)}\)`, s: R`\(P(H_1)\): high year 1`, c: 1 },
              { t: R`\(${(JOINT.hh / (JOINT.hh + JOINT.hl)).toFixed(2)}\)`, s: R`\(P(H_2 \mid H_1)\): high again`, c: 2 },
              { t: R`\(${JOINT.hh.toFixed(2)}\)`, s: R`\(P(HH)\): the whole path`, c: 3 }],
              links: [R`\(\times\)`, R`\(=\)`],
              cap: R`Walk along the path and multiply. The second branch is **conditional**: it assumes year 1 was high.` } },
        ...
        cards: [
          { kind: 'learn', title: 'Flexibility is valuable',
            ...
            viz: { type: 'flow', key: true, steps: [
              { icon: '🟠', t: 'A bad first year', s: 'chance: not her choice', c: 2 },
              { icon: '🟦', t: 'Her choice', s: 'a right, not a duty', c: 1 },
              { icon: '🚪', t: 'Close and sell', s: 'or carry on', c: 3 }],
              cap: R`The option comes **after** the bad news. She **may** close and sell, but she does not have to.` } },
          { kind: 'learn', title: 'Three real options',
            ...
            viz: { type: 'cards', c: 1, items: [
              { icon: '🚪', t: 'Abandon', s: 'Things go badly: stop and sell' },
              { icon: '📈', t: 'Expand', s: 'Demand is high: invest more' },
              { icon: '⏳', t: 'Wait', s: 'Start small: decide later' }],
              cap: R`Each option is a choice you make **after** you see how things turn out.` } },
          { kind: 'learn', title: 'An option never lowers the NPV',
            ...
            // an idea picture: no numbers in the card, so the bars carry no values
            viz: { type: 'bars', values: false, bars: [{ label: 'Without the option', parts: [{ v: 10, c: 1 }] }, { label: 'With the option', parts: [{ v: 10, c: 1 }, { v: 3, c: 3 }] }],
              keys: [{ c: 1, label: 'NPV without the option' }, { c: 3, label: R`What the option adds: never below \(0\)` }],
              cap: R`At worst you ignore the option and the NPV stays the same. It can only add value.` } },
          ...
          { kind: 'learn', title: 'Expand: machine B’s upgrade',
            ...
            viz: { type: 'bars', hi: [1], bars: [{ label: 'Upgrade', v: CHIP.upH, c: 2 }, { label: 'No upgrade', v: CHIP.noH, c: 1 }].map((b) => Object.assign(b, { note: `$${T.numT(b.v, 3)}m` })),
              cap: R`Values at \(t = 3\). Not upgrading is worth more, so the decision node keeps that branch.` },
            tree: TREE_CHIP_UP,
