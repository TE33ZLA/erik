          ...
          { kind: 'guided', title: 'Your turn: a 5-year project', q: R`A project costs $300,000 today. Each unit sells for $45 and costs $20 to make. Sales are the same every year for **5 years**. No tax. The cost of capital is 10%. How many units a year make NPV = 0?`,
            parts: [
              ...
              { ask: R`Which yearly margin makes NPV = 0? It must repay \(\$300{,}000\) over 5 years at 10%.`, answer: FIN.pmt(300000, 0.10, 5), unit: '$', dp: 2,
                ...
                mistakes: [{ v: 60000, why: 'That ignores the time value of money (300,000 ÷ 5). Discount at 10%.' }, { v: 30000, why: 'That treats it as a perpetuity. It only lasts 5 years.' }] },
        ...
        cards: [
          ...
          { kind: 'guided', title: 'Your turn: the best case', q: R`Lecture project, best case: 6,500 units, price $85, cost $58 per unit and an 8% cost of capital. It costs $500,000, with cash flows forever.`,
            parts: [
              ...
              { ask: R`What is the yearly margin (margin × units)?`, answer: 27 * 6500, unit: '$', dp: 0, hint: R`\(27 \times 6{,}500\)`, why: R`\(27 \times 6{,}500 = \$175{,}500\).` },
          ...
          { kind: 'recap', title: 'Remember', points: [
            ...
            R`Exam trap: sensitivity = **one** input at a time; scenario = **many** at once. A statement that swaps them is false.`,
          ], formula: 'npv' },
        ],
      },
    },
        ...
        make(rng) {
          ...
          for (let k = 0; k < 80; k++) {
            ...
            return {
              ...
              ti: [
                TI.line(`(${tn(lh)}*${tn(H)}+${tn(ll)}*${tn(Lo)})/${tn(b.pL1)}${dsc(r, 1)}→k`, { note: R`Keep after a low year 1. The missing LH path is \(1 - ${L.dec(hh)} - ${L.dec(hl)} - ${L.dec(ll)} = ${L.dec(lh)}\). Divide the LH and LL paths by \(P(L_1) = ${L.dec(b.pL1)}\), then discount one year. Selling gives \(${LM(b.sal)}\), so ${b.abandon ? 'sell' : 'keep'}.` }),
                TI.line(`(${tn(hh)}*${tn(H)}+${tn(hl)}*${tn(Lo)})/${tn(b.pH1)}${dsc(r, 1)}→h`, { note: R`Year 2 after a high year 1, valued at \(t = 1\), stored in h.` }),
                TI.line(`-${tn(C)}+(${tn(b.pH1)}*(${tn(H)}+h)+${tn(b.pL1)}*(${tn(Lo)}+${b.abandon ? tn(b.sal) : 'k'}))${dsc(r, 1)}`, { note: b.abandon ? 'After a low year 1 the boat is sold, so that branch gets the sale price.' : 'After a low year 1 the boat is kept, so that branch gets k.' }),
              ],
        ...
        make(rng) {
          ...
          for (let k = 0; k < 80; k++) {
            ...
            return {
              ...
              ti: (() => {
                const upS = TI.line(`npv(${P(r)},0,{${S}},{${N - 2}})-${U}`, { note: upWins ? R`Upgrade, valued at \(t = 2\) ($m). It beats not upgrading, so upgrade.` : R`Upgrade, valued at \(t = 2\) ($m).` });
                const noS = TI.cmd('npv', [P(r), 0, [s], [N - 2]], { note: upWins ? R`No upgrade, valued at \(t = 2\) ($m).` : R`No upgrade, valued at \(t = 2\) ($m). It beats upgrading, so do not upgrade.` });
                const d = tn(1 + r);
                return [upWins ? noS : upS, upWins ? upS : noS,
