...
(function (root) {
  ...
  // the bike from lesson 2 (day it is bought, the bill is paid, it is sold, the cash comes in), for the lesson pictures
  const BIKE = { buy: 0, pay: 30, sell: 50, cash: 75 };
  BIKE.inv = BIKE.sell - BIKE.buy; BIKE.ar = BIKE.cash - BIKE.sell; BIKE.ap = BIKE.pay - BIKE.buy; BIKE.op = BIKE.inv + BIKE.ar; BIKE.ccc = BIKE.op - BIKE.ap;
      ...
      'w8-L1': {
        ...
        cards: [
          { kind: 'learn', title: 'Where did the cash go?',
            ...
            viz: { type: 'cycle', center: 'Where the\ncash goes', steps: [
              { t: 'Cash in\nthe bank', icon: '🏦', c: 1 },
              { t: 'Bikes on\nthe shelf', icon: '🚲', c: 2 },
              { t: 'Unpaid\nbills', icon: '🧾', c: 2 }],
              cap: R`The cash is stuck in the orange boxes until customers pay. Only then is it back in the bank.` } },
          ...
          { kind: 'learn', title: 'Net working capital',
            ...
            // an idea picture: the card has no numbers, so the steps show signs, not amounts (shape from A Ltd in the next card)
            viz: { type: 'waterfall', key: true, steps: [
              { label: 'Cash', v: 1.6, note: '+' }, { label: 'Inventory', v: 3.0, note: '+' }, { label: 'A/R', v: 4.2, note: '+' },
              { label: 'A/P', v: -1.8, note: '−' }, { label: 'Accruals', v: -4.7, note: '−' }, { label: 'NWC', total: true, note: '=' }],
              cap: R`Stack up the current assets (green), take away the current liabilities (red). What is left is NWC.` } },
          ...
          { kind: 'learn', title: 'Working capital and free cash flow',
            ...
            viz: { type: 'seesaw', down: 'right', left: { icon: '📦', label: 'NWC' }, right: { icon: '💵', label: 'Free cash flow' },
              cap: R`More cash tied up in NWC means less FCF. Less tied up means more FCF, and a higher value.` } },
      ...
      'w8-L2': {
        ...
        cards: [
          ...
          { kind: 'learn', title: 'Three stretches of time',
            viz: bikeTL({ spans: [
              { from: 0, to: 2, label: `Inventory days: ${BIKE.inv}`, c: 1 },
              { from: 0, to: 1, label: `A/P days: ${BIKE.ap}`, c: 'grey', row: 1 }, { from: 2, to: 3, label: `A/R days: ${BIKE.ar}`, c: 2, row: 1 }],
              cap: R`The bike waits 50 days to sell, the customer takes 25 to pay, and the shop takes 30 to pay.` }),
          ...
          { kind: 'learn', title: 'The operating cycle',
            ...
            viz: bikeTL({ hi: [0, 3], spans: [
              { from: 0, to: 2, label: `Inventory: ${BIKE.inv}`, c: 1 }, { from: 2, to: 3, label: `A/R: ${BIKE.ar}`, c: 2 },
              { from: 0, to: 3, label: `Operating cycle: ${BIKE.op} days`, c: 3, row: 1 }],
              cap: R`From buying the bike (day 0) to collecting the cash (day 75): \(50 + 25 = 75\) days.` }) },
          { kind: 'learn', title: 'The cash cycle',
            ...
            viz: bikeTL({ key: true, hi: [1, 3], spans: [
              { from: 0, to: 1, label: `A/P days: ${BIKE.ap}`, c: 'grey' },
              { from: 1, to: 3, label: `Cash cycle: ${BIKE.ccc} days`, c: 3, row: 1 }],
              cap: R`The shop’s own cash is out only from day 30 (it pays) to day 75 (it collects): 45 days.` }),
            formula: 'ccc' },
          ...
          { kind: 'learn', title: 'Shorter is better',
            ...
            viz: { type: 'flow', op: true, steps: [
              { t: 'Inventory ↓', s: 'sell faster', c: 1 }, { t: 'A/R ↓', s: 'collect sooner', c: 2 },
              { t: 'A/P ↑', s: 'pay later', c: 'grey' }, { t: 'CCC ↓', s: 'shorter', c: 3 }],
              links: [R`\(+\)`, R`\(-\)`, R`\(=\)`],
              cap: R`A/P days are **subtracted**, so paying suppliers later makes the cash cycle shorter.` },
      ...
      'w8-L3': {
        ...
        cards: [
          { kind: 'learn', title: 'Too little stock',
            ...
            viz: { type: 'flow', steps: [
              { icon: '🍞', t: 'No bread left', s: 'a stock-out at 9 am', c: 'grey' },
              { icon: '🚶', t: 'Customers leave', s: 'they buy elsewhere', c: 2 },
              { icon: '💸', t: 'Lost sales', s: 'and unhappy customers', c: 'bad' }],
              cap: R`Too little stock costs sales. That is why firms keep enough inventory to meet demand.` } },
          { kind: 'learn', title: 'Too much stock',
            body: R`Holding stock costs money too.`,
            viz: { type: 'compare', key: true, items: [
              { icon: '📦', title: 'Too little stock', points: ['Stock-outs', 'Lost sales'], mark: 'bad', markText: 'Costs you sales', c: 2 },
              { icon: '📦📦📦📦', title: 'Too much stock', points: ['Order and carrying costs', 'Cash tied up for more days'], mark: 'bad', markText: 'Costs you cash', c: 2 }],
              cap: R`Both ends cost money. Firms aim for enough stock, and no more.` },
          ...
          { kind: 'learn', title: 'Just-in-time',
            ...
            viz: { type: 'flow', steps: [
              { icon: '🛒', t: 'A customer orders', s: 'a computer', c: 1 },
              { icon: '📦', t: 'Dell orders parts', s: 'only now', c: 2 },
              { icon: '🖥️', t: 'Build and ship', s: 'almost no stock waits', c: 3 }],
              cap: R`The parts arrive only when a customer needs them, so the stock balance stays near zero.` } },
          ...
          { kind: 'learn', title: 'Why hold cash at all?',
            ...
            viz: { type: 'cards', c: 1, items: [
              { icon: '🧾', t: 'Day-to-day', s: 'Bills and wages' },
              { icon: '☂️', t: 'Precaution', s: 'A cushion for surprises' },
              { icon: '🏦', t: 'Compensating', s: 'The bank requires it' }],
              cap: R`Cash earns almost nothing, yet these three needs make some worth holding.` },
          ...
          { kind: 'learn', title: 'Parking spare cash',
            ...
            viz: { type: 'flow', steps: [
              { icon: '💵', t: 'Spare cash', s: 'not needed today', c: 1 },
              { icon: '🏛️', t: 'Short-term securities', s: 'parked here: safe', c: 2 },
              { icon: '💵', t: 'Cash again', s: 'sold when needed', c: 3 }],
              cap: R`Safe and quick to sell, so the parked cash earns a little while it waits.` } },
      ...
      'w8-L4': {
        ...
        cards: [
          { kind: 'learn', title: 'From balances to days',
            ...
            viz: { type: 'flow', op: true, steps: [
              { t: R`\(\$30{,}000\)`, s: 'customers owe (a balance)', c: 1 },
              { t: R`\(\$1{,}000\)`, s: R`one day of sales: \(\frac{\$365{,}000}{365}\)`, c: 2 },
              { t: `${T.numT(30000 / (365000 / 365), 2)} days`, s: 'of sales', c: 3 }],
              links: [R`\(\div\)`, R`\(=\)`],
              cap: R`Divide a balance by one day’s worth of the yearly total. The answer is in days.` } },
          { kind: 'learn', title: 'Sales or COGS?',
            ...
            // an idea picture: one sale seen two ways (no numbers in the card, so the bars carry words, not amounts)
            viz: { type: 'split', scale: 'abs', rows: [
              { label: 'What the customer owes you: A/R, at the sales price', parts: [{ label: 'Sales', v: 100, c: 2, show: 'Sales' }] },
              { label: 'What the goods cost: inventory and A/P, at cost', parts: [{ label: 'COGS', v: 60, c: 1, show: 'COGS' }] }],
              cap: R`The same goods are worth more at the sales price. So A/R uses sales; inventory and A/P use COGS.` },
          ...
          { kind: 'learn', title: 'The cash conversion cycle',
            ...
            viz: { type: 'split', key: true, scale: 'abs', rows: [
              { label: 'Stock sits, then the customer pays', parts: [{ label: 'Inventory days', v: BIKE.inv, c: 1, show: `Inventory ${BIKE.inv}` }, { label: 'A/R days', v: BIKE.ar, c: 2, show: `A/R ${BIKE.ar}` }] },
              { label: 'Who waits for the money?', parts: [{ label: 'A/P days', v: BIKE.ap, c: 'grey', show: `Supplier ${BIKE.ap}` }, { label: 'CCC', v: BIKE.ccc, c: 3, show: `You: CCC ${BIKE.ccc}` }] }],
              cap: R`The bike again: \(50 + 25 - 30 = 45\). The supplier waits 30 days; your own cash waits 45.` },
            formula: 'ccc' },
          { kind: 'example', title: 'Worked example: A Ltd',
          ...
          { kind: 'learn', title: 'A negative CCC',
            ...
            viz: { type: 'split', scale: 'abs', rows: [
              { label: 'Stock sells, customers pay, then Woolworths holds the cash', parts: [{ label: 'Inventory days', v: 33.2, c: 1, show: '33.2' }, { label: 'A/R days', v: 1.4, c: 2, show: '' },
                { label: R`Days it holds the cash: the CCC of \(-11.4\)`, v: +(46.0 - 33.2 - 1.4).toFixed(1), c: 3, show: (46.0 - 33.2 - 1.4).toFixed(1) }] },
              { label: 'Suppliers wait for their money', parts: [{ label: 'A/P days', v: 46.0, c: 'grey', show: 'A/P 46.0' }] }],
              cap: R`Customers have paid by day 34.6, but suppliers wait until day 46. The aqua gap is the negative CCC.` },
          ...
          { kind: 'learn', title: 'Days into dollars',
            ...
            viz: { type: 'flow', op: true, steps: [
              { t: '10 days', s: 'collected sooner', c: 2 },
              { t: R`\(\$${L.numT(73 / 365, 2)}\text{m}\)`, s: R`one day of sales: \(\frac{73}{365}\)`, c: 1 },
              { t: R`\(\$${L.numT((10 * 73) / 365, 2)}\text{m}\)`, s: 'cash freed', c: 3 }],
              links: [R`\(\times\)`, R`\(=\)`],
              cap: R`Each day cut from A/R frees one day of sales in cash.` },
      ...
      'w8-L5': {
        ...
        cards: [
          { kind: 'learn', title: 'Buy now, pay later',
            ...
            viz: { type: 'flow', steps: [
              { icon: '📦', t: 'Goods now', s: 'from the supplier', c: 1 },
              { icon: '🧾', t: 'Invoice', s: 'the terms: when to pay', c: 2 },
              { icon: '💵', t: 'Pay later', s: 'a little less if early', c: 3 }],
              links: ['with', 'then'],
              cap: R`You get the goods today and pay for them later. That delay is **trade credit**.` } },
          ...
          { kind: 'learn', title: 'The seller’s side',
            ...
            viz: { type: 'compare', items: [
              { icon: '🐇', title: 'Customer pays by day 10', big: R`\(\$${100 - 2}\)`, points: [R`The seller gives up \(\$2\)`, 'The cash arrives sooner'], c: 3 },
              { icon: '🐢', title: 'Customer pays on day 30', big: R`\(\$100\)`, points: ['The full price', 'The cash arrives later'], c: 1 }],
              vsText: 'or',
              cap: R`The seller swaps \(\$2\) of the price for getting its cash 20 days sooner.` } },
          ...
          { kind: 'learn', title: 'Skipping the discount is a loan',
            ...
            viz: { type: 'tl', key: true, n: 2, unit: 'Day', labels: { 0: '0', 1: '10', 2: '30' }, at: { 0: 'Invoice', 1: 'Pay $98', 2: 'Pay $100' }, hi: [1, 2],
              moves: [{ from: 1, to: 2, label: '+ $2 of interest', c: 2 }],
              spans: [{ from: 1, to: 2, label: 'a 20-day loan', c: 1 }],
              cap: R`Skip the discount and the \(\$98\) grows to \(\$100\) in 20 days: a loan, with \(\$2\) of interest.` } },
          { kind: 'learn', title: 'The rate for those 20 days',
            ...
            viz: { type: 'flow', op: true, steps: [
              { t: R`\(\$2\)`, s: 'the interest', c: 2 },
              { t: R`\(\$98\)`, s: 'the amount borrowed', c: 1 },
              { t: R`\(${L.pct(2 / 98, 2)}\)`, s: 'for 20 days', c: 3 }],
              links: [R`\(\div\)`, R`\(=\)`],
              cap: R`Divide by \(\$98\), the amount you really borrow, not by \(\$100\).` },
      ...
      'w8-L6': {
        ...
        cards: [
          { kind: 'learn', title: 'From 20 days to a year',
            ...
            viz: { type: 'bars', fmt: '%', c: 2, bars: [20, 100, 200, 365].map((day) => ({ label: `Day ${day}`, v: 100 * ((1 + 2 / 98) ** (day / 20) - 1) })),
              cap: R`Pay \(2.04\%\) every 20 days, again and again: by the end of the year the cost is about \(${L.pct(ear(0.02, 10, 30), 1)}\).` } },
          { kind: 'learn', title: 'The formula',
            ...
            // the formula with the numbers of 2/10, net 30 in it, each coloured by its job
            viz: { type: 'anatomy', tex: R`EAR = \left(1 + \colA{\frac{2}{98}}\right)^{\colB{365} / \colC{20}} - 1`,
              parts: [
                { sym: R`\frac{2}{98}`, say: R`the rate for one period, \(\frac{d}{1-d}\): here \(${L.pct(2 / 98, 2)}\)` },
                { sym: '365', say: 'the days in a year (360 only if the question says so)' },
                { sym: '20', say: R`the extra days of credit: net days minus discount days, \(30 - 10\)` }],
              cap: R`2/10, net 30 in the formula: the period rate sits in the brackets, and the power counts the periods.` },
          ...
          { kind: 'learn', title: 'Take it or leave it?',
            ...
            // a decision rule with a cut-off; the card has no numbers, so the line has no ticks
            viz: { type: 'numline', key: true, min: 0, max: 2, ticks: [], zones: [{ from: 0, to: 1, c: 1, label: 'Skip discount' }, { from: 1, to: 2, c: 3, label: 'Take discount' }],
              marks: [{ v: 1, label: 'The bank rate', c: 2 }],
              cap: R`Put the trade-credit EAR on this line. Above the bank rate: take the discount. Below it: skip.` },
          ...
          { kind: 'learn', title: 'Never pay in between',
            ...
            viz: { type: 'tl', n: 3, unit: 'Day', labels: { 0: '0', 1: '10', 2: '20', 3: '30' }, at: { 1: '$98', 2: '$100', 3: '$100' }, hi: [1, 3], dim: [2],
              spans: [{ from: 2, to: 3, label: '10 days wasted', c: 'bad' }],
              cap: R`Day 20 pays the full \(\$100\) **and** wastes 10 free days. Pay on day 10 or on day 30.` } },
      ...
      'w8-L7': {
        ...
        cards: [
          { kind: 'learn', title: 'Two good days to pay',
            ...
            viz: { type: 'numline', min: 0, max: 30, ticks: [0, 10, 20, 30], zones: [{ from: 0, to: 30, c: 'bad', label: 'Any other day wastes money' }],
              marks: [{ v: 10, label: 'Day 10', c: 'good' }, { v: 30, label: 'Day 30', c: 'good' }],
              cap: R`On 2/10, net 30: pay on day 10 to take the discount, or on day 30 to skip it.` },
          ...
          { kind: 'learn', title: 'Check it with A/P days',
            ...
            viz: { type: 'flow', op: true, steps: [
              { icon: '🧾', t: 'Accounts payable', s: 'owed to suppliers', c: 1 },
              { icon: '📅', t: 'Daily COGS', s: R`yearly COGS \(\div\) 365`, c: 2 },
              { t: 'A/P days', s: 'when you pay', c: 3 }],
              links: [R`\(\div\)`, R`\(=\)`],
              cap: R`Then hold the A/P days up against the terms. Do they land on one of the two good days?` },
            formula: 'ap-days' },
          ...
          { kind: 'learn', title: 'Stretching payables',
            ...
            viz: { type: 'tl', key: true, n: 3, unit: 'Day', labels: { 0: '0', 1: '15', 2: '40: due', 3: '60: late' }, at: { 1: '$99', 2: '$100', 3: '$100' }, hi: [1, 3],
              spans: [{ from: 1, to: 2, label: `On time: ${40 - 15} days`, c: 1 }, { from: 1, to: 3, label: `Stretched: ${60 - 15} days`, c: 3, row: 1 }],
              cap: R`The same 1% now buys 45 days of credit instead of 25, so the yearly cost falls.` } },
          ...
          { kind: 'learn', title: 'The risks of stretching',
            ...
            viz: { type: 'balance', tilt: 'level', left: { icon: '💰', label: 'Cheaper credit', sub: 'a lower EAR', c: 'good' }, right: { icon: '⚠️', label: 'The risks', sub: 'a broken deal', c: 'bad' },
              note: 'Weigh both sides before you stretch.',
              cap: R`Cheap money on one side, an angry supplier on the other. Cost alone does not decide it.` },
      ...
      'w8-L8': {
        ...
        cards: [
          { kind: 'learn', title: 'The other side of trade credit',
            ...
            viz: { type: 'flow', steps: [
              { icon: '🏭', t: 'Supplier', s: 'you pay it later: A/P', c: 'grey' },
              { icon: '🏢', t: 'Your firm', s: 'sells the goods on', c: 1 },
              { icon: '🛍️', t: 'Customers', s: 'they pay you later: A/R', c: 2 }],
              cap: R`Goods pass from the supplier to you to your customers. Their cash comes later, and some never comes.` } },
          { kind: 'learn', title: 'Three ways to watch receivables',
            ...
            viz: { type: 'bars', fmt: '$', bars: [['0–30', 60000, 1], ['31–60', 25000, 1], ['61–90', 10000, 'bad'], ['Over 90', 5000, 'bad']].map(([label, v, c]) => ({ label, v, c })),
              cap: R`The ageing schedule as bars: most money is owed recently. The red bars are the old, risky debts.` },
          ...
          { kind: 'learn', title: 'Judge a credit policy with NPV',
            ...
            viz: { type: 'flow', key: true, steps: [
              { icon: '📅', t: 'Cash flows', s: 'month by month', c: 1 },
              { icon: '🧮', t: 'NPV', s: R`month 0 \(+ \frac{C}{r}\)`, c: 2 },
              { icon: '✅', t: 'Keep the higher NPV', c: 'good' }],
              cap: R`Do the same steps for both policies. The one with the bigger NPV wins.` } },
