...
(function (root) {
  ...
  const CW_TABLE = { head: ['Probability', 'Calm Co', 'Wild Co'], rows: CW.p.map((p, k) => [T.num(p, 2), tp(CW.calm[k]), tp(CW.wild[k])]) };
  // L3: Example 3 worked in a table (expected return 11%)
  const EX3_STEPS = { head: ['Return', 'Probability', 'Squared deviation'], rows: EX3.r.map((r, k) => [T.num(r, 2), T.numT(EX3.p[k], 2), T.numT((r - 0.11) * (r - 0.11), 4)]) };
  ...
  // L1: $100 through the four-year example (10%, −5%, 20%, 15%)
  const R4 = [0.10, -0.05, 0.20, 0.15];
  const R4_PATH = R4.reduce((a, r) => a.concat(a[a.length - 1] * (1 + r)), [100]);
  const sgn = (r) => (r < 0 ? '−' : '+') + T.numT(Math.abs(r * 100), 2) + '%'; // plain-text signed percent: +10%, −5%
  const cash = (v) => (Math.abs(v - Math.round(v)) < 1e-9 ? T.moneyT(Math.round(v)) : T.money(v, 2)); // $110 or $104.50
  // L2: one colour per state of the economy (recession, normal year, boom)
  const ST_C = ['bad', 1, 'good'];
  const stateBars = (rs) => rs.map((r, k) => ({ label: STATES[k], v: P(r), c: ST_C[k] }));
  // L3: Calm Co and Wild Co drawn on the same scale, with their SDs
  const CW_L = ['Bad', 'Normal', 'Good'];
  const cwBars = (rs, c, cap, key) => ({ type: 'bars', title: rs === CW.calm ? 'Calm Co' : 'Wild Co', fmt: '%', min: -12, max: 32, values: false, c, key, cap,
    line: { v: P(FIN.expRet(CW.p, rs)), label: 'E(R)' }, bars: rs.map((r, k) => ({ label: CW_L[k], v: P(r) })) });
  // L4: the US asset classes from HIST_TABLE as [SD, average return] (inflation is not an asset)
  const HIST_PTS = HIST_TABLE.rows.slice(0, 5).map((r) => [parseFloat(r[2]), parseFloat(r[1])]);
  // L5: made-up monthly returns for the pictures only (no axis numbers are drawn). Miners: correlation 0.99; airline and supermarket: 0.
  const MOVE = { minerA: [3, 7, 4, 9, 6, 10, 5, 8], minerB: [2, 6.5, 3.5, 8, 5.5, 9, 4, 7.5], air: [4, 8, 5, 9, 3, 7, 6, 8], shop: [6.5, 5.5, 5, 6.5, 6.5, 7, 5, 6] };
  const overTime = (title, a, b, cap) => ({ type: 'lines', title, cap, x: { label: 'Month', ticks: [] }, y: { label: 'Return', ticks: [] }, endLabels: false,
    series: [{ name: a[0], c: 1, pts: a[1].map((v, k) => [k + 1, v]) }, { name: b[0], c: 2, pts: b[1].map((v, k) => [k + 1, v]) }] });
  // L5: three clouds of made-up points (correlations 0.98, −0.98 and about 0)
  const CLOUD = { pos: [[1, 1.8], [2, 2.4], [3, 3.9], [4, 3.6], [5, 5.4], [6, 5.9], [7, 7.2]], neg: [[1, 7.1], [2, 6.2], [3, 5.6], [4, 3.9], [5, 4.3], [6, 2.4], [7, 1.9]],
    none: [[1, 4.2], [2, 6.8], [3, 2.1], [4, 5.9], [5, 3.1], [6, 6.3], [7, 3.6]] };
  const cloud = (title, pts, c, fit, cap, key) => ({ type: 'scatter', title, cap, key, x: { label: 'Return on i', ticks: [] }, y: { label: 'Return on j', ticks: [] }, sets: [{ c, fit, pts }] });
  // L6: portfolio SD of Cheaters Anonymous (6.32%) and Tricky Dicky (12.42%) as the weight in Tricky Dicky goes from 0 to 100%
  const RHO_SET = [1, 0.9336, 0.5, 0, -0.5, -0.9336];
  const sdCurve = (rho) => {
    const ws = Array.from({ length: 41 }, (_, k) => k / 40);
    if (rho === -1) ws.push(0.0632 / (0.0632 + 0.1242)); // the zero-risk mix
    return ws.sort((a, b) => a - b).map((w) => [P(w), P(FIN.portSD2(1 - w, 0.0632, w, 0.1242, rho))]);
  };
  // L7: one share's variance (0.4²) = systematic part (0.4² × 0.25, what no number of shares removes) + unsystematic part.
  // Split bars take their widths from the values (drawn in percentage points: a split needs values that add up to at least 1).
  const DIV_VAR = { sys: 0.4 ** 2 * 0.25, total: (n) => divSD(n) ** 2 };
  // L8: the SML before and after a change (r_f = 4%, E[R_M] = 10%)
  const smlPts = (rf, prem) => [[0, P(rf)], [2, P(rf + 2 * prem)]];
  ...
  const pack = {
    ...
    lessons: {
      'w9-L1': {
        ...
        goal: R`Work out the return on a share over one year, and over several years.`,
        topics: ['ret'],
        cards: [
          { kind: 'learn', title: 'A return is a percentage gain',
            body: R`You buy something for $100. A year later it is worth $110. You gained $10.\n\nThe **return** is that gain as a percentage of what you paid: \(\frac{10}{100} = 0.10 = 10\%\). Percentages let you compare a small investment with a big one.`,
            viz: [
              { type: 'bars', fmt: '$', cap: R`The orange part is your **gain**: \(\$10\).`,
                bars: [{ label: 'You pay', parts: [{ v: 100, c: 1 }] }, { label: 'A year later', parts: [{ v: 100, c: 1 }, { v: 10, c: 2 }] }],
                keys: [{ c: 1, label: 'What you paid' }, { c: 2, label: 'Your gain' }] },
              { type: 'flow', op: true, cap: R`Gain \(\div\) what you paid = the **return**.`,
                steps: [{ t: R`\(\$10\)`, s: 'gain', c: 2 }, { t: R`\(\$100\)`, s: 'paid', c: 1 }, { t: R`\(10\%\)`, s: 'return', c: 3 }], links: [R`\(\div\)`, R`\(=\)`] },
            ] },
          { kind: 'learn', title: 'Two ways a share pays you',
            body: R`A **share** is a small piece of ownership in a company. It can pay you in two ways.\n\nThe company may pay a **dividend**: cash for each share you own. And the share **price** can rise (or fall) while you hold it.`,
            viz: { type: 'compare', vsText: '+', cap: R`Your return comes from **two** places: cash paid to you, plus the change in the price.`,
              items: [{ icon: '💵', title: 'Dividend', points: ['Cash paid to you for each share you own'], c: 4 },
                { icon: '📈', title: 'Price change', points: ['The share price can rise (or fall) while you hold it'], c: 2 }] } },
          { kind: 'learn', title: 'The realised return',
            body: R`The **realised return** is the return you actually got over one period:\n\n\[R_{t+1} = \frac{DIV_{t+1} + P_{t+1} - P_t}{P_t}\]\n\n\(P_t\) is the price you paid, \(P_{t+1}\) is the price at the end, and \(DIV_{t+1}\) is the dividend paid during the period.`,
            points: [R`**Dividend yield**: \(\frac{DIV_{t+1}}{P_t}\), the cash part.`, R`**Capital gain yield**: \(\frac{P_{t+1} - P_t}{P_t}\), the price part.`, R`Realised return = dividend yield + capital gain yield.`],
            formula: 'realised',
            viz: { type: 'anatomy', key: true, cap: R`On top: the cash part plus the price part. Underneath: the price you **paid**.`,
              tex: R`R_{t+1} = \frac{\colD{DIV_{t+1}} + \colB{P_{t+1} - P_t}}{\colA{P_t}}`,
              parts: [{ sym: R`DIV_{t+1}`, say: 'the dividend: cash paid to you during the period', c: 'D' }, { sym: R`P_{t+1} - P_t`, say: 'the change in the price: the capital gain', c: 'B' }, { sym: 'P_t', say: 'the price you paid at the start', c: 'A' }] } },
          { kind: 'example', title: 'Worked example', q: R`You bought 100 shares of ABC Ltd one year ago at $25.00 each. You received a dividend of 20 cents per share. The share price is now $30.00. What is your realised return?`,
            steps: [R`Dividend yield: \(\frac{0.20}{25} = 0.008 = 0.8\%\).`, R`Capital gain yield: \(\frac{30 - 25}{25} = 0.20 = 20\%\).`, R`Add them: \(R = 0.8\% + 20\% = 20.8\%\). The number of shares does not change the percentage.`],
            answer: R`Your realised return is \(20.8\%\).`,
            ti: [TI.line('(0.20+30-25)/25', PCT(0.208))] },
          { kind: 'check', gen: 'w9-g-realised' },
          { kind: 'learn', title: 'Returns over several years',
            body: R`Returns **compound**, just like interest. To combine yearly returns, multiply the growth factors \((1 + R)\), then subtract 1:\n\n\[HPR = (1+R_1)\cdots(1+R_T) - 1\]\n\nThis is the **holding period return** (HPR): your total return over all \(T\) years. A loss of 5% is a growth factor of \(0.95\).`,
            formula: 'hpr',
            tip: R`To turn an HPR into a return **per year** (the **annualised** return), take the \(T\)-th root: \((1 + HPR)^{1/T} - 1\).`,
            viz: { type: 'bars', fmt: '$', dp: 2, cap: R`Returns of \(10\%\), \(-5\%\), \(20\%\) and \(15\%\) grow \(\$100\) to \(${L.money(R4_PATH[4])}\): an HPR of \(${pc(FIN.hprMulti(R4))}\). Adding them gives \(${pc(sum(R4), 0)}\): wrong.`,
              bars: R4_PATH.map((v, t) => ({ label: t ? `Yr ${t}\n${sgn(R4[t - 1])}` : 'Start', v, note: cash(v), c: t === R4.length ? 3 : 1 })) } },
          { kind: 'example', title: 'Worked example: four years', q: R`An investment returned 10%, −5%, 20% and 15% in years 1 to 4. What is the four-year HPR, and what is the return per year?`,
            table: { head: ['Year', 'Return', 'Growth factor'], rows: [[1, '10%', '1.10'], [2, '−5%', '0.95'], [3, '20%', '1.20'], [4, '15%', '1.15']] },
            steps: [R`Multiply the growth factors: \(1.10 \times 0.95 \times 1.20 \times 1.15 = ${nt(1 + FIN.hprMulti([0.10, -0.05, 0.20, 0.15]), 4)}\).`,
              R`Subtract 1: \(HPR = ${nt(1 + FIN.hprMulti([0.10, -0.05, 0.20, 0.15]), 4)} - 1 = ${pc(FIN.hprMulti([0.10, -0.05, 0.20, 0.15]))}\).`,
              R`Per year, take the 4th root: \((${nt(1 + FIN.hprMulti([0.10, -0.05, 0.20, 0.15]), 4)})^{1/4} - 1 = ${pc(FIN.annualise(0.4421, 4))}\).`],
            answer: R`The HPR is \(${pc(FIN.hprMulti([0.10, -0.05, 0.20, 0.15]))}\), which is \(${pc(FIN.annualise(0.4421, 4))}\) a year.`,
            ti: [TI.line('1.10*0.95*1.20*1.15-1', { note: R`The HPR: \(${pc(FIN.hprMulti([0.10, -0.05, 0.20, 0.15]))}\).` }), TI.line('(1+ans)^(1/4)-1', PCT(FIN.annualise(0.4421, 4)))] },
          { kind: 'guided', title: 'Your turn', q: R`A share returned 12%, −8% and 6% in years 1 to 3. Find its three-year HPR, then its annualised return.`,
            parts: [
              { ask: R`What is the growth factor for the year with −8%?`, answer: 0.92, unit: '', dp: 2, hint: R`A loss of 8% leaves \(1 - 0.08\) of your money.`, why: R`\(1 - 0.08 = 0.92\).` },
              { ask: 'What is the three-year HPR?', answer: P(G1_HPR), unit: '%', dp: 2, hint: R`Multiply \(1.12 \times 0.92 \times 1.06\), then subtract 1.`, why: R`\((1.12)(0.92)(1.06) - 1 = ${nt(G1_HPR, 6)} = ${pc(G1_HPR)}\).`,
                mistakes: [{ v: 10, why: 'Adding the returns (12 − 8 + 6) ignores compounding. Multiply the growth factors.' }] },
              { ask: 'What is the annualised return (per year)?', answer: P(G1_ANN), unit: '%', dp: 2, hint: R`Take the cube root: \((${nt(1 + G1_HPR, 6)})^{1/3} - 1\).`, why: R`\((${nt(1 + G1_HPR, 6)})^{1/3} - 1 = ${pc(G1_ANN)}\) a year.`,
                mistakes: [{ v: P(G1_HPR / 3), why: 'Dividing the HPR by 3 ignores compounding. Take the cube root.' }] },
            ],
            answer: R`The HPR is \(${pc(G1_HPR)}\), which is \(${pc(G1_ANN)}\) a year.`,
            ti: [TI.line(`${G1.map((x) => tn(1 + x)).join('*')}-1`, { note: R`The HPR: \(${pc(G1_HPR)}\).` }), TI.line('(1+ans)^(1/3)-1', PCT(G1_ANN))] },
          { kind: 'check', ref: 'w9-q06' },
          { kind: 'recap', title: 'Remember', points: [
            R`Return = gain \(\div\) what you paid, as a percentage.`,
            R`Realised return = dividend yield + capital gain yield \(= \frac{DIV_1 + P_1 - P_0}{P_0}\).`,
            R`Several years: multiply the \((1 + R)\) factors, then subtract 1. Per year: \((1 + HPR)^{1/T} - 1\).`,
            R`Exam trap: returns compound, so never just add them. And always divide by the price you **paid**.`,
          ], formula: 'realised' },
        ],
      },
      'w9-L2': {
        title: 'Risk and expected return',
        goal: R`Explain what risk is, and work out an expected return from a table of probabilities.`,
        topics: ['prob'],
        cards: [
          { kind: 'learn', title: 'Risk: the return could be different',
            body: R`A bank deposit at 4% pays 4%. A share might return 20%, or it might lose 10%. You do not know in advance.\n\n**Risk** is this uncertainty: the return you get could be different from the return you expect.`,
            viz: { type: 'numline', min: -15, max: 25, fmt: '%', ticks: [-10, 0, 10, 20], cap: R`The bank pays exactly \(4\%\). The share could land anywhere from \(-10\%\) to \(20\%\): that is **risk**.`,
              zones: [{ from: -10, to: 20, c: 2, label: 'The share' }], marks: [{ v: 4, label: 'Bank: 4%', c: 1, below: true }] } },
          { kind: 'learn', title: 'States of the economy',
            body: R`Analysts often list a few possible **states** of the economy, with a **probability** (a chance between 0 and 1) for each. The probabilities add up to 1.\n\nHere is Mr Henry’s forecast for Highbull shares.`,
            points: STATES.map((s, k) => R`**${s === 'Normal' ? 'Normal year' : s}**: probability ${T.num(HB.p[k], 2)}, return ${tp(HB.h[k], 1)}.`),
            tip: R`\(0.25 + 0.60 + 0.15 = 1\). If the probabilities do not add up to 1, check the table.`,
            viz: [
              { type: 'grid100', cap: R`The chances fill all 100 squares: \(0.25 + 0.60 + 0.15 = 1\).`,
                parts: HB.p.map((p, k) => ({ n: Math.round(p * 100), c: ST_C[k], label: `${STATES[k]}: ${T.num(p, 2)}` })) },
              { type: 'bars', fmt: '%', cap: 'Highbull’s return in each state.', bars: stateBars(HB.h) },
            ] },
          { kind: 'learn', title: 'The expected return',
            body: R`The **expected return**, \(E(R)\), is a **probability-weighted average**. Multiply each return by its probability, then add:\n\n\[E(R) = \sum_k P_k R_k\]\n\nThat is \(P_1R_1 + P_2R_2 + \cdots\). The sign \(\sum\) (sigma) just means “add them all up”.`,
            formula: 'exp-ret',
            viz: { type: 'waterfall', fmt: '%', dp: 2, key: true, cap: R`Highbull: each return times its probability, then add them up. \(E(R) = ${pc(eH)}\).`,
              steps: HB.p.map((p, k) => ({ label: STATES[k], v: P(p * HB.h[k]), c: ST_C[k] })).concat([{ label: 'E(R)', total: true, c: 3 }]) } },
          { kind: 'example', title: 'Worked example: Highbull', q: R`Use Mr Henry’s forecast: recession 0.25 and −2%, normal 0.60 and 9.2%, boom 0.15 and 15.4%. What is Highbull’s expected return?`,
            steps: [R`Write the returns as decimals: \(-0.02\), \(0.092\) and \(0.154\).`,
              R`Multiply each by its probability: \(0.25 \times (-0.02) = -0.005\), \(0.60 \times 0.092 = 0.0552\) and \(0.15 \times 0.154 = 0.0231\).`,
              R`Add: \(-0.005 + 0.0552 + 0.0231 = ${nt(eH, 4)} = ${pc(eH)}\).`],
            answer: R`\(E(R) = ${pc(eH)}\).`,
            ti: [sto(HB.p, 'p', { note: 'The probabilities.' }), sto(HB.h, 'r', { note: 'The returns, as decimals.' }), TI.line('sum(p*r)', PCT(eH))] },
          { kind: 'ti', title: 'Lists do the multiplying',
            body: R`Store the probabilities in a list called \(p\) and the returns in a list called \(r\). Type **→** with **ctrl var**.\n\nThen \(p \times r\) multiplies the two lists item by item, and \(\text{sum}\) adds the results. Here are the two parts on their own.`,
            ti: [TI.line(`${TI.list(HB.p)}*${TI.list(HB.h)}`, { note: 'Each probability times its return.' }), TI.line('sum(ans)', { note: R`Add them: \(E(R) = ${nt(eH, 4)}\).` })] },
          { kind: 'check', gen: 'w9-g-exp-prob' },
          { kind: 'learn', title: 'Expected does not mean likely',
            body: R`Highbull never returns exactly 7.33%. It returns −2%, 9.2% or 15.4%.\n\nThe expected return is the average you would get if the same year could be repeated many times. The gap between what happens and what you expect is the risk.`,
            viz: { type: 'bars', fmt: '%', cap: R`No bar stops at the dashed line. The gaps between the bars and the line are the risk.`,
              bars: stateBars(HB.h), line: { v: P(eH), label: `E(R) = ${tp(eH)}` } } },
          { kind: 'guided', title: 'Your turn: Slowbear', q: R`Mr Henry also forecasts Slowbear shares: 5% in a recession (probability 0.25), 6.2% in a normal year (0.60) and 7.4% in a boom (0.15). Find the expected return.`,
            table: { head: ['Probability', 'Return'], rows: HB.p.map((p, k) => [T.num(p, 2), tp(HB.s[k], 1)]) },
            parts: [
              { ask: R`Recession: what is \(0.25 \times 0.05\)?`, answer: 0.25 * 0.05, unit: '', dp: 4, hint: 'Multiply the probability by the return written as a decimal.', why: R`\(0.25 \times 0.05 = 0.0125\).` },
              { ask: R`Normal: what is \(0.60 \times 0.062\)?`, answer: 0.6 * 0.062, unit: '', dp: 4, hint: R`\(6.2\% = 0.062\).`, why: R`\(0.60 \times 0.062 = 0.0372\).` },
              { ask: R`Boom: what is \(0.15 \times 0.074\)?`, answer: 0.15 * 0.074, unit: '', dp: 4, hint: R`\(7.4\% = 0.074\).`, why: R`\(0.15 \times 0.074 = 0.0111\).` },
              { ask: R`Add the three. What is \(E(R)\), as a percentage?`, answer: P(eS), unit: '%', dp: 2, hint: R`\(0.0125 + 0.0372 + 0.0111\), then multiply by 100.`, why: R`\(0.0125 + 0.0372 + 0.0111 = 0.0608 = 6.08\%\).`,
                mistakes: [{ v: 6.2, why: 'That is the most likely return (and here also the simple average). Weight each return by its probability.' }] },
            ],
            answer: R`Slowbear’s expected return is \(${pc(eS)}\): lower than Highbull’s \(${pc(eH)}\), but much steadier.`,
            ti: [sto(HB.p, 'p'), sto(HB.s, 'r', { note: 'Slowbear’s returns.' }), TI.line('sum(p*r)', PCT(eS))] },
          { kind: 'check', ref: 'w9-q09' },
          { kind: 'recap', title: 'Remember', points: [
            R`**Risk**: the return you get could be different from the return you expect.`,
            R`Expected return: \(E(R) = \sum P_k R_k\), a probability-weighted average.`,
            R`On the TI-Nspire: \(\{\ldots\} \to p\), \(\{\ldots\} \to r\), then \(\text{sum}(p \times r)\).`,
            R`Exam trap: do not take a simple average, and do not divide by the number of states. The probabilities already do the averaging.`,
          ], formula: 'exp-ret' },
        ],
      },
      'w9-L3': {
        title: 'Measuring risk: variance and standard deviation',
        goal: R`Measure how spread out returns are, from probabilities and from past data.`,
        topics: ['prob', 'hist'],
        cards: [
          { kind: 'learn', title: 'Same average, different risk',
            body: R`Calm Co and Wild Co both have an expected return of 10%. But Calm Co stays between 8% and 12%, while Wild Co swings from −10% to 30%.\n\nWild Co is riskier. To measure risk, look at each return’s **deviation**: how far it is from the expected return, \(R_k - E(R)\).`,
            table: CW_TABLE,
            viz: [cwBars(CW.calm, 1, R`Calm Co’s returns (\(8\%\) to \(12\%\)) stay close to the \(10\%\) line: small deviations.`, true),
              cwBars(CW.wild, 2, R`Wild Co’s returns (\(-10\%\) to \(30\%\)) land far above and below it: big deviations.`)] },
          { kind: 'learn', title: 'Variance and standard deviation',
            body: R`Square each deviation, so that the minus signs do not cancel out. Then take the probability-weighted average. That is the **variance**:\n\n\[\sigma^2 = \sum_k P_k\left[R_k - E(R)\right]^2\]\n\nThe **standard deviation** (SD) is its square root, \(\sigma = \sqrt{\sigma^2}\). It is back in percent, so it is easier to read.`,
            formula: 'var-prob',
            tip: R`\(\sigma\) is the Greek letter sigma. Calm Co’s SD is \(${pc(FIN.sdProb(CW.p, CW.calm))}\). Wild Co’s is \(${pc(FIN.sdProb(CW.p, CW.wild))}\).`,
            viz: [
              { type: 'flow', dir: 'col', cap: R`Square each deviation, weight by the probabilities and add, then take the square root.`,
                steps: [{ t: R`\(R_k - E(R)\)`, s: 'each deviation' }, { t: R`\(\sigma^2\)`, s: 'the variance', c: 2 }, { t: R`\(\sigma\)`, s: 'the standard deviation', c: 3 }],
                links: [R`square, \(\times P_k\), add`, R`\(\sqrt{\phantom{x}}\)`] },
              { type: 'bars', fmt: '%', dp: 2, cap: R`Wild Co’s SD is 10 times Calm Co’s: much more risk.`,
                bars: [{ label: 'Calm Co', v: P(FIN.sdProb(CW.p, CW.calm)), c: 1 }, { label: 'Wild Co', v: P(FIN.sdProb(CW.p, CW.wild)), c: 2 }] },
            ] },
          { kind: 'example', title: 'Worked example, step by step', q: R`Returns of 9%, 10%, 11%, 12% and 13% have probabilities 0.1, 0.2, 0.4, 0.2 and 0.1. The expected return is 11%. What is the standard deviation?`,
            table: EX3_STEPS,
            steps: [R`Deviations from \(0.11\): \(-0.02, -0.01, 0, 0.01, 0.02\).`, R`Square them (the last column): \(0.0004, 0.0001, 0, 0.0001, 0.0004\).`,
              R`Multiply each by its probability and add: \(0.1 \times 0.0004 + 0.2 \times 0.0001 + 0.4 \times 0 + 0.2 \times 0.0001 + 0.1 \times 0.0004 = ${nt(FIN.varProb(EX3.p, EX3.r), 5)}\). This is the variance.`,
              R`\(\sigma = \sqrt{${nt(FIN.varProb(EX3.p, EX3.r), 5)}} = ${nt(FIN.sdProb(EX3.p, EX3.r), 6)} = ${pc(FIN.sdProb(EX3.p, EX3.r))}\).`],
            answer: R`The standard deviation is \(${pc(FIN.sdProb(EX3.p, EX3.r))}\).`,
            ti: [sto(EX3.p, 'p'), sto(EX3.r, 'r'), TI.line('sqrt(sum(p*(r-0.11)^2))', PCT(FIN.sdProb(EX3.p, EX3.r)))] },
          { kind: 'ti', title: R`When you do not know \(E(R)\) yet`,
            body: R`Usually you must find \(E(R)\) first. Store it in a letter, \(m\), and use the letter in the next line. Type \(\text{sqrt}\) with the letter keys.\n\nHere is Highbull again: probabilities 0.25, 0.60 and 0.15; returns −2%, 9.2% and 15.4%.`,
            ti: tiSdProb(HB.p, HB.h) },
          { kind: 'check', gen: 'w9-g-sd-prob' },
          { kind: 'learn', title: 'Risk from past data',
            body: R`Sometimes there are no probabilities, just a list of past returns. This is a **sample**.\n\nThe average return is the **mean**: \(\bar{R} = \frac{R_1 + \cdots + R_T}{T}\). For the variance, add the squared deviations from the mean and divide by \(T - 1\), not by \(T\):\n\n\[Var(R) = \frac{1}{T-1}\sum_t (R_t - \bar{R})^2\]`,
            formula: 'var-sample',
            tip: R`\(T\) is the number of past returns. Divide by \(T - 1\) only for a sample. With probabilities you never divide: the probabilities do the averaging.`,
            viz: { type: 'compare', cap: R`The same idea, two recipes. Divide by \(T - 1\) only when you have past data.`,
              items: [{ icon: '🎲', title: 'Probabilities', big: R`\(\times P_k\)`, points: ['The probabilities do the averaging', R`Never divide by \(T - 1\)`], c: 1 },
                { icon: '📜', title: 'Past data (a sample)', big: R`\(\div (T - 1)\)`, points: [R`Deviations from the mean \(\bar{R}\)`, R`Add the squares, then divide by \(T - 1\)`], c: 2 }] } },
          { kind: 'example', title: 'Worked example: past data', q: R`Asset A returned 9%, 10%, 11%, 12% and 13% over five years. Treat this as a sample. What is the standard deviation?`,
            steps: [R`Mean: \(\bar{R} = \frac{0.09 + 0.10 + 0.11 + 0.12 + 0.13}{5} = 0.11\).`, R`Squared deviations: \(0.0004, 0.0001, 0, 0.0001, 0.0004\). They add up to \(0.001\).`,
              R`Divide by \(T - 1 = 4\): \(Var(R) = \frac{0.001}{4} = ${nt(FIN.varS(EX3.r), 5)}\).`, R`\(SD = \sqrt{${nt(FIN.varS(EX3.r), 5)}} = ${nt(FIN.sdS(EX3.r), 4)} = ${pc(FIN.sdS(EX3.r))}\).`],
            answer: R`The SD is \(${pc(FIN.sdS(EX3.r))}\). With the probabilities it was only \(${pc(FIN.sdProb(EX3.p, EX3.r))}\), because the middle return had 40% of the weight.`,
            ti: [sto(EX3.r, 'r'), TI.line('mean(r)', { note: 'The mean: 11%.' }), TI.line('stDevSamp(r)', Object.assign(PCT(FIN.sdS(EX3.r)), { note: R`\(\text{stDevSamp}\) divides by \(T - 1\) for you. That is \(${pc(FIN.sdS(EX3.r))}\).` }))] },
          { kind: 'guided', title: 'Your turn, step by step', q: R`A share returned 15%, −5%, 10% and 20% over four years (a sample). Find the standard deviation.`,
            parts: [
              { ask: 'What is the mean return?', answer: P(FIN.mean(G3)), unit: '%', dp: 2, hint: 'Add the four returns and divide by 4.', why: R`\(\frac{0.15 - 0.05 + 0.10 + 0.20}{4} = 0.10 = 10\%\).` },
              { ask: 'What is the sum of the squared deviations from the mean? (4 decimal places)', answer: G3_SS, unit: '', dp: 4, hint: R`The deviations are \(0.05, -0.15, 0\) and \(0.10\). Square each one, then add.`, why: R`\(0.0025 + 0.0225 + 0 + 0.01 = ${nt(G3_SS, 4)}\).` },
              { ask: R`Divide by \(T - 1\). What is the variance? (6 decimal places)`, answer: FIN.varS(G3), unit: '', dp: 6, hint: R`\(T = 4\), so divide by 3.`, why: R`\(\frac{${nt(G3_SS, 4)}}{3} = ${nt(FIN.varS(G3), 6)}\).`,
                mistakes: [{ v: G3_SS / 4, why: R`That divides by \(T = 4\). For a sample divide by \(T - 1 = 3\).` }] },
              { ask: 'Take the square root. What is the SD?', answer: P(FIN.sdS(G3)), unit: '%', dp: 2, hint: R`\(\sqrt{${nt(FIN.varS(G3), 6)}}\), then multiply by 100.`, why: R`\(\sqrt{${nt(FIN.varS(G3), 6)}} = ${nt(FIN.sdS(G3), 4)} = ${pc(FIN.sdS(G3))}\).`,
                mistakes: [{ v: P(Math.sqrt(FIN.varP(G3))), why: R`That divides by \(T\). For a sample divide by \(T - 1\).` }, { v: P(FIN.varS(G3)), why: 'That is the variance. Take the square root.' }] },
            ],
            answer: R`The SD is \(${pc(FIN.sdS(G3))}\).`,
            ti: [sto(G3, 'r'), TI.line('stDevSamp(r)', PCT(FIN.sdS(G3)))] },
          { kind: 'check', gen: 'w9-g-sd-hist' },
          { kind: 'check', ref: 'w9-q15' },
          { kind: 'recap', title: 'Remember', points: [
            R`Risk = how far returns spread around the expected return. Measure it with the variance \(\sigma^2\) or the SD \(\sigma\).`,
            R`Probabilities: \(\sigma^2 = \sum P_k[R_k - E(R)]^2\). TI-Nspire: \(\text{sqrt}(\text{sum}(p \times (r - m)^2))\).`,
            R`Past data (a sample): divide by \(T - 1\). TI-Nspire: \(\text{stDevSamp}(r)\).`,
            R`Exam trap: the SD is the square root of the variance, so do not stop at the variance. And never divide by \(T - 1\) when you have probabilities.`,
          ], formula: 'var-prob' },
        ],
      },
      'w9-L4': {
        title: 'Risk per unit of return: CV and the Sharpe ratio',
        goal: R`Explain the risk–return trade-off, and compare investments with the CV and the Sharpe ratio.`,
        topics: ['trade', 'cv'],
        cards: [
          { kind: 'learn', title: 'How investors feel about risk',
            body: R`Investors can be sorted by how they feel about risk.`,
            points: [R`**Risk averse**: wants the most return for the least risk.`, R`**Risk neutral**: wants the most return and does not care about risk.`, R`**Risk seeking**: wants more return and more risk.`],
            tip: R`In finance we assume investors are **risk averse**. They take on more risk only if they expect more return.`,
            viz: { w: 360, type: 'lines', keys: [], cap: R`For more risk, a **risk-averse** investor wants more return (blue). Finance assumes this investor.`,
              x: { label: 'Risk', ticks: [] }, y: { label: 'Return they want', hide: true },
              series: [{ name: 'Risk averse', c: 1, pts: [[0, 5], [10, 12]] }, { name: 'Risk neutral', c: 'grey', dash: true, pts: [[0, 5], [10, 5]] }, { name: 'Risk seeking', c: 2, dash: true, pts: [[0, 5], [10, 1]] }] } },
          { kind: 'learn', title: 'The risk–return trade-off',
            body: R`History agrees. In the US from 1925 to 2011, shares had much bigger SDs than bonds and Treasury bills, and they also earned much higher average returns.\n\nThis is the **risk–return trade-off**: across **asset classes** (groups of similar investments), more risk has come with more return.`,
            table: HIST_TABLE,
            tip: R`This holds for asset classes and big portfolios. For **individual shares** there is no clear link. The diversification lesson explains why.`,
            viz: { w: 360, type: 'scatter', title: 'US asset classes, 1925–2011', cap: R`Each dot is an asset class. The riskier ones (further right) earned more (higher up).`,
              x: { label: 'Risk (SD)', fmt: '%', min: 0, max: 35 }, y: { label: 'Average return', fmt: '%', min: 0, max: 18 },
              sets: [{ c: 1, fit: true, pts: HIST_PTS }],
              marks: [{ x: HIST_PTS[4][0], y: HIST_PTS[4][1], label: 'T-bills', c: 1, pos: 'right' }, { x: HIST_PTS[3][0], y: HIST_PTS[3][1], label: 'Bonds', c: 1, pos: 'right' },
                { x: HIST_PTS[1][0], y: HIST_PTS[1][1], label: 'Large shares', c: 1, pos: 'left' }, { x: HIST_PTS[0][0], y: HIST_PTS[0][1], label: 'Small shares', c: 1, pos: 'left' }] } },
          { kind: 'check', ref: 'w9-q22' },
          { kind: 'learn', title: 'The coefficient of variation',
            body: R`What if one share has a higher return **and** a higher risk? Compare the risk you take for each unit of return.\n\nThe **coefficient of variation** is \(CV = \frac{\sigma}{E(R)}\). A risk-averse investor picks the **lowest** CV.`,
            formula: 'cv',
            tip: R`Same return? Pick the lower SD. Same SD? Pick the higher return. You need the CV when both are different.`,
            viz: { type: 'compare', cap: R`Beta has more return **and** more risk than Gamma. Its CV is lower: less risk per unit of return.`,
              items: [{ title: 'Asset Beta', big: R`\(CV = \frac{20.0}{17.4} = ${nt(20 / 17.4, 2)}\)`, points: [R`Return \(17.4\%\)`, R`SD \(20.0\%\)`], mark: 'good', markText: 'Lower CV: pick it', c: 1 },
                { title: 'Asset Gamma', big: R`\(CV = \frac{18.8}{13.8} = ${nt(18.8 / 13.8, 2)}\)`, points: [R`Return \(13.8\%\)`, R`SD \(18.8\%\)`], mark: 'bad', markText: 'Higher CV', c: 2 }] } },
          { kind: 'example', title: 'Worked example', q: R`Four assets have the expected returns and risks below. Which one should a risk-averse investor prefer?`, table: EX5_TABLE,
            steps: [R`\(CV_{Alpha} = \frac{12.3}{17.6} = ${nt(12.3 / 17.6, 4)}\) and \(CV_{Beta} = \frac{20.0}{17.4} = ${nt(20 / 17.4, 4)}\).`,
              R`\(CV_{Gamma} = \frac{18.8}{13.8} = ${nt(18.8 / 13.8, 4)}\) and \(CV_{Delta} = \frac{13.4}{1.7} = ${nt(13.4 / 1.7, 4)}\).`,
              'The lowest CV is Alpha’s.'],
            answer: R`Alpha: the least risk per unit of return (\(CV = ${nt(12.3 / 17.6, 4)}\)).`,
            ti: [TI.line('12.3/17.6', { note: 'Alpha’s CV. You can use the percentages as they are: the % signs cancel out.' })] },
          { kind: 'check', gen: 'w9-g-cv' },
          { kind: 'learn', title: 'The risk-free rate and the excess return',
            body: R`Some investments have almost no risk, such as a short-term government loan called a **Treasury bill**. Their return is the **risk-free rate**, \(r_f\).\n\nAny return above \(r_f\) is your reward for taking risk. It is called the **excess return**: \(E[R] - r_f\).`,
            viz: { type: 'bars', fmt: '%', cap: R`Say T-bills pay \(${pc(G4.rf, 0)}\) and portfolio P is expected to earn \(${pc(G4.eP, 0)}\). The orange \(${pc(G4.eP - G4.rf, 0)}\) is its excess return.`,
              bars: [{ label: 'T-bill', parts: [{ v: P(G4.rf), c: 1 }] }, { label: 'Portfolio P', parts: [{ v: P(G4.rf), c: 1 }, { v: P(G4.eP - G4.rf), c: 2 }] }],
              keys: [{ c: 1, label: R`Risk-free rate \(r_f\)` }, { c: 2, label: 'Excess return: the reward for risk' }] } },
          { kind: 'learn', title: 'The Sharpe ratio',
            body: R`The **Sharpe ratio** measures the excess return for each unit of risk:\n\n\[\text{Sharpe} = \frac{E[R_P] - r_f}{\sigma_P}\]\n\n\(P\) stands for a **portfolio** (a mix of investments), but it works for a single share too. A **higher** Sharpe ratio is better.`,
            formula: 'sharpe',
            tip: R`CV: lower is better. Sharpe: higher is better. Do not mix them up.`,
            viz: { w: 360, type: 'lines', key: true, endLabels: false, cap: R`The Sharpe ratio is the **slope** from \(r_f\). Q’s line is steeper: more reward per unit of risk.`,
              x: { label: 'Risk (SD)', fmt: '%', min: 0, max: 18 }, y: { label: 'Expected return', fmt: '%', min: 0, max: 14 },
              series: [{ name: R`P: Sharpe \(${nt(FIN.sharpe(G4.eP, G4.rf, G4.sP), 2)}\)`, c: 2, pts: [[0, P(G4.rf)], [P(G4.sP), P(G4.eP)]] }, { name: R`Q: Sharpe \(${nt(FIN.sharpe(G4.eQ, G4.rf, G4.sQ), 2)}\)`, c: 1, pts: [[0, P(G4.rf)], [P(G4.sQ), P(G4.eQ)]] }],
              marks: [{ x: P(G4.sP), y: P(G4.eP), label: 'P', c: 2 }, { x: P(G4.sQ), y: P(G4.eQ), label: 'Q', c: 1 }, { x: 0, y: P(G4.rf), label: 'r_f', c: 'grey', pos: 'right' }] } },
          { kind: 'example', title: 'Worked example', q: R`Portfolio C has an expected return of 9.44% and an SD of 8.08%. The risk-free rate is 8%. What is its Sharpe ratio?`,
            steps: [R`Excess return: \(9.44\% - 8\% = 1.44\%\).`, R`\(\text{Sharpe} = \frac{0.0144}{0.0808} = ${nt(FIN.sharpe(0.0944, 0.08, 0.0808), 4)}\).`],
            answer: R`Sharpe \(= ${nt(FIN.sharpe(0.0944, 0.08, 0.0808), 4)}\): about 0.18 of excess return for each unit of risk.`,
            ti: [TI.line('(0.0944-0.08)/0.0808', { note: 'The brackets make the calculator subtract first.' })] },
          { kind: 'guided', title: 'Your turn: which is better?', q: R`Portfolio P has an expected return of 12% and an SD of 16%. Portfolio Q has an expected return of 10% and an SD of 10%. The risk-free rate is 4%.`,
            parts: [
              { ask: R`What is P’s excess return, \(E[R_P] - r_f\)?`, answer: P(G4.eP - G4.rf), unit: '%', dp: 2, hint: R`\(12\% - 4\%\).`, why: R`\(12\% - 4\% = 8\%\).` },
              { ask: 'What is P’s Sharpe ratio?', answer: FIN.sharpe(G4.eP, G4.rf, G4.sP), unit: '', dp: 2, hint: R`\(\frac{0.08}{0.16}\).`, why: R`\(\frac{0.12 - 0.04}{0.16} = ${nt(FIN.sharpe(G4.eP, G4.rf, G4.sP), 2)}\).`,
                mistakes: [{ v: G4.eP / G4.sP, why: R`Subtract \(r_f\) first.` }, { v: G4.sP / (G4.eP - G4.rf), why: 'That is upside down. The excess return goes on top.' }] },
              { ask: 'What is Q’s Sharpe ratio?', answer: FIN.sharpe(G4.eQ, G4.rf, G4.sQ), unit: '', dp: 2, hint: R`\(\frac{0.10 - 0.04}{0.10}\).`, why: R`\(\frac{0.06}{0.10} = ${nt(FIN.sharpe(G4.eQ, G4.rf, G4.sQ), 2)}\).`,
                mistakes: [{ v: G4.eQ / G4.sQ, why: R`Subtract \(r_f\) first.` }] },
              { ask: 'Which portfolio gives more reward for each unit of risk?', choices: ['Q: it has the higher Sharpe ratio', 'P: it has the higher expected return', 'P: it has the higher SD', 'They are equally good'], answer: 0, hint: 'A higher Sharpe ratio is better.', why: R`\(0.6 > 0.5\), so Q wins, even though P returns more.` },
            ],
            answer: R`Q is better: \(0.6\) of excess return per unit of risk, against \(0.5\) for P.`,
            ti: [TI.line('(0.12-0.04)/0.16', { note: 'P’s Sharpe ratio.' }), TI.line('(0.10-0.04)/0.10', { note: 'Q’s Sharpe ratio: higher, so Q is better.' })] },
          { kind: 'check', gen: 'w9-g-sharpe' },
          { kind: 'recap', title: 'Remember', points: [
            R`Investors are **risk averse**: more risk must be paid for with more expected return.`,
            R`\(CV = \frac{\sigma}{E(R)}\): risk per unit of return. The **lowest** CV wins.`,
            R`\(\text{Sharpe} = \frac{E[R_P] - r_f}{\sigma_P}\): excess return per unit of risk. The **highest** Sharpe ratio wins.`,
            R`Exam trap: the CV puts risk on top; the Sharpe ratio puts the excess return on top. Subtract \(r_f\) first.`,
          ], formula: 'cv',
          viz: { type: 'compare', cap: 'Two ratios with opposite rules. Do not mix them up.',
            items: [{ icon: '📏', title: 'Coefficient of variation', big: R`\(CV = \frac{\sigma}{E(R)}\)`, points: ['Risk on top', '**Lowest** wins'], c: 2 },
              { icon: '🏆', title: 'Sharpe ratio', big: R`\(\frac{E[R_P] - r_f}{\sigma_P}\)`, points: ['Excess return on top', '**Highest** wins'], c: 1 }] } },
        ],
      },
      'w9-L5': {
        title: 'Covariance and correlation',
        goal: R`Measure whether two shares move together, with the covariance and the correlation.`,
        topics: ['corr'],
        cards: [
          { kind: 'learn', title: 'Do they move together?',
            body: R`Some shares rise and fall together, like two mining companies. Others do not, like an airline and a supermarket.\n\nTo build good portfolios (the next lesson), you need to measure this. The **covariance** and the **correlation** do it.`,
            viz: [overTime('Two mining companies', ['Miner A', MOVE.minerA], ['Miner B', MOVE.minerB], 'They rise and fall together.'),
              overTime('An airline and a supermarket', ['Airline', MOVE.air], ['Supermarket', MOVE.shop], 'No pattern: one moving tells you nothing about the other.')] },
          { kind: 'learn', title: 'Covariance',
            body: R`In each state, multiply the two shares’ deviations from their expected returns. Weight by the probability, then add. For shares \(i\) and \(j\), \(Cov(R_i,R_j)\) is\n\n\[\sum_n P_n\,(R_{i,n} - E(R_i))(R_{j,n} - E(R_j))\]\n\nIf both shares tend to be above (or below) their averages at the same time, the products are mostly positive.`,
            points: [R`**Positive** covariance: they tend to move **together**.`, R`**Negative** covariance: they tend to move in **opposite** directions.`, R`Close to **zero**: no clear link.`],
            formula: 'cov-prob',
            viz: [cloud('Positive', CLOUD.pos, 1, true, 'When i is high, j is high: they move together.', true),
              cloud('Negative', CLOUD.neg, 2, true, 'When i is high, j is low: they move opposite.'),
              cloud('Close to zero', CLOUD.none, 'grey', false, 'No pattern: no clear link.')] },
          { kind: 'example', title: 'Worked example: Highbull and Slowbear', q: R`Use Mr Henry’s forecasts (recession, normal year, boom). \(E(R_H) = 7.33\%\) and \(E(R_S) = 6.08\%\). What is the covariance between the two shares?`,
            table: { head: ['Probability', 'Highbull', 'Slowbear'], rows: HB.p.map((p, k) => [T.num(p, 2), tp(HB.h[k], 1), tp(HB.s[k], 1)]) },
            steps: HB.p.map((p, k) => R`${STATES[k]}: \(${L.num(p, 2)} \times (${nt(HB.h[k])} - 0.0733)(${nt(HB.s[k])} - 0.0608) = ${L.num(p, 2)} \times (${nt(HB.h[k] - eH, 4)})(${nt(HB.s[k] - eS, 4)}) = ${nt(p * (HB.h[k] - eH) * (HB.s[k] - eS), 6)}\).`)
              .concat([R`Add: \(Cov(R_H,R_S) = ${nt(covHS, 6)}\). It is positive: the two shares move together.`]),
            answer: R`\(Cov(R_H,R_S) = ${nt(covHS, 6)}\).`,
            ti: [sto(HB.p, 'p'), sto(HB.h, 'x', { note: 'Highbull’s returns.' }), sto(HB.s, 'y', { note: 'Slowbear’s returns.' }), TI.line('sum(p*(x-0.0733)*(y-0.0608))', { note: 'Each probability times both deviations, then add.' })] },
          { kind: 'learn', title: 'Correlation: covariance on a scale',
            body: R`Is a covariance of 0.000425 big or small? It is hard to tell. Divide it by both standard deviations to get the **correlation**:\n\n\[\rho_{ij} = \frac{Cov(R_i,R_j)}{\sigma_i\,\sigma_j}\]\n\nThe correlation \(\rho\) (rho) always lies between \(-1\) and \(+1\). Turned around: \(Cov(R_i,R_j) = \rho_{ij}\,\sigma_i\,\sigma_j\).`,
            points: [R`\(\rho = +1\): they move perfectly together.`, R`\(\rho = 0\): no link.`, R`\(\rho = -1\): they move perfectly opposite.`],
            formula: 'corr',
            viz: { type: 'numline', min: -1, max: 1, ticks: [-1, -0.5, 0, 0.5, 1], cap: R`Divided by both SDs, the covariance \(0.000425\) becomes \(\rho = ${nt(covHS / (sdH * sdSB), 2)}\): almost perfectly together.`,
              zones: [{ from: -1, to: 0, c: 2, label: 'Opposite' }, { from: 0, to: 1, c: 1, label: 'Together' }], marks: [{ v: covHS / (sdH * sdSB), label: `ρ = ${T.numT(covHS / (sdH * sdSB), 2)}`, c: 3 }] } },
          { kind: 'example', title: 'Worked example', q: R`For Highbull and Slowbear: \(Cov = 0.000425\), \(\sigma_H = 5.80\%\) and \(\sigma_S = 0.749\%\). What is the correlation?`,
            steps: [R`Write the SDs as decimals: \(0.058\) and \(0.00749\).`, R`\(\rho = \frac{0.000425}{0.058 \times 0.00749} = \frac{0.000425}{${nt(0.058 * 0.00749, 8)}} = ${nt(0.000425 / (0.058 * 0.00749), 4)}\).`],
            answer: R`\(\rho \approx ${nt(0.000425 / (0.058 * 0.00749), 3)}\): close to \(+1\), so the two shares move almost perfectly together.`,
            ti: [TI.line('0.000425/(0.058*0.00749)')] },
          { kind: 'check', gen: 'w9-g-corr' },
          { kind: 'learn', title: R`Past data: divide by \(N - 1\)`,
            body: R`With a sample of past returns, use the means and divide by \(N - 1\), just like the sample variance. \(Cov(R_i,R_j)\) is\n\n\[\frac{1}{N-1}\sum_{n}(R_{i,n} - \bar{R}_i)(R_{j,n} - \bar{R}_j)\]\n\nFor the correlation, divide by the **sample** SDs. Then all three parts use \(N - 1\).`,
            formula: 'cov-sample',
            viz: { type: 'flow', op: true, cap: R`With past data, the covariance **and** both SDs divide by \(N - 1\).`,
              steps: [{ t: R`\(Cov(R_i,R_j)\)`, s: R`sample: \(\div (N - 1)\)`, c: 1 }, { t: R`\(\sigma_i \times \sigma_j\)`, s: R`sample SDs: \(\div (N - 1)\)`, c: 2 }, { t: R`\(\rho_{ij}\)`, s: 'the correlation', c: 3 }], links: [R`\(\div\)`, R`\(=\)`] } },
          { kind: 'ti', title: 'Covariance and correlation on the TI-Nspire',
            body: R`Store the two return lists as \(x\) and \(y\). \(\text{dim}(x)\) counts the items in a list, so \(\text{dim}(x) - 1\) is \(N - 1\).\n\nThe third line is the sample covariance. Store it in \(c\), then divide by both sample SDs. These are the Nikkei and Russell 2000 returns from the tutorial.`,
            ti: [sto(NIK, 'x', { note: 'The Nikkei returns.' }), sto(RUS, 'y', { note: 'The Russell 2000 returns.' }), TI.line(`${COV_XY}→c`, { note: R`The covariance: \(${nt(covNR, 6)}\).` }), TI.line('c/(stDevSamp(x)*stDevSamp(y))', { note: R`The correlation: \(${nt(FIN.corrS(NIK, RUS), 4)}\).` })] },
          { kind: 'guided', title: 'Your turn', q: R`Share X returned 10%, −4% and 6% over three months. Share Y returned 8%, −2% and 3%. Treat them as a sample.`,
            parts: [
              { ask: 'What is the mean return of X?', answer: P(FIN.mean(G5.x)), unit: '%', dp: 2, hint: 'Add the three returns and divide by 3.', why: R`\(\frac{0.10 - 0.04 + 0.06}{3} = 0.04 = 4\%\).` },
              { ask: 'What is the mean return of Y?', answer: P(FIN.mean(G5.y)), unit: '%', dp: 2, hint: 'Add the three returns and divide by 3.', why: R`\(\frac{0.08 - 0.02 + 0.03}{3} = 0.03 = 3\%\).` },
              { ask: 'What is the covariance? (4 decimal places)', answer: FIN.covS(G5.x, G5.y), unit: '', dp: 4, hint: R`Deviations: X \(0.06, -0.08, 0.02\) and Y \(0.05, -0.05, 0\). Multiply the pairs, add, and divide by \(3 - 1 = 2\).`, why: R`\(\frac{0.003 + 0.004 + 0}{2} = ${nt(FIN.covS(G5.x, G5.y), 4)}\).`,
                mistakes: [{ v: FIN.covS(G5.x, G5.y) * 2 / 3, why: R`That divides by \(N = 3\). For a sample divide by \(N - 1 = 2\).` }] },
              { ask: R`The sample SDs are \(${nt(FIN.sdS(G5.x), 4)}\) and \(${nt(FIN.sdS(G5.y), 4)}\). What is the correlation? (4 decimal places)`, answer: FIN.corrS(G5.x, G5.y), unit: '', dp: 4, hint: 'Divide the covariance by both SDs multiplied together.', why: R`\(\rho = \frac{${nt(FIN.covS(G5.x, G5.y), 4)}}{${nt(FIN.sdS(G5.x), 4)} \times ${nt(FIN.sdS(G5.y), 4)}} \approx ${nt(FIN.corrS(G5.x, G5.y), 4)}\): a strong positive link.`,
                mistakes: [{ v: FIN.covS(G5.x, G5.y) / (FIN.sdS(G5.x) + FIN.sdS(G5.y)), why: 'Multiply the two SDs; do not add them.' }] },
            ],
            answer: R`\(Cov = ${nt(FIN.covS(G5.x, G5.y), 4)}\) and \(\rho = ${nt(FIN.corrS(G5.x, G5.y), 4)}\): X and Y move closely together.`,
            ti: [sto(G5.x, 'x'), sto(G5.y, 'y'), TI.line(`${COV_XY}→c`, { note: 'The sample covariance.' }), TI.line('c/(stDevSamp(x)*stDevSamp(y))')] },
          { kind: 'check', ref: 'w9-q35' },
          { kind: 'recap', title: 'Remember', points: [
            R`**Covariance**: do two returns move together (+) or in opposite directions (−)?`,
            R`**Correlation**: \(\rho = \frac{Cov}{\sigma_i\sigma_j}\), always between \(-1\) and \(+1\). And \(Cov = \rho\,\sigma_i\,\sigma_j\).`,
            R`Probabilities: weight each product by \(P_n\). Past data: divide by \(N - 1\).`,
            R`Exam trap: a correlation above 1 means a mistake. With past data use \(N - 1\) in the covariance **and** in both SDs.`,
          ], formula: 'corr' },
        ],
      },
      'w9-L6': {
        title: 'Portfolios: return and risk',
        goal: R`Work out a portfolio’s expected return and risk, and see why a low correlation lowers risk.`,
        topics: ['port', 'corr'],
        cards: [
          { kind: 'learn', title: 'Portfolio weights',
            body: R`A **portfolio** is a collection of investments held together.\n\nEach investment’s **weight** is its share of the total value, using **market values** (shares \(\times\) price): \(w_i = \frac{\text{value in asset } i}{\text{total value}}\). The weights add up to 1.`,
            tip: R`135 shares at $47 and 105 shares at $41 are worth $6,345 and $4,305. So \(w_A = \frac{6{,}345}{10{,}650} = ${pc(6345 / 10650)}\) and \(w_B = ${pc(4305 / 10650)}\).`,
            viz: { type: 'pie', center: `Total\n${T.moneyT(135 * 47 + 105 * 41)}`, cap: R`Each weight is its slice of the total value. The slices add up to 1 (100%).`,
              parts: [{ label: R`A: \(135 \times \$47 = ${L.moneyT(135 * 47)}\)`, v: 135 * 47, c: 1, show: R`\(${pc((135 * 47) / (135 * 47 + 105 * 41))}\)` },
                { label: R`B: \(105 \times \$41 = ${L.moneyT(105 * 41)}\)`, v: 105 * 41, c: 2, show: R`\(${pc((105 * 41) / (135 * 47 + 105 * 41))}\)` }] } },
          { kind: 'check', gen: 'w9-g-weights' },
          { kind: 'learn', title: 'Portfolio return: a weighted average',
            body: R`A portfolio’s expected return **is** a weighted average of the assets’ expected returns:\n\n\[E[R_p] = w_1E[R_1] + w_2E[R_2] + \cdots\]\n\nPut 60% in a share expected to return 8% and 40% in one expected to return 12%: \(0.6(8\%) + 0.4(12\%) = 9.6\%\).`,
            formula: 'port-ret',
            ti: [TI.line('0.6*0.08+0.4*0.12', PCT(0.096))],
            viz: { type: 'bars', fmt: '%', cap: R`\(0.6(8\%) + 0.4(12\%) = ${pc(FIN.portRet([0.6, 0.4], [0.08, 0.12]), 1)}\): between the two, nearer the share with the bigger weight.`,
              bars: [{ label: 'Share 1\n60%', v: 8, c: 1 }, { label: 'Share 2\n40%', v: 12, c: 2 }, { label: 'Portfolio', v: P(FIN.portRet([0.6, 0.4], [0.08, 0.12])), c: 3 }] } },
          { kind: 'check', gen: 'w9-g-port-ret' },
          { kind: 'learn', title: 'Portfolio risk is not a weighted average',
            body: R`Risk depends on how the assets move together. For two assets:\n\n\[\sigma_p^2 = w_1^2\sigma_1^2 + w_2^2\sigma_2^2 + 2w_1w_2\rho_{12}\sigma_1\sigma_2\]\n\nThe last part is the **covariance term** (\(\rho_{12}\sigma_1\sigma_2\) is the covariance). A low correlation makes it small. A negative correlation makes it subtract.`,
            formula: 'port-var',
            viz: { w: 360, type: 'lines', key: true, endLabels: false, cap: R`Only \(\rho = +1\) gives a weighted average (the straight line). The lower \(\rho\), the lower the curve.`,
              title: 'Two shares with SDs of 6.32% and 12.42%',
              x: { label: 'Weight in the riskier share', fmt: '%', min: 0, max: 100 }, y: { label: 'Portfolio SD', fmt: '%', min: 0, max: 13 },
              series: [{ name: R`\(\rho = +1\): weighted average`, c: 'grey', dash: true, pts: sdCurve(1) }, { name: R`\(\rho = 0\)`, c: 1, pts: sdCurve(0) }, { name: R`\(\rho = -1\)`, c: 2, pts: sdCurve(-1) }] } },
          { kind: 'example', title: 'Worked example', q: R`You invest $14,000 in Cheaters Anonymous (SD 6.32%) and $6,000 in Tricky Dicky (SD 12.42%). Their correlation is 0.9336. What is the SD of your portfolio?`,
            steps: [R`Weights: \(w_1 = \frac{14{,}000}{20{,}000} = 0.70\) and \(w_2 = 0.30\).`,
              R`First part: \(0.70^2 \times 0.0632^2 = ${nt(0.49 * 0.0632 * 0.0632, 6)}\).`,
              R`Second part: \(0.30^2 \times 0.1242^2 = ${nt(0.09 * 0.1242 * 0.1242, 6)}\).`,
              R`Covariance part: \(2 \times 0.70 \times 0.30 \times 0.9336 \times 0.0632 \times 0.1242 = ${nt(2 * 0.7 * 0.3 * 0.9336 * 0.0632 * 0.1242, 6)}\).`,
              R`Add: \(\sigma_p^2 = ${nt(0.49 * 0.0632 * 0.0632, 6)} + ${nt(0.09 * 0.1242 * 0.1242, 6)} + ${nt(2 * 0.7 * 0.3 * 0.9336 * 0.0632 * 0.1242, 6)} = ${nt(FIN.portVar2(0.7, 0.0632, 0.3, 0.1242, 0.9336), 6)}\).`,
              R`\(\sigma_p = \sqrt{${nt(FIN.portVar2(0.7, 0.0632, 0.3, 0.1242, 0.9336), 6)}} = ${pc(FIN.portSD2(0.7, 0.0632, 0.3, 0.1242, 0.9336))}\). The weighted average of the two SDs would be \(${pc(0.7 * 0.0632 + 0.3 * 0.1242)}\).`],
            answer: R`\(\sigma_p = ${pc(FIN.portSD2(0.7, 0.0632, 0.3, 0.1242, 0.9336))}\): a little below the weighted average, because \(\rho\) is just under 1.`,
          ...
          { kind: 'learn', title: 'Why a low correlation lowers risk',
            body: R`Keep the same two shares and the same weights, and change only the correlation. The lower the correlation, the lower the portfolio’s risk.`,
            table: RHO_TABLE,
            tip: R`With \(\rho = +1\) there is no risk reduction: the SD is just the weighted average, \(${pc(0.7 * 0.0632 + 0.3 * 0.1242)}\). Any \(\rho\) below 1 cancels some risk. This is **diversification**.`,
            viz: { type: 'bars', fmt: '%', dp: 2, cap: R`Same shares, same weights (70% and 30%). Each step down in \(\rho\) cuts the portfolio SD.`,
              bars: RHO_SET.map((rho) => ({ label: 'ρ =\n' + T.numT(rho, 4), v: P(FIN.portSD2(0.7, 0.0632, 0.3, 0.1242, rho)), c: rho === 1 ? 'grey' : 1 })) } },
          { kind: 'guided', title: 'Your turn, step by step', q: R`You put 50% of your money in share A (SD 20%) and 50% in share B (SD 30%). The correlation between them is 0.4. Find the portfolio SD.`,
            parts: [
              { ask: R`What is \(w_A^2\sigma_A^2 = (0.5)^2(0.20)^2\)? (4 decimal places)`, answer: 0.25 * 0.04, unit: '', dp: 4, hint: R`\(0.25 \times 0.04\).`, why: R`\(0.25 \times 0.04 = 0.01\).` },
              { ask: R`What is \(w_B^2\sigma_B^2 = (0.5)^2(0.30)^2\)? (4 decimal places)`, answer: 0.25 * 0.09, unit: '', dp: 4, hint: R`\(0.25 \times 0.09\).`, why: R`\(0.25 \times 0.09 = 0.0225\).` },
              { ask: R`What is the covariance term \(2(0.5)(0.5)(0.4)(0.20)(0.30)\)? (4 decimal places)`, answer: 2 * 0.5 * 0.5 * 0.4 * 0.2 * 0.3, unit: '', dp: 4, hint: 'Multiply all six numbers together.', why: R`\(2 \times 0.5 \times 0.5 \times 0.4 \times 0.20 \times 0.30 = 0.012\).` },
              { ask: R`Add the three parts and take the square root. What is \(\sigma_p\)?`, answer: P(Math.sqrt(G6_V)), unit: '%', dp: 2, hint: R`\(\sqrt{0.01 + 0.0225 + 0.012}\), then multiply by 100.`, why: R`\(\sqrt{${nt(G6_V, 4)}} = ${pc(Math.sqrt(G6_V))}\).`,
                mistakes: [{ v: 25, why: 'That is the weighted average of the SDs. With a correlation below 1, the portfolio is less risky.' }, { v: P(G6_V), why: 'That is the variance. Take the square root.' }] },
            ],
            answer: R`\(\sigma_p = ${pc(Math.sqrt(G6_V))}\), below the weighted average of \(25\%\).`,
            ti: [TI.line('0.5^2*0.2^2+0.5^2*0.3^2+2*0.5*0.5*0.4*0.2*0.3', { note: 'The portfolio variance.' }), TI.line('sqrt(ans)', PCT(Math.sqrt(G6_V)))] },
          { kind: 'learn', title: 'Adding the risk-free asset',
            body: R`The risk-free asset has \(\sigma = 0\) and does not move with anything. So every term with it in the formula is zero.\n\nWith a weight \(w\) in one risky asset and the rest risk-free, the risk is just \(\sigma_p = w\,\sigma\). The return still counts: \(E[R_p] = w\,E[R] + (1 - w)\,r_f\).`,
            tip: R`40% in asset A (SD 40%) and 60% risk-free: \(\sigma_p = 0.4 \times 40\% = 16\%\).`,
            viz: { w: 360, type: 'lines', cap: R`With the rest risk-free, risk is a straight line: \(\sigma_p = w\,\sigma\). At \(w = 40\%\) it is \(16\%\).`,
              title: 'Asset A (SD 40%) mixed with the risk-free asset',
          ...
          { kind: 'recap', title: 'Remember', points: [
            ...
            R`Portfolio risk is **not**: \(\sigma_p^2 = w_1^2\sigma_1^2 + w_2^2\sigma_2^2 + 2w_1w_2\rho\sigma_1\sigma_2\).`,
            R`The lower the correlation, the lower the risk. With \(\rho = +1\) there is no risk reduction.`,
            R`Exam trap: square the weights and the SDs, keep the 2 in the last term, and take the square root at the end.`,
          ], formula: 'port-var' },
        ],
      },
      'w9-L7': {
        title: 'Diversification: the risk you can remove',
        goal: R`Split risk into systematic and unsystematic risk, and explain which one investors are paid for.`,
        topics: ['divers'],
        cards: [
          { kind: 'learn', title: 'Do not put all your eggs in one basket',
            body: R`**Diversification** means spreading your money across many different investments, so that one bad result cannot sink you.\n\nOwning 50 internet shares is **not** diversified: they all share the same industry risk. Owning 50 shares from 20 different industries is.`,
            viz: { type: 'compare', cap: R`Both hold 50 shares. Only the mix of **different** industries is diversified.`,
              items: [{ icon: '💻💻💻', title: '50 internet shares', points: ['One industry', 'The same bad news hits them all'], mark: 'bad', markText: 'Not diversified', c: 2 },
                { icon: '🏭🛒✈️', title: '50 shares, 20 industries', points: ['Many kinds of business', 'One bad result cannot sink you'], mark: 'good', markText: 'Diversified', c: 1 }] } },
          { kind: 'learn', title: 'Two kinds of news',
            body: 'Share prices move on two kinds of news.',
            points: [R`**Firm-specific news**: a CEO resigns, a factory burns down, a new drug fails. It hits one firm (or one industry).`, R`**Market-wide news**: interest rates rise, a recession starts, inflation jumps. It hits almost every firm.`],
            viz: [
              { type: 'grid100', title: 'Firm-specific news', parts: [{ n: 1, c: 2, label: 'Hit: one firm' }], cap: R`Each square is a firm. A CEO resigns: only **one** firm is hit.` },
              { type: 'grid100', title: 'Market-wide news', parts: [{ n: 96, c: 1, label: 'Hit: almost every firm' }], cap: R`Interest rates rise: **almost every** firm is hit.` },
            ] },
          { kind: 'learn', title: 'Total risk has two parts',
            body: R`**Unsystematic risk** comes from firm-specific news. It is also called unique, firm-specific or **diversifiable** risk.\n\n**Systematic risk** comes from market-wide news. It is also called market or **non-diversifiable** risk.\n\n**Total risk = systematic risk + unsystematic risk.**`,
            viz: { type: 'split', total: 'Total risk of one share', cap: R`One share’s risk has two parts. Diversifying can remove the orange part, never the blue part.`,
              parts: [{ label: R`**Systematic**: market-wide news (non-diversifiable)`, v: P(DIV_VAR.sys), c: 1, show: '' }, { label: R`**Unsystematic**: firm-specific news (diversifiable)`, v: P(DIV_VAR.total(1) - DIV_VAR.sys), c: 2, show: '' }] } },
          { kind: 'example', title: 'Worked example: sort the news', q: 'Which kind of risk is each piece of news? (a) The Reserve Bank raises interest rates. (b) A company’s CEO suddenly resigns. (c) A recession hits the whole economy. (d) A fire destroys one firm’s factory.',
            steps: [R`(a) Interest rates affect almost every firm: **systematic**.`, R`(b) Only one firm is hit: **unsystematic**.`, R`(c) A recession cuts profits across the market: **systematic**.`, R`(d) Bad luck for one firm only: **unsystematic**.`],
            answer: 'Ask yourself: does this news hit the whole market, or just one firm?' },
          { kind: 'check', ref: 'w9-q61' },
          { kind: 'learn', title: 'More shares, less risk, up to a point',
            body: R`In a portfolio, firm-specific surprises tend to cancel out: one firm’s bad news is balanced by another’s good news. So the portfolio SD falls as you add shares.\n\nIt never falls to zero. It levels off at the systematic risk that all the shares have in common. In this example each share has an SD of 40% and a correlation of 0.25 with the others, and the portfolio SD levels off near ${tp(divSD(1e9), 0)}.`,
            table: DIV_TABLE,
            tip: R`About **12 to 16 shares** from different industries remove most unsystematic risk (Fama, 1976).`,
            viz: { w: 360, type: 'lines', key: true, cap: R`Risk falls fast, then levels off near \(${pc(divSD(1e9), 0)}\): the systematic risk that no number of shares removes.`,
              x: { label: 'Number of shares', min: 1, max: 40, ticks: [1, 10, 20, 30, 40] }, y: { label: 'Portfolio SD', fmt: '%', min: 0, max: 42 },
              series: [{ c: 1, pts: Array.from({ length: 40 }, (_, k) => [k + 1, P(divSD(k + 1))]) }, { c: 'grey', dash: true, pts: [[1, P(divSD(1e9))], [40, P(divSD(1e9))]] }],
              marks: [{ x: 30, y: P(divSD(1e9)), label: 'Systematic risk', c: 'grey', pos: 'below' }], shade: [{ x1: 12, x2: 16, c: 'good', label: '12–16 shares' }] } },
          { kind: 'check', ref: 'w9-q56' },
          { kind: 'learn', title: 'Only systematic risk is rewarded',
            body: R`Unsystematic risk can be removed for free, just by diversifying. So the market does not pay you for bearing it.\n\nThe **risk premium** (the extra expected return for bearing risk) depends only on **systematic** risk. That is why a single share’s total SD does not predict its return.`,
            viz: { type: 'split', scale: 'abs', cap: R`Diversifying removes the orange part for free. The blue part stays in every portfolio, so only it earns a reward.`,
              rows: [1, 30].map((n) => ({ label: n === 1 ? 'One share' : '30 shares', parts: [{ label: R`**Systematic**: earns a risk premium`, v: P(DIV_VAR.sys), c: 1, show: '' }, { label: R`**Unsystematic**: earns nothing`, v: P(DIV_VAR.total(n) - DIV_VAR.sys), c: 2, show: '' }] })) } },
          { kind: 'check', ref: 'w9-q58' },
          { kind: 'check', ref: 'w9-q60' },
          { kind: 'recap', title: 'Remember', points: [
            R`**Diversification**: spread your money across many different investments.`,
            R`**Unsystematic** (unique, diversifiable) risk comes from firm-specific news. Diversification removes it.`,
            R`**Systematic** (market, non-diversifiable) risk comes from economy-wide news. It stays.`,
            R`About 12 to 16 shares from different industries remove most unsystematic risk.`,
            R`Exam trap: only systematic risk earns a risk premium, and no number of shares removes it.`,
          ] },
        ],
      },
      'w9-L8': {
        title: 'Beta, CAPM and the Security Market Line',
        goal: R`Measure systematic risk with beta, and use CAPM to find the return a share should offer.`,
        topics: ['beta', 'capm'],
        cards: [
          { kind: 'learn', title: 'Beta measures systematic risk',
            body: R`Systematic risk is the risk that is rewarded. **Beta**, \(\beta\), measures it: how strongly a share moves when the whole market moves.\n\n\[\beta_i = \frac{Cov(R_i,R_M)}{\sigma_M^2}\]\n\n\(R_M\) is the return on the **market portfolio** (all shares together), and \(\sigma_M^2\) is its variance.`,
            points: [R`\(\beta = 1\): as risky as the market.`, R`\(\beta > 1\): riskier than the market (it swings more).`, R`\(\beta < 1\): less risky than the market.`],
            formula: 'beta',
            viz: { w: 360, type: 'lines', keys: [], title: R`The grey line, \(\beta = 1\), moves exactly with the market`, cap: R`When the market moves \(10\%\), a \(\beta = 1.5\) share tends to move \(15\%\); a \(\beta = 0.5\) share, \(5\%\).`,
              x: { label: 'Market return', fmt: '%', min: -10, max: 10 }, y: { label: 'Share return', fmt: '%' },
              series: [1.5, 1, 0.5].map((b) => ({ name: 'β = ' + b, c: b === 1 ? 'grey' : b > 1 ? 2 : 1, dash: b === 1, pts: [[-10, -10 * b], [10, 10 * b]] })) } },
          { kind: 'example', title: 'Worked example: beta from a matrix', q: R`Here is part of a **variance–covariance matrix**. The diagonal holds each asset’s variance, and the other cells hold covariances. What is the beta of SSBB?`,
            table: { head: ['', 'SSBB', 'Market'], rows: [['SSBB', '0.0221', ''], ['Market', '0.0040', '0.0100']] },
            steps: [R`Read \(Cov(R_{SSBB},R_M) = 0.0040\) from the market row, and the market’s variance \(\sigma_M^2 = 0.0100\) from the diagonal.`, R`\(\beta_{SSBB} = \frac{0.0040}{0.0100} = ${nt(bSS, 2)}\).`],
            answer: R`\(\beta = ${nt(bSS, 2)}\): SSBB is much less risky than the market.`,
            ti: [TI.line('0.0040/0.0100')] },
          { kind: 'learn', title: 'Portfolio beta',
            body: R`A portfolio’s beta is simply the weighted average of the betas:\n\n\[\beta_p = w_1\beta_1 + w_2\beta_2 + \cdots + w_n\beta_n\]\n\nPut 60% in a share with \(\beta = 1.7\) and 40% in one with \(\beta = 1.0\): \(\beta_p = 0.6(1.7) + 0.4(1.0) = ${nt(FIN.portBeta([0.6, 0.4], [1.7, 1.0]), 2)}\).`,
            formula: 'port-beta',
            tip: R`A risk-free asset has \(\beta = 0\), but it still counts in the weights.`,
            viz: { type: 'bars', dp: 2, cap: R`\(0.6(1.7) + 0.4(1.0) = ${nt(FIN.portBeta([0.6, 0.4], [1.7, 1.0]), 2)}\): a weighted average, just like the portfolio return.`,
              bars: [{ label: 'Share 1\n60%', v: 1.7, c: 1 }, { label: 'Share 2\n40%', v: 1.0, c: 2 }, { label: 'Portfolio', v: FIN.portBeta([0.6, 0.4], [1.7, 1.0]), c: 3 }] } },
          { kind: 'check', gen: 'w9-g-port-beta' },
          { kind: 'learn', title: 'CAPM: the return a share should offer',
            body: R`The **Capital Asset Pricing Model** (CAPM) gives the expected return that a share’s beta deserves:\n\n\[E[R_i] = r_f + \beta_i\left(E[R_M] - r_f\right)\]\n\nThe bracket, \(E[R_M] - r_f\), is the **market risk premium**: the extra return the market pays above the risk-free rate.`,
            points: [R`\(\beta = 0\): you get just \(r_f\).`, R`\(\beta = 1\): you get the market return, \(E[R_M]\).`],
            formula: 'capm',
            viz: [
              { type: 'anatomy', cap: R`Start from \(r_f\). Then add beta times the market risk premium.`, tex: R`\small E[R_i] = \colA{r_f} + \colB{\beta_i}\,\colC{(E[R_M] - r_f)}`,
                parts: [{ sym: 'r_f', say: 'the risk-free rate: paid to everyone', c: 'A' }, { sym: R`\beta_i`, say: 'how much systematic risk the share has', c: 'B' }, { sym: R`E[R_M] - r_f`, say: 'the market risk premium: the reward for each unit of beta', c: 'C' }] },
              { type: 'bars', fmt: '%', cap: R`With \(r_f = 5\%\) and \(E[R_M] = 11\%\): the orange risk premium grows with beta.`,
                bars: [0, 1, 1.5].map((b) => ({ label: 'β = ' + b, parts: [{ v: 5, c: 1 }, { v: P(b * (0.11 - 0.05)), c: 2 }] })),
                keys: [{ c: 1, label: R`\(r_f\)` }, { c: 2, label: R`\(\beta \times 6\%\) premium` }] },
            ] },
          { kind: 'example', title: 'Worked example', q: R`A share has a beta of 1.5. The expected market return is 11% and the risk-free rate is 5%. Using CAPM, what should the share return?`,
            steps: [R`Market risk premium: \(11\% - 5\% = 6\%\).`, R`This share’s risk premium: \(1.5 \times 6\% = 9\%\).`, R`Add the risk-free rate: \(5\% + 9\% = 14\%\).`],
            answer: R`\(E[R] = 14\%\).`,
            ti: [TI.line('0.05+1.5*(0.11-0.05)', PCT(0.14))] },
          { kind: 'guided', title: 'Your turn', q: R`A share has a beta of 1.3. The risk-free rate is 3% and the expected market return is 9%. Use CAPM.`,
            parts: [
              { ask: 'What is the market risk premium?', answer: P(G8.rm - G8.rf), unit: '%', dp: 2, hint: R`\(E[R_M] - r_f\).`, why: R`\(9\% - 3\% = 6\%\).` },
              { ask: R`What is this share’s risk premium, \(\beta \times\) the market risk premium?`, answer: P(G8.b * (G8.rm - G8.rf)), unit: '%', dp: 2, hint: R`\(1.3 \times 6\%\).`, why: R`\(1.3 \times 6\% = 7.8\%\).` },
              { ask: 'What is the expected return on the share?', answer: P(FIN.capm(G8.rf, G8.b, G8.rm)), unit: '%', dp: 2, hint: R`Add \(r_f\).`, why: R`\(3\% + 7.8\% = 10.8\%\).`,
                mistakes: [{ v: P(G8.rf + G8.b * G8.rm), why: R`Multiply beta by the premium \((E[R_M] - r_f)\), not by \(E[R_M]\).` }, { v: P(G8.b * (G8.rm - G8.rf)), why: R`That is only the risk premium. Add \(r_f\).` }] },
            ],
            answer: R`\(E[R] = 3\% + 1.3(6\%) = ${pc(FIN.capm(G8.rf, G8.b, G8.rm), 1)}\).`,
            ti: [TI.line('0.03+1.3*(0.09-0.03)', PCT(FIN.capm(G8.rf, G8.b, G8.rm)))] },
          { kind: 'learn', title: 'The Security Market Line',
            body: R`Draw CAPM as a chart: beta along the bottom, expected return up the side. The straight line is the **Security Market Line** (SML). It starts at \(r_f\) and passes through the market (M) at \(\beta = 1\). Here \(r_f = 4\%\) and \(E[R_M] = 10\%\).\n\nA fairly priced share sits **on** the line. A share **above** the line offers more than CAPM requires: it is **undervalued**, so buy. A share **below** the line is **overvalued**, so sell.`,
            viz: { w: 360, type: 'lines', key: true, cap: R`Share A sits **above** the line: undervalued, buy. Share B sits **below** it: overvalued, sell.`,
              x: { label: 'Beta (β)', min: 0, max: 2, ticks: [0, 0.5, 1, 1.5, 2] }, y: { label: 'Expected return', fmt: '%', min: 0, max: 18 },
              series: [{ name: 'SML', c: 1, pts: smlPts(0.04, 0.06) }],
              marks: [{ x: 0, y: 4, label: 'r_f', c: 'grey', pos: 'right' }, { x: 1, y: 10, label: 'M', c: 1, pos: 'below' },
                { x: 0.6, y: 11, label: 'A: buy', c: 'good' }, { x: 1.5, y: 10, label: 'B: sell', c: 'bad', pos: 'below' }] },
            tip: R`Share A needs only \(4\% + 0.6(6\%) = ${pc(FIN.capm(0.04, 0.6, 0.10), 1)}\), but it is expected to earn 11%: above the line, so undervalued. Share B needs \(4\% + 1.5(6\%) = ${pc(FIN.capm(0.04, 1.5, 0.10), 0)}\), but it is expected to earn only 10%: below the line, so overvalued.` },
          { kind: 'check', gen: 'w9-g-sml' },
          { kind: 'learn', title: 'When the SML moves',
            body: 'The SML is not fixed. Two kinds of change move it.',
            points: [R`Expected **inflation** rises by 3%: \(r_f\) and \(E[R_M]\) both rise by 3%. The SML shifts **up in parallel**.`, R`Investors become more **risk averse**: the market risk premium rises. The SML gets **steeper**, and high-beta shares need the biggest increase.`],
            viz: [
              { type: 'lines', title: 'Inflation up 3%', cap: R`\(r_f\) and \(E[R_M]\) both rise \(3\%\): the line moves **up in parallel**.`, keys: [],
                x: { label: 'Beta', min: 0, max: 2 }, y: { label: 'Expected return', fmt: '%', min: 0, max: 22 },
                series: [{ name: 'Before', c: 'grey', dash: true, pts: smlPts(0.04, 0.06) }, { name: 'After', c: 2, pts: smlPts(0.07, 0.06) }] },
              { type: 'lines', title: 'More risk averse', cap: R`Say the market risk premium rises from \(6\%\) to \(8\%\): the line gets **steeper**. High betas rise most.`, keys: [],
                x: { label: 'Beta', min: 0, max: 2 }, y: { label: 'Expected return', fmt: '%', min: 0, max: 22 },
                series: [{ name: 'Before', c: 'grey', dash: true, pts: smlPts(0.04, 0.06) }, { name: 'After', c: 2, pts: smlPts(0.04, 0.08) }] },
            ] },
          { kind: 'check', gen: 'w9-g-sml-shift' },
          { kind: 'recap', title: 'Remember', points: [
            R`**Beta** measures systematic risk: \(\beta_i = \frac{Cov(R_i,R_M)}{\sigma_M^2}\). The market’s beta is 1.`,
            R`**Portfolio beta** is the weighted average of the betas.`,
            R`**CAPM**: \(E[R_i] = r_f + \beta_i(E[R_M] - r_f)\).`,
